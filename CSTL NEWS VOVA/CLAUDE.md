...
| БД: подання `page_comment_counts` | ✅ **НАКАТАНО 25.07** (міграція `page_comment_counts_view`, `security_invoker = on` → RLS лишається чинною). Числа коментарів по постах, рядок на пост. Стрічка тягне його замість усіх коментарів усіх постів; самі коментарі — лише при відкритті листа (`fetchPostComments`, сторінками по кореневих). |
