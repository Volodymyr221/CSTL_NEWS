...
## 🚀 Поточний стан проекту (на 2026-07-25)

> 🔴 **НОВИЙ ЧАТ ПОЧИНАЙ ЗВІДСИ:** повний хендовер — верхній блок
> `_ai-tools/SESSION_STATE_VOVA.md`. Там: що зроблено, що лишилось (6 задач), уроки й пастки.
> Докладний журнал із причинами кожного рішення — `_session-log/vova-2026-07-25.md`.

**Остання робота (25.07, вечір):** блок коментарів «Стрічки» — лінія-гілка як у Facebook,
згортання «Ще N» ⇄ «Сховати відповіді», структура рядка (імʼя+час зверху, текст під ними),
згадка «Віктор,», сповіщення (відповідь одразу + адмінам з накопиченням), каскадне видалення
гілки. PR #619–#622, деплой run 3341, CACHE `cstl-20260725-2004`.

**🔴 Найгостріше зараз:**
1. **Живий тест** усього блоку коментарів на телефоні (= **СТ-2**) — Вова ще нічого не бачив
   наживо. Чеклист готовий: `_ai-tools/ЖИВИЙ_ТЕСТ_КОМЕНТАРІ.md` (6 блоків).
2. **СИ-9** — створення спільнот з адмінки (`admin.html`). Заявлено Вовою як «наступна головна
   задача». БД готова, RLS міняти не треба; перед кодом — 3 питання до Вови.
3. **Зсув шапки під клавіатурою** — стан невідомий після фіксів #615/#616. Потрібен скрін
   із `#kbdebug` у адресі (режим діагностики з живими числами з айфона).

> ✅ **B-23 закрито 25.07** (тригер `trg_page_comments_guard_update`).
> ✅ **B-24 закрито 25.07** — `send-chat-push` задеплоєно, v3 на проді. Відкритих 🔴 багів нема.
