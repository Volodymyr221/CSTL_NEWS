...
| Файл | Коли читати |
|------|-------------|
| `RODMAP.md` | 🗺️ **Загальний стратегічний роадмап** (CASTLE LIFE, 8 етапів: MVP → тест → маркетинг → аналітика → v2/v3 → монетизація → МАРКЕТ/БАЗАР → масштабування на інші громади). Для всіх причетних (Вова/Рома/Алла) |
| `_ai-tools/AUDIT_2026-07.md` | 📋 **Майстер-список задач по вкладках** (за пріоритетом MVP/V2, з розподілом [Вова]/[Рома]). Джерело істини для «що робити». |
| `_ai-tools/ПРОПОЗИЦІЇ.md` | 📝 **Список пропозицій між власниками** (по вкладках). Записуй сюди пропозицію/виправлення по ЧУЖІЙ вкладці, щоб відповідальний побачив. |
...
- **🆕 НАВІГАЦІЯ «ЧАТИ» + ПРИВАТНІ ГРУПИ — ЗАВЕРШЕНО ✅ (25-27.06)** (план `docs/CHATS_NAV_PLAN.md`)
  ...
  - Деталі — `_ai-tools/SESSION_STATE.md` (верхній запис ⭐ ХЕНДОВЕР)
...
> у дужках · bump `CACHE_NAME` при зміні коду · оновлюй `SESSION_STATE.md`.
