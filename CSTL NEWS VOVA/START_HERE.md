...
## 🚀 Поточний стан проекту (на 2026-08-02, сесія U-5)

> 🔴 **НОВИЙ ЧАТ ПОЧИНАЙ ЗВІДСИ:** повний стан — верхні блоки
> `_ai-tools/SESSION_STATE_VOVA.md` (сесії **U** і **T**, обидві за 01.08).
> Журнали — `_session-log/vova-2026-08-01.md` і `vova-2026-07-29b.md`.

**Прод:** PR **#738** → `main`, `CACHE_NAME = cstl-20260802-0108`.
Стенди — **31 / 466 перевірок**. Відкритих 🔴 багів нема. Додаток досі закритий
заслінкою розробки (`src/core/dev-lock.js`, `DEV_LOCK = true`).

**Зроблено 01-02.08 (пʼять потоків):**
...
2. **Табло новин** (#735): прилад **знято вранці й повернуто ввечері** — рішення
   Вови по знімках; картка дістала обідок сторінки новин.
3. **🔴 ДОШКА — ПРОЩАННЯ З КОРКОМ** (#736): дві колонки «стікерів» зі шпильками →
   **один стовпчик** карток-рядків (фото 76px зліва, тип, час, заголовок, локація,
   ціна). Повернулись назва «ДОШКА ОГОЛОШЕНЬ» і слоган-дієслова, зʼявились кнопка
   «+» у шапці й ряд чіпів типів із лічильниками (порожні категорії не малюються).
   Привід — концепт від ChatGPT, який прислав Вова; рішення: «Попрощаємось».
   📐 Шапка 79→180px, картка 96px, видно 6 карток. Розбір → `_ai-tools/AUDIT_DOSHKA_2026-08.md`.
4. **🔴 ДОШКА — ДІЇ ВНИЗУ, ЛИСТУВАННЯ ВГОРІ** (#737): подати оголошення — найчастіша
   дія, тож вона в НИЖНЬОМУ FAB (зона великого пальця), а конверт із бейджем — у шапці
   (постійне місце). Показ конверта: `canSeeMessages()` = **є оголошення АБО є розмова**
   — правило «лише автори» відрізало б 7 із 8 людей від їхнього ж листування.
5. **🔴 МОДАЛКА ОГОЛОШЕННЯ** (#738): повноекранний аркуш знизу (**776 з 844**) замість
   картки на 300px · hero-фото 38% · **липка панель «Подзвонити/Написати»** (раніше при
   описі до 1296 символів кнопки їхали за екран) · блок безпеки · картка автора з фото
   і переходом у профіль. ⏸ Перегляди, рейтинг і характеристики свідомо НЕ робили —
   таких даних у базі немає (розбір: `_ai-tools/AUDIT_DOSHKA_2026-08.md`, частина 3).

🔑 **Правила, які треба знати перед дотиком до новин:**
- Форма картки живе ТІЛЬКИ в `style/news-card.css`; екран задає лише токени
  `--nc-*`. Сторож — `tests/news-widget.mjs`.
- **Дошка: розкладку дій за добу перевертали двічі** — не міняй її «бо так логічніше»
  без нового слова Вови. Чинне: дії внизу, конверт у шапці; причина — зона досяжності
  великого пальця (верхній правий кут найважчий). Сторож — `tests/board-fab-icon.mjs`.
- **Дошка: клас `cm-board-note` на картці ЛИШИВСЯ** попри новий вигляд — за ним
  чіпляється розгортання в зум-модалку. Приберете «щоб було чисто» — тап по
  оголошенню перестане відкривати картку. Увесь новий вигляд scoped під
  `#board-content`, бо той самий клас носять віджет Громади, прев'ю подачі й зум-модалка.
- **Віджет «ТАБЛО НОВИН» — багряний прилад, і це вже перевертали двічі за день.**
  Знімаєш прилад — знімай і силует замку (темні вежі на світлому читаються як три
  сірі прямокутники) і перевертай кольори тексту. Порівняти дороги, не чіпаючи
  бойовий код, — `tests/tools/news-road1.mjs`.

⬜ **Чекає живої перевірки Вови:** вигляд табла на iPhone; фото в приватних чатах
(старі й нові) після переїзду в закритий бакет; два секрети для бекапу
(`SUPABASE_DB_URL` — саме Session pooler, і `BACKUP_PASSPHRASE`) — без них
бекап жодного разу не запускався.
