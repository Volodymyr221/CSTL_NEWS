...
## 🚀 Поточний стан проекту (на 2026-07-26)

> 🔴 **НОВИЙ ЧАТ ПОЧИНАЙ ЗВІДСИ:** повний хендовер — блок **⭐ ХЕНДОВЕР У НОВИЙ ЧАТ**
> на самому верху `_ai-tools/SESSION_STATE_VOVA.md`. Там: єдина відкрита задача з усією
> історією спроб, git-пастки, як перевіряти. Докладний журнал із причинами кожного
> рішення — `_session-log/vova-2026-07-25c.md`.

**Прод:** деплой **run 3377** ✅, `CACHE_NAME = cstl-20260726-1125`.
Робоча гілка сесії — `claude/startuem-k2kzci`, змерджена в `main` (PR #642).

**Зроблено 26.07 (усе на сайті):** фото-пуш у чаті (B-24) · клавіатура у **встановленому**
додатку (модуль не вмикався взагалі — знав лише один із двох способів iOS) · статичний задник
під клавіатурою · дві фази згортання топбару «Стрічки» · дата поста «23 липня» · обмін
прихованих входів у шапці · комплексна обробка мережевих збоїв (`netCall`) · **три рівні
коментарів як у Facebook** із суцільною лінією-гілкою.

**🔴 Єдина відкрита задача — залишковий рух верху при ПІДЙОМІ клавіатури.**
Дві спроби вже провалились, друга зламала базову дію (тап у поле закривав лист коментарів)
і відкочена. **Не починай з нуля — прочитай блок хендовера,** там механіка, живі числа з
айфона і напрями, які ще не пробували.

**Що вирішено і закрито (не переобговорювати):** глибина коментарів — **3 рівні** (як
Facebook), реалізовано й задеплоєно 26.07.

> ✅ **B-23 і B-24 закриті 25.07.** Відкритих 🔴 багів нема.

**ℹ️ Supabase MCP працює** (проєкт `uabyfecseqnemvcqhdem`) — через нього можна накатувати
міграції, деплоїти Edge Functions і прогонити `get_advisors`.
