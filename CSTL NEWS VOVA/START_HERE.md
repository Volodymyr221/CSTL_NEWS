...
## 🚀 Поточний стан проекту (на 2026-08-03, вечір, сесія U-9)
...
### 🔙 ГРОМАДУ ВІДКОЧЕНО, БРЕНД-КОЛІР ЗАЛИШЕНО

03.08 Громаду переробляли **пʼять разів** (картки → дашборд «ПУЛЬС» → фото тлом →
фото лише в плитці → новини на весь екран). Вова відхилив усі пʼять і сказав
повернути ранкову версію. PR **#766** відкотив, PR **#767** повернув назад
бренд-градієнт — за прямою поправкою Вови: «Відкотити треба було лише громаду,
кольори бордові які ми налаштовували треба залишити».

🛑 **Громада зараз = версія до 03.08:** фото 560px, липке «ШО В СЕЛІ?», карусель
Дошки на корку, карусель подій, погода окремим блоком унизу, контакти 420px.
Файлів `src/tabs/home-*.js`, `style/home.css`, `style/home-blocks.css`,
`tests/community-home.mjs` **у репозиторії НЕМА** — вони живі лише в PR
#757 · #758 · #759 · #760 · #762 · #765.

> 🔙 **ДВІ ТОЧКИ ВІДКАТУ** (пряме прохання Вови — не видаляти):
> `backup/community-pre-redesign-20260803` (`c81f744e`) — оригінал (= чинний прод);
> `backup/community-v2-cards-20260803` (`65282c86`) — варіант 2 (список карток).
...
Угорі «+» (меню FAB виїжджає вниз), унизу — конверт повідомлень. Код готовий,
стенди зелені, **але «+» у шапці не намалювався** і тому НЕ задеплоєно.
Діагноз: на `#board-trigger` тепер зійшлись стилі шапки (`.bd-hero-msgs`) і стилі
нижнього круга (`.board-trigger--fixed`, `position: fixed`). Наступний крок —
подивитись у стенді, які правила виграють, і розвести їх. **Одна правка стилів.**
...
- `data/fundraisers.json` лишився в репозиторії, але його **ніхто не читає** —
  `home-fund.js` зник із відкотом.
...
4. **Блок «Актуальні збори»** — замовлення Вови, зроблене й **втрачене відкатом**
   Громади. Код живий у PR #757 (`src/tabs/home-fund.js`), дані — `data/fundraisers.json`
   (лишився в репозиторії, порожній). Повертати разом із наступним підходом до Громади.
