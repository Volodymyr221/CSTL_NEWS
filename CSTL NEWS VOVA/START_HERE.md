...
## 🚀 Поточний стан проекту (на 2026-07-26, сесія F)

> 🔴 **НОВИЙ ЧАТ ПОЧИНАЙ ЗВІДСИ:** повний хендовер — блок **⭐ ХЕНДОВЕР У НОВИЙ ЧАТ**
> на самому верху `_ai-tools/SESSION_STATE_VOVA.md`. Там усе за сесію F з причинами
> кожного рішення. Докладний журнал — `_session-log/vova-2026-07-26b.md`.

**Прод:** PR **#656** → `main` (`eb719763`), `CACHE_NAME = cstl-20260726-1615`.
Робоча гілка сесії — `claude/startuem-7g0why`. Відкритих 🔴 багів нема.

**Зроблено 26.07 у сесії F (чотири деплої):**
1. **Мережеві збої** — ядро `netCall`/`netInsert` накрило **всі 56 записів** у базу (було 6).
   Обрив зв'язку більше не коштує написаного тексту, а повтор вставки не створює дубля
   (звірка за `client_tag` перед повтором).
2. **Лічильник коментарів (B-27)** — «пише 2, а коментар один». Корінь: жива відповідь під
   ВИДАЛЕНИМ коментарем. Закрито **трьома рубежами**: заборона на вставці · подання рахує лише
   досяжне · нічне самозаліковування (`pg_cron` 03:15).
3. **Жива синхронізація постів** — підписки на `page_posts` і `posts` (їх не було взагалі).
   Новий пост «Стрічки» приходить пігулкою «↑ N нових публікацій».
4. **UX «Стрічки»** — гілки згортаються глибше (видно корінь + одну відповідь) · біла підкладка
   під компоузером (стара була тінню за межами екрана) · лист розширюється до верху при
   відкритті клавіатури + три рівні свайпу шапки.

**🔴 Чекає живого тесту Вови:** синхронізація між двома пристроями і поведінка клавіатури
на iPhone — з пісочниці це недосяжно (мережа не пускає до Supabase, Chromium не відтворює
клавіатуру iOS).

**Велика незакрита задача:** ревізія `AUDIT_2026-07.md` (~60 пунктів висять, частина вже
зроблена) — метод узгоджено з Вовою, див. хендовер.

**ℹ️ Supabase MCP працює** (проєкт `uabyfecseqnemvcqhdem`) — міграції, читання бази,
Edge Functions. У сесії F через нього накатано 3 міграції й доведено корінь B-27.
