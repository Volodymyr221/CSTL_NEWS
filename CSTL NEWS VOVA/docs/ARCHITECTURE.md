...
> 🔑 **КАРТА ФАЙЛІВ ЖИВЕ В ОДНОМУ МІСЦІ — `START_HERE.md`, розділ «Карта файлів проекту».**
>
> ⚠️ **Тут була ДРУГА КОПІЯ карти, і 06.08 аудит знайшов її застарілою по всіх розділах**
> (`src/core/` — 3 модулі замість 31, workflows — 1 замість 9, `_ai-tools/` — 2 файли,
> плюс неіснуючий `data/curated.json`). Копію **прибрано, а не оновлено**, і це свідомо:
> дві карти того самого завжди розходяться — рівно так, як у цьому проєкті вже
> розходились два списки антиспаму і два описи `messages-ui.js`.
>
> **Цей файл — про те, ЯК ЩО ПРАЦЮЄ** (збірка, деплой, кеш, потік даних).
> **`START_HERE.md` — про те, ДЕ ЩО ЛЕЖИТЬ.** Не змішувати.
...
⚠️ **Виправлено 06.08:** тут стояло «Окремо: `data/curated.json` — ручні ексклюзиви Олики,
мерджаться з `articles.json` на рівні парсера». **Такого файлу не існує** — окремого
контуру для ручних статей не збудували. Ручні ексклюзиви лежать **у самому
`data/articles.json`** із прапорцем `exclusive: true`, поряд із авто-RSS.
🔑 Наслідок, який варто памʼятати: парсер пише в **той самий** файл, тож будь-яка
автоматика над `articles.json` мусить зберігати чужі записи, а не перезаписувати масив.
...
│                                            │
│  Ручне:                                    │
│  - data/articles.json (правка в GitHub)   │
│  - AI-агент → чернетка → публікує Вова     │
│                                            │
└────────────┬───────────────────────────────┘
             │
             ↓
┌─ GitHub Actions cron (кожні 30 хв) ───────┐
...
│  6. Дописати у data/articles.json,         │
│     ЗБЕРІГАЮЧИ ручні статті                │
│  7. health_check.py                        │
│  8. git commit + push → штовхнути deploy   │
└────────────┬───────────────────────────────┘
             │
             ↓
