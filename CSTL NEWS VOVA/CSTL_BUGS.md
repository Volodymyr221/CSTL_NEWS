...
| # | Файл | Опис | Рішення |
|---|------|------|---------|
| **B-05** | `src/tabs/buses.js:16-30` | `getNextDeparture()` ігнорує поле `days`. У неділю показує "через X хв" для рейсів "пн-сб", хоча їх не буде. | Фаза 2.4. Додати перевірку поточного дня тижня перед тим як пропонувати рейс. |
