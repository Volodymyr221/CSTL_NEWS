...
## 0. ЕТАЛОН ДО ЗМІН — прогін `scripts/security_regression.sql` (09.08)

**15 ✅ · 6 🔴 · 2 ℹ️** (відомі, закріплені за потоком 2)

| роль | перевірка | очікувано | фактично |
|---|---|---|---|
| анонім | RPC `heal_orphan_page_comments` | заборонено | 🔴 **виконалась** |
| анонім | RPC `flush_page_comment_push` | заборонено | 🔴 **виконалась** |
| анонім | `get_avatars` має стелю на масив | так | 🔴 ні |
| користувач А | RPC `heal_orphan_page_comments` | заборонено | 🔴 **виконалась** |
| користувач А | підпис коментаря бере сервер | `Володимир` | 🔴 **`ПІДРОБКА · Адміністрація`** |
| користувач А | підпис допису Обговорень бере сервер | `Володимир` | 🔴 **`ПІДРОБКА · Сільрада`** |
| анонім | телефони дошки | 0 | ℹ️ 12 (потік 2, крок 10) |
| анонім | видно хто лайкнув | 0 | ℹ️ 5 uid (потік 2, крок 9) |

✅ Тримає в усіх чотирьох ролях: профілі · приватні повідомлення · групові
повідомлення · треди · `ads` · чужі push-підписки · сторож `is_admin()`.

🔑 **Підробка автора — не теорія.** Проба справді вставила рядки з чужим
підписом і база їх прийняла. Транзакцію відкочено, у базі нічого не лишилось.

## 0.1 ПІСЛЯ ПОТОКУ 1 — той самий прогін

**25 ✅ · 0 🔴 · 2 ℹ️** — усі шість провалів закрито, жодної регресії.

| що змінилось | було | стало |
|---|---|---|
| підпис коментаря | `ПІДРОБКА · Адміністрація` | `Володимир` (з `profiles`) |
| підпис допису Обговорень | `ПІДРОБКА · Сільрада` | `Володимир` |
| обхід через `UPDATE author` | спрацьовував | перезаписується сервером |
| `heal_orphan_page_comments` для аноніма | виконалась | заборонено |
| `flush_page_comment_push` для аноніма | виконалась | заборонено |
| `get_avatars` без межі | без стелі | стеля 500 uid |
| `ads` з живим рядком: телефон для аноніма | видно було б | таблиця недосяжна, вітрина без контактів |

Контролі, які доводять, що правило не «прибило все підряд»:
- легасі-оголошення з автором «**Іван**» (`owner_uid = null`) пережило UPDATE —
  тригер справді пропускає рядки без uid;
- користувач Б, який підписався «Володимир», отримав «**Макс Безушкевич**» —
  тригер бере ім'я за реальним uid, а не константу;
- **адмін бачить `client_phone` і `paid_amount`** — адмінка не осліпла;
- анонім бачить Дошку і Стрічку; `get_avatars` віддає профіль.

⚠️ **Перша редакція перевірки `ads` брехала.** «Анонім бачить 0 рядків» проходило
вхолосту — у таблиці **0 рядків узагалі**. Переміряно зі справжнім рядком
(`ТОВ Ромашка`, `+380501112233`, 2500 грн), і лише тоді перевірка щось довела.
Це десятий випадок брехливої перевірки в проєкті — той самий клас.

**Радник Supabase: 88 WARN → 79.** Зникли всі 7 `search_path` і обидві службові
RPC. З'явився **ERROR `security_definer_view` на `ads_public`** — розбір і чому
замін немає у `scripts/security_regression.sql` (сторож на колонки подання).

`npm test` — **54 стенди, 942 перевірки**, зелено.
