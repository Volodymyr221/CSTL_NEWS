...
### 🆕 Дошка громади — фінальна концепція (затверджено 13.06.2026, «Роби»)

> **Задача №1.** Повний детальний план: `CSTL NEWS VOVA/docs/BOARD_FINAL_PLAN.md`.
> Реалізація ще не почата (план збережено в документацію перед переходом в інший чат).

| ✓ | Задача (по черзі) |
|---|--------|
| ✅ | ~~**Фаза А.1** — видалити тип 🎉 Вітання (`greeting`) і категорію ❤️ Подяка ПОВНІСТЮ~~ — DONE 13.06. Код (modal/blocks), JSON (17→12 постів), Supabase (2 greeting → status='rejected'). `admin.html` лейбл + type CHECK-constraint лишені свідомо |
| ✅ | ~~**Фаза А.2** — згрупувати чіпи у 6~~ — DONE 13.06. `BOARD_CATEGORIES` з полем `match`, окрема `CATEGORY_EMOJI`, фільтр через `cat.match.includes()` |
| ✅ | ~~**Фаза А.3** — кнопки контакту (іконки)~~ — DONE 13.06. `renderContact()`: номер → 📞+🟣 Viber; @username → ✈️ Telegram. `toE164()`+`parseTelegram()`, нові SVG, CSS |
| ✅ | ~~**Фаза А.4** — bump `CACHE_NAME` + `node build.js`~~ — DONE 13.06. `cstl-20260613-1123`, build exit 0. Перевірка наживо — після `/finish` (чекає Вову) |
| ☐ | **Фаза А2** — Обговорення → повноекранна модалка-чат (публічний груповий чат, замість inline-коментарів). Спершу ASCII-макет |
| ☐ | **Фаза Б** — Google-логін = «Кабінет жителя» (Імʼя+Пошта+Вік), гейтинг дій, приватний чат покупець↔продавець + push (див. нижче «Google-авторизація») |
