...
| ☐ | **Фаза А.2** — згрупувати чіпи у 6: Всі · Куплю/Продам · Шукаю · Послуги · Знайдено/Загубилось · Оголошення (пост тримає конкретну, чіп групує) |
| ☐ | **Фаза А.3** — кнопки контакту (іконки): номер → 📞 Дзвінок + 🟣 Viber; @username → ✈️ Telegram (Telegram за номером НЕ робимо) |
| ☐ | **Фаза А.4** — bump `CACHE_NAME` + `node build.js` + перевірка |
