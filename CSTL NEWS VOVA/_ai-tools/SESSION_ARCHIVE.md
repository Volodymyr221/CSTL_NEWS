...
## Сесія 2026-04-17/18 — Модалка новин, автобуси, RSS, UX

### Комміти сесії
| Коміт | Що зроблено |
|-------|-------------|
| `ac3883a` | fix(modal): свайп закриття тільки від ручки + збільшена зона дотику |
| `1d228b6` | fix(buses): прибрати підписи «Від»/«До» над полями пошуку |
| `664daf9` | fix(ci): окрема concurrency group для rss-parser, не конфліктує з deploy |
| `e3023ca` | feat(news): футер статті — автор зліва, «Читати оригінал» справа |
| `b632e55` | fix(news): назва видання теж жирним сірим |

### Зміни з попередньої сесії (були в summary — тут фіксуємо)
| Коміт | Що зроблено |
|-------|-------------|
| *(серія)* | **Погода** — назва міста по GPS через Nominatim reverse geocoding |
| *(серія)* | **Автобуси** — повний Kyiv-timezone фікс: `kyivNowMins()`, `kyivDayOfWeek()`, `kyivToLocal()`, «За Києвом» для закордонних |
| *(серія)* | **Модалка новини** — scroll lock, клікабельне джерело, HTML entities decoding, повна стаття через `content:encoded` |
| *(серія)* | **Модалка** — `top: var(--header-h)`, затемнення шапки, iOS анімація, свайп з RAF |
| *(серія)* | **RSS cron** — змінено з `0 * * * *` на `*/30 * * * *` |

### Що зроблено (детально)

#### 1. Свайп для закриття модалки — тільки від ручки
- Додано прапор `startedOnHandle` — свайп активується тільки якщо `touchstart` на `.modal-handle`
- Скрол всередині статті більше не конфліктує з жестом закриття
- Зона дотику ручки збільшена: `width: 80px; height: 24px` + псевдоелемент для візуальної рисочки
- `cursor: grab` на ручці

#### 2. Модалка — повна архітектура (підсумок)
- Відкривається знизу під шапкою (`top: var(--header-h)`)
- `@keyframes modalSlideUp` з iOS `cubic-bezier(0.32, 0.72, 0, 1)` еасингом
- Шапка затемнюється: `body.modal-open .app-header { filter: brightness(0.55) }`
- Свайп вниз за ручку → RAF 60fps → закриття при `dy > 80px`
- `touchstart` на ручці скасовує поточну анімацію (`animation: none`) щоб палець одразу «підхопив»

#### 3. RSS-парсер — архітектура (підсумок)
- **Повний текст:** `content:encoded` → `get_full_content()` → `strip_html()` → до 8000 символів
- **`strip_html()`:** блокові теги → `\n\n`, BR → `\n`, решта → пробіл, `html.unescape()`
- **`decodeEntities()`** у JS через `textarea.innerHTML` — безпечне декодування без XSS
- **Cron:** `*/30 * * * *` — кожні 30 хвилин
- **Concurrency:** `group: "rss-parser"` (було `"pages"` — конфліктувало з deploy.yml)

#### 4. Футер статті
- `<div class="article-source-row">` — flex рядок з `justify-content: space-between`
- Зліва: «Автор публікації:» (жирний темний) + назва видання (жирний сірий)
- Справа: «Читати оригінал →» (жирний червоний)
- Розділювальна лінія зверху `border-top: 1px solid var(--border)`

#### 5. Автобуси — Kyiv timezone (підсумок)
- `kyivNowMins()` — поточний час Києва через `Intl.DateTimeFormat`
- `kyivDayOfWeek()` — поточний день тижня Києва
- `kyivToLocal(hhmm)` — конвертує Київський час у локальний для відображення
- `localTzLabel()` — `UTC+2`, `UTC+1` тощо
- `isForeign` прапор → показує «За Києвом HH:MM → HH:MM» якщо різні пояси
- `isDayActive(days)` — фільтрує рейси за `щодня / пн-сб / пн-пт` (закриває B-05)

### Стан на кінець сесії
- `sw.js` CACHE_NAME: `cstl-20260418-0302`
- Останній коміт: `b632e55`
