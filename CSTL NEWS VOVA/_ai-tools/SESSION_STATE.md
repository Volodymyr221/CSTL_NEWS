...
| **Робоча гілка (поточна сесія)** | `claude/startup-uem-V5K7g` |
...
### ✅ Зроблено 08.06.2026 (поточна сесія)

**UI-покращення списку зупинок (buses.js / buses.css):**
- `.bs-stops-body` — `border-radius: 8px 8px 12px 12px` (були гострі верхні кути сірого фону за смужкою пункту Б)
- Смужки пунктів А і Б (`hl:first-child` / `hl:last-child`) — плавний `linear-gradient` на внутрішньому краї (перехід `#E1E4EC` → `var(--gray-light)` до середини)

**Парсер VOPAS — 21-денне вікно з розумним кешуванням:**
- `get_21_days()` замість `get_14_days()` — 21 день (3 тижні від Пн поточного)
- Буферний тиждень (дні 14–20): завантажується ОДИН РАЗ, далі кешується поки не ввійде у видиме вікно
- Видиме вікно (дні 0–13, Пн поточного → Нд наступного тижня): оновлюється кожні 30 хв
- Поріг: `monday + 13 днів` (кінець наступного тижня) — точна синхронізація з UI-календарем
- Мета: у неділю парсер вже зберігає наступний тиждень → коли настає понеділок і UI зсовується вперед — дані вже є

### 🔜 Заплановано (наступна сесія)

- **Зворотні маршрути** — Вова надасть список рейсів з боку Олики
- **Закріпити маршрут** — кнопка «Відстежити» на картці → hero (головний блок) показує цей маршрут до прибуття (localStorage — локальне сховище браузера)
