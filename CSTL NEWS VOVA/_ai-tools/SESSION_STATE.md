...
**Оновлено:** 2026-06-05 (Вкладка АВТОБУСИ — фінальні доробки, фікси overscroll та tab bar)
...
## 🚌 Модуль «Автобуси» — фінальний стан (05.06.2026)
...
**Розклад:**
- Заголовок «РОЗКЛАД АВТОБУСНИХ МАРШРУТІВ» — чорний, 15px, по центру, тінь, фading-underline
- Фон вкладки `#F0F2F5` (сірий) — тільки `#page-buses`, не глобально
- Картки рейсів — фон `#F0F2F5` (в колір сторінки)
- Hero-блок — `box-shadow` (тінь)
- Нижній блок «VOPAS — Оновлено:» — `padding-bottom: 72px` (щоб таб-бар не перекривав)
...
- **Закріпити маршрут** — кнопка «Відстежити» на картці → hero показує цей маршрут до прибуття (localStorage)
