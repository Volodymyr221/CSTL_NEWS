...
**Оновлено:** 2026-06-06 (Автобуси: рядок "Оновлено" тепер завжди видно — знизу під VOPAS коли рейсів 0, зверху під заголовком коли рейси є)
...
**UI-покращення вкладки Автобуси:**
- Дублювання порожнього стану виправлено — список показує `''` коли немає фільтру (hero вже показує «СЬОГОДНІ РЕЙСІВ БІЛЬШЕ НЕ ЗАПЛАНОВАНО»)
- Кнопка «Показати всі N рейсів ↓» переміщена ПІД список маршрутів
- Час відправлення і прибуття — однаковий розмір шрифту (20px / 20px)
- Назви маршрутів у списку: ВЕЛИКІ ЛІТЕРИ + стрілка (ЛУЦЬК → ЛИЧАНИ)
- Назва маршруту у hero-картці: теж ЛУЦЬК → ЛИЧАНИ (однаковий формат)

**Парсер VOPAS — захист від обнулення:**
- `parse_vopas.py`: якщо `build_schedule()` повертає `routes=[]` (всі рейси відсіяні фільтром `route_is_local()`) — `schedule.json` НЕ перезаписується
- Попередній захист (при `unique=[]` — сайт ліг) вже був. Тепер обидва рівні захищені.
- Причина проблеми: VOPAS іноді повертає лише транзитні рейси (Луцьк-Львів і т.п.) → `route_is_local()` відсікає всі → без захисту файл обнулявся

### ✅ Зроблено 06.06.2026 (сесія 3 — продовження)

**Парсер VOPAS — фінальне виправлення:**
- Причина порожнього розкладу: VOPAS прибирає рейси що вже відправились → після обіду 0 результатів
- Тепер parser завжди оновлює `schedule.json` якщо VOPAS відповів (навіть при routes=[])
- UI показує "Сьогодні рейсів більше не заплановано" з актуальним verifiedTime
- Захист залишається тільки для повного відказу VOPAS (unique=0)

**UI автобусів — дрібні покращення:**
- Плавне затухання (fade) знизу панелі пошуку — 18px, 70% прозорість
- Крапка статусу hero (🟢🔵🔴) — зменшено 10px→8px
- Крапки шкали прогресу — пройдені стали зеленими (були білі)
- Трек шкали (сіра лінія) — статичний при свайпі, не мигає
- Час прибуття у списку — насиченіший колір (ink-mute→ink-soft)
