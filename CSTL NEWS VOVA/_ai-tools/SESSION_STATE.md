...
| **CACHE_NAME у `sw.js`** | `cstl-20260608-1922` |

## ✅ Зроблено у поточній сесії (08.06.2026, продовження 2)

**fix(buses)** — відстежений рейс у hero для будь-якого дня:
- `findActiveRoutes()` — додано перевірку `trackedRouteId && _trackDate === busDay` перед `selectedRouteId`
- Тепер натиснути «Відстежити» на картці завтра (або будь-якого майбутнього дня) → цей рейс одразу стає першим у hero-віджеті
- Раніше логіка для не-сьогоднішніх днів ігнорувала `trackedRouteId` — перевіряла тільки `selectedRouteId` (тап на картку)
...
- Level A ✅ ЗАВЕРШЕНО: кнопка на картці, банер над таб-баром, localStorage, снуз ✕, `_notifiedWarning`, `_bannerSnoozedUntil`
- Level B: справжні push-сповіщення через Web Push API (VAPID) + Supabase Edge Function
  - Потрібно: VAPID ключі (генерація через `web-push generate-vapid-keys`), Edge Function `send-bus-push`, таблиця `push_subscriptions` у Supabase
