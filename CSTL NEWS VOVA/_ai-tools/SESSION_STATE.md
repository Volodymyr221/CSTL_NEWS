...
| **CACHE_NAME у `sw.js`** | `cstl-20260608-1711` |
...
- Іконка закладки перенесена всередину `.bus-card-main` як третій flex-елемент — прибирає порожнє місце під основним блоком картки.
...
**🟢 Пізніше**
- «Відстежити маршрут» ✅ ЗРОБЛЕНО — кнопка на картці, банер над таб-баром, localStorage, снуз ✕, _notifiedWarning
