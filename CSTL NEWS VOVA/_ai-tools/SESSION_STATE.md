...
| **CACHE_NAME у `sw.js`** | `cstl-20260608-0327` |
...
### ✅ Зроблено 07.06.2026 (поточна сесія)

**14-денний календар зі свайпом (головна задача сесії):**
- `weekPage` (0 = поточний тиждень, 1 = наступний тиждень) — новий стейт (стан) у `buses.js`
- `getWeekDays(page)` — повертає 7 об'єктів `Date` для заданої сторінки тижня
- `.bus-week-track` — контейнер (outer — зовнішній) шириною 200%, містить обидва тижні поруч
- `translateX(-${weekPage * 50}%)` — позиціонує трек: 0% = тиждень 0, -50% = тиждень 1
- Свайп пальцем: плавна анімація `cubic-bezier 0.28s`, трек слідує за пальцем в реальному часі
- Дві точки-навігатори (dots — крапки) під тижнями; активна — бордова, неактивна — прозора
- `passive: false` на `touchmove` — дозволяє `preventDefault()` для блокування вертикального скролу сторінки
- `isHorizSwipe` флаг — визначається одноразово при русі > 4px; горизонтальний рух блокує скрол

**Парсер VOPAS — 14 днів:**
- `get_14_days()` — Пн поточного тижня + 13 днів = рівно 2 тижні (синхронізовано з UI)
- `schedule.json` тепер зберігає `days: { "2026-06-07": { routes: [...] }, ... }` для кожного з 14 днів

**Пошук (from/to — звідки/куди) — date-scoped (обмежений датою):**
- `getAllStops()` — тепер збирає зупинки тільки з `getDayData().routes` (поточна обрана дата)
- Раніше збирав з усіх днів → показував зупинки з усіх 14 днів у dropdown (випадаючому списку)

**Фільтр маршрутів — виправлення неправильних результатів:**
- `matchesSearch()` — km-based (за кілометражем) перевірка напряму без `getOrderedStops()`
- `filterActive` флаг — коли обидві зупинки знайдені → картка показує сегментну мітку (напр. «ЛУЦЬК → ЛИЧАНИ») замість повної назви маршруту
- `bs-route-full` — малий підзаголовок з повною назвою (напр. "Ківерці → Олика") якщо відрізняється від сегменту
- Раніше пошук "Луцьк → Личани" показував "КІВЕРЦІ → ОЛИКА" маршрути — тепер тільки точні результати

**Повідомлення "нема рейсів" — виправлення позиції:**
- `renderSmartRow()` — завжди порожній (`innerHTML = ''`) коли `routes.length === 0`; повідомлення НЕ рендериться вгорі у hero-зоні
- `renderRouteList()` — повідомлення «СЬОГОДНІ РЕЙСІВ БІЛЬШЕ НЕ ЗАПЛАНОВАНО» або «НА ЦЕЙ ДЕНЬ РЕЙСІВ НЕ ЗНАЙДЕНО» рендериться нижче заголовка «РОЗКЛАД АВТОБУСНИХ МАРШРУТІВ»
- Виправлено баг: при `all.length === 0` (без фільтру) повідомлення не з'являлось взагалі

**VOPAS-посилання:**
- `buildSourceHtml()` — обертає `busData.source` у `<a href="https://vopas.com.ua">` → клікабельне
- CSS клас `.buses-updated-link` — `color: inherit`, `text-decoration: none`, `:active` opacity 0.6

**Стиль активного дня в календарі (фінальний стан):**
- `.bus-week-day--active`: `background: #BEC4D2`, `border-radius: 8px`, `border-color: #9AA3B5`, `box-shadow: 0 0 10px 6px rgba(190,196,210,0.9)` — синьо-сіра заливка з розмитими краями
- `.bus-week-day--today.bus-week-day--active`: `border-color: var(--red)` — коли сьогодні = обраний день, ободок залишається бордовим (тільки override — перевизначення ободка)
- Раніше активний стиль перекривав бордовий ободок today через специфічність CSS (каскадність)

**Last commit (останній коміт):** `734b00f` — `Легка тінь під списком зупинок маршруту`

### ✅ Зроблено 07.06.2026 (сесія 2 — поточна)

**UI-покращення вкладки Автобуси:**
- Крапки статусу (●) у hero-картці — CSS `<span>` замість emoji, три стани: `waiting` (сірий), `urgent` (бордовий пульсуючий), `enroute` (зелений)
- Шкала прогресу A/B — мітки оновлюються при свайпі між рейсами (`switchHeroCard()` оновлює `.bhv4-labels`)
- Поточна зупинка в розгорнутому списку — пульсуючий зелений кружечок (CSS-анімація `bs-live-pulse`)
- Наступна зупинка — велика жирна стрілка `▷`, чорна, bold
- Якщо наступна = кінцева — пін-іконка (SVG) замість стрілки
- Ціна за сегмент у кожній зупинці розгорнутого списку
- Іконки зупинок: ● (Звідки) та SVG-пін (Куди) — як у рядку пошуку
- Скасовані рейси залишаються у списку до моменту відправлення, потім переходять у "минулі"
- Майбутні рейси завжди вище минулих (`[...future, ...past]`)
- Бейдж «ЧЕРЕЗ X ХВ» — бордовий текст + пульсуючий бордовий кружечок, з'являється за ≤15 хв до відправлення
- Кружечок «відправляється» у hero — бордовий пульсуючий (такий самий як у бейджі)
- Відступ між зеленим кружечком і назвою міста у списку зупинок (`margin-right: 6px`)
- Кнопка «Сховати минулі ↑» переміщена НАД список минулих рейсів (між майбутніми і минулими)
- Кнопки показати/сховати — однаковий стиль, великі літери (`text-transform: uppercase`), розмір 11px
- Список зупинок — заокруглені нижні кути (`border-radius: 0 0 12px 12px`) + легка тінь

**Парсер VOPAS — sticky cancellation (закріплене скасування):**
- Якщо рейс був скасований до відправлення і VOPAS прибрав плашку після — статус `cancelled` зберігається
- Логіка: перевіряє `prev_routes` (попередні дані) + час відправлення → якщо вже відправився скасованим = залишається скасованим

**Фікс community-blocks.js:**
- `renderBusBlock()` читає нову структуру `data.days[todayISO].routes` (була стара `data.routes`)

### ✅ Зроблено 08.06.2026 (сесія — фінальний стан)

**Фікс назви зупинки «Гараджа» → «Гаразджа»** (правильна назва):
- `data/schedule.json` — всі входження замінено на «Гаразджа»
- `data/route-stops.json` — всі входження замінено на «Гаразджа»
- `src/tabs/buses.js` — runtime-нормалізація (виправлення під час роботи) після завантаження JSON: `STOP_ALIASES = { 'Гараджа': 'Гаразджа' }` — захист від кешованих старих даних у браузері. Застосовується до всіх зупинок у всіх маршрутах усіх днів одразу після парсингу JSON.

**UI-покращення списку зупинок (`style/buses.css`):**
- `.bs-stops-body` — `border-radius: 8px 8px 12px 12px`: верхні кути контейнера заокруглені, щоб збігались зі смужками А/Б (були гострі — сірий фон виглядав позаду)
- Смужки пунктів А і Б (`hl:first-child` / `hl:last-child`) — плавний `linear-gradient` на внутрішньому краї: `#E1E4EC 45% → #f5f5f5 100%` — перехід у фон контейнера

**Парсер VOPAS (`scripts/parse_vopas.py`) — 21-денне вікно:**
- `get_21_days()` замість `get_14_days()` — 21 день (3 тижні від Пн поточного)
- **Видиме вікно** (дні 0–13, = 2 тижні у календарі): оновлюється кожні 30 хв
- **Буфер** (дні 14–20, 3-й тиждень, невидимий в UI): завантажується один раз, потім кеш
- **Поріг**: `monday + 13 днів` — точна межа між видимим і буфером
- **Навіщо**: у неділю парсер вже має дані наступного тижня → коли настає понеділок і UI зсовується вперед — порожніх дат немає

**Поточний стан UI-календаря:**
- Показує 2 тижні (page 0 = поточний, page 1 = наступний), свайп між ними
- 3-й тиждень — тільки в `schedule.json` як буфер, у календарі НЕ відображається

### 🔜 Наступна сесія — ПЕРШЕ ЗАВДАННЯ

**Зворотні рейси «з боку Олики»** — парсинг маршрутів Олика→Луцьк, Олика→Ківерці тощо.
- VOPAS вже частково має ці маршрути (Олика є як «from» у деяких запитах)
- Вова надасть конкретний список рейсів які потрібно додати/перевірити
- До отримання списку — нічого не змінювати

### 🔜 Пізніше

- **Уточнення відстаней між зупинками** — вручну через Google Maps виміряти км між пунктами А, Б і кожною проміжною зупинкою для кожного маршруту. Зараз значення `km` у `data/route-stops.json` приблизні → розрахунковий час прибуття на проміжні зупинки не точний. Після вимірювання — оновити JSON вручну.
- **Закріпити маршрут** — кнопка «Відстежити» → hero показує обраний маршрут до прибуття (localStorage)
