...
**Оновлено:** 2026-06-05 (Вкладка АВТОБУСИ — редизайн hero-картки, поля пошуку, шкала прогресу)
...
| **CACHE_NAME у `sw.js`** | `cstl-20260605-0007` |
...
## 🚌 Модуль «Автобуси» — стан (05.06.2026)

### ✅ Зроблено у сесії 05.06.2026
- **Hero-картка v4** (`renderSmartRow`) — повний редизайн:
  - Фонове фото `images/bus-hero2.png` через `<img class="bhv4-bg-img">` (не CSS background)
  - Темний оверлей (градієнт) поверх фото для читабельності тексту
  - Статус-рядок зверху: іконка автобуса + стан (очікується/в дорозі/прибув) + синя крапка
  - Назва маршруту великим шрифтом
  - Капсула з часом відправлення→прибуття + тривалість
  - «ЧЕРЕЗ X ГОД X ХВ» — countdown до наступного рейсу
  - Шкала прогресу внизу картки (`renderRouteMapV4`) — нормальний потік документу (не `position: absolute`)
  - Пульсуюча зелена крапка на поточній зупинці — **тільки** коли автобус `enroute` (клас `.bhv4--enroute`)
  - Розмір фото для редизайну: **720×320px**
- **Поля пошуку** — редизайн у стилі reference screenshot:
  - «Від» → «**Звідки**» (червона крапка ●)
  - «До» → «**Куди**» (SVG pin-іконка)
  - Кнопка swap (поміняти місцями) — кругла біла з тінню
  - Стрілочки swap: **горизонтальні ← →** (були вертикальні ↑↓)
- Відступ між hero-карткою і списком рейсів
- `CACHE_NAME` бамп при кожній значній зміні

### 🔜 Заплановано (наступна сесія)
- **Закріпити маршрут** — кнопка на картці рейсу «Відстежити» → hero показує цей маршрут до прибуття (localStorage)
- **Стрілочка →** праворуч у hero-картці — свайп/перехід між найближчими рейсами
- Інші покращення вкладки за потреби

### Поточний стан коду
- Пошук Звідки→Куди, акордеон зупинок, smart-рядок (hero v4), дні тижня (`isDayActive`)
- `filter-bar` `position: fixed`
- Фото: `images/bus-hero2.png` (720×320px, підготовлено ChatGPT — замок + автобус на бордовому фоні)
...
- **Email+password login** через `supa.auth.signInWithPassword({email, password})` (раніше був magic-link, але впирався у rate limit Supabase Free ~4 листи/год).
- **Forgot password** — magic-link на пошту → нова сторінка → таб Налаштування для зміни пароля.
- **Whitelist** — після auth перевіряємо чи email є у таблиці `admins`. Інакше — екран «Немає доступу». Захист від catch-22 у RLS (рекурсивний `auth.email() IN admins`) через SECURITY DEFINER функцію `is_admin()` (див. `scripts/supabase_fix_rls.sql`).
- **6 табів:**
  - 📋 **Модерація** — `pending` posts з кнопками Опублікувати / Відхилити / Видалити. Бейджі: тип/категорія/локація.
  - 🏛️ **Оголошення** — CRUD для `announcements` (форма додати з title/body/author/pinned-чекбокс + видалити).
  - 📰 **Опубліковані** — фільтр `status='published'` → можна видалити неприйнятне якщо щось пропустив через approve.
  - 💬 **Коментарі** — список з таблиці `comments` з delete-кнопками (раніше можна було тільки через SQL).
  ...
  - ⚙️ **Налаштування** — зміна пароля адміна.
- **Шапка:** кнопка «← На сайт» (без logout, сесія тримається ~30 днів).
- **Прихований вхід:** 5 тапів на лого `CSTL LIFE` у шапці головного сайту → редирект на `/admin.html`.
- **Конфіг:** `SUPABASE_URL` і `SUPABASE_ANON_KEY` зашиті у `<script>` (production). Service_role key один раз leak → ROTATE через Supabase Dashboard. В код тільки publishable anon — RLS policies захищають усе.
