...
import { FILTER_HOMOGLYPHS } from './homoglyphs.js';   // латинські двійники кирилиці — спільні з пошуком
...
// ── ПОВНЕ ІМʼЯ: ОДНА СКЛЕЙКА НА ВЕСЬ ЗАСТОСУНОК (04.09) ──────────────────────
//
// 🗣️ Замовлення Вови: «при коментуванні постів автор пише просто імʼя, а треба,
// щоб писав імʼя та прізвище… Де це доцільно. А в більшості випадків це доцільно».
//
// 🔴 Вада була НЕ в оформленні. `profiles.surname` існує з липня, але публічні
// RPC (`get_avatars`, `get_public_profile`) віддавали лише `name` — прізвище
// фізично не доїжджало до екрана. Полагоджено міграцією
// `surname_split_and_public_rpc` (дзеркало — `scripts/supabase_full_name.sql`).
//
// 🔑 ЧОМУ СКЛЕЙКА ТУТ, А НЕ В КОЖНОГО СПОЖИВАЧА. Її потребують три незалежні
// шляхи: кеш чужих імен (`supabase.js` → `hydrateNames`), моє власне імʼя
// (`auth.js` → `currentUserName`) і картка жителя (`profile-card.js`). Три копії
// одного правила в цьому проєкті вже розходились чотири рази (два списки
// антиспаму, дві каруселі, дві розмітки пункту FAB, три переглядачі фото), і
// симптом щоразу виглядав як вада продукту, а не як розсинхрон.
//
// ⚠️ Порожнє прізвище — звичайний випадок, а не помилка: 2 профілі з 14 його не
// мають, і в них рядок лишається самим імʼям, без хвостового пробілу.
export function fullName(name, surname) {
  return [String(name || '').trim(), String(surname || '').trim()]
    .filter(Boolean).join(' ');
}

// Перше слово повного імені — для ТІСНИХ рядків, де прізвище однаково обрізалось
// би трьома крапками. Рішення Вови 04.09: таких місць рівно два — привітання на
// Громаді («Доброго дня, Вово» з прізвищем звучить казенно) і цитата відповіді в
// картці Питання. Решта поверхонь показує повне імʼя.
export function firstNameOf(full, fallback = '') {
  const s = String(full || '').trim();
  return s ? (s.split(/\s+/)[0] || s) : fallback;
}
...
// 🔴 ЕКСПОРТОВАНО 05.08: селектор населеного пункту (`core/settlements-geo.js`)
// потребує тих самих координат. Заводити там власну копію означало б ту саму
// хворобу «двох копій», що вже коштувала проєкту розсинхрону списків антиспаму.
// ⚠️ Відома розбіжність, НЕ виправлена навмисно: дефолт `sunTimes()` нижче
// тримає інші координати Олики (50.717 / 25.810, різниця ~1.8 км). Для сходу
// сонця це секунди, а зміна дефолту чіпає функцію поза цим замовленням. Записано
// в BYYOU_PLAN як відкритий борг.
export const OLYKA_COORDS = { lat: 50.7333, lon: 25.8167 };
...
// ── КІНЕЦЬ БЛОКА TOAST ──────────────────────────────────────────────────────
// 🛡 ЦЕЙ РЯДОК — МЕЖА ДЛЯ СТЕНДА `tests/toast-queue.mjs`, не окраса. Він вирізає
// звідси код і виконує його, тож потребує кінця блока. Раніше межею був
// коментар СУСІДНЬОЇ функції («Перегляд фото на весь екран») — і коли 04.09
// `openPhotoLightbox` звідси прибрали, стенд упав із «не знайшов блок toast»,
// хоча сам toast не змінився ні на символ.
// ➡️ Прив'язка до ВИПАДКОВОГО СУСІДА ламається від чужої правки; прив'язка до
// названої межі — ні. Пересуваєш цей рядок — знай, що переносиш межу.
...
// 🔑 30.08 переїхала у `core/homoglyphs.js` — її споживає ще й пошук, а другий
// список розійшовся б (як уже було з двома списками антиспаму). Причина саме
// ОКРЕМОГО файлу технічна: `search.js` мусить запускатись у node без `window`.
...
const LS_MSG_RATE = 'cstl-msg-rate-v1';   // { last: { scope: {t,at} }, times: [ts, …] }
...
// 🔴 25.08 — ДУБЛЬ ТЕПЕР МАЄ ВІКНО. Скарга Вови: «я вчора написав повідомлення,
// сьогодні зранку не можу те саме слово написати».
//
// Вада була структурна: рейт-ліміт мав вікно (15с), а перевірка на дубль —
// НЕ МАЛА ЖОДНОГО. Порівнювався останній текст у цьому місці, і «Окей»,
// написане вчора, блокувало «Окей» назавжди — поки людина не напише там
// щось інше. Тобто найкоротші й найприродніші відповіді («Окей», «Дякую»,
// «Так») блокувались НАЙНАДІЙНІШЕ, бо саме їх пишуть повторно.
//
// 🔑 Вікно ТЕ САМЕ, що в рейт-ліміті, і це навмисно: два різні числа довелось
// би пояснювати і людині, і наступній сесії. Одне правило одним числом —
// «за 15 секунд не більше 8 повідомлень і не двічі те саме».
// 🛑 Дзеркалиться БАЗОЮ (`scripts/supabase_antispam_shared.sql`). Міняєш тут —
// міняй і там, інакше людина побачить «надіслано», а база мовчки відхилить.
const DUP_WINDOW = 15000;
...
// Чи це дубль щойно надісланого в цьому ж місці (у межах DUP_WINDOW)
export function isDuplicateMsg(text, scope = 'default') {
  const rec = lastMap(lsGet(LS_MSG_RATE, {}))[scope];
  if (!rec) return false;
  // ⚠️ СТАРИЙ ФОРМАТ (рядок без часу) БІЛЬШЕ НЕ БЛОКУЄ — і це не недогляд.
  // На телефонах уже лежать записи без позначки часу, і вважати їх «щойно»
  // означало б лишити людину замкненою рівно там, де вона й скаржилась.
  // Перший же надісланий текст перезапише запис у новому форматі.
  if (typeof rec === 'string') return false;
  return rec.t === text && (Date.now() - rec.at) < DUP_WINDOW;
}
...
//
// 🔑 ПРИНЦИП: рядок не обрізається — він ВИБИРАЄ. Кожна роль оголошує ДРАБИНУ
// варіантів від найповнішого до найкоротшого, і береться перший, що вміщається.
// Обрізка по межі СЛОВА — останній рубіж, і лише для найкоротшого варіанта.
//
// 🔴 18.08 — ЦЕ ДРУГА РЕДАКЦІЯ, І ПЕРША БУЛА НЕПРАВИЛЬНА. Було: «частини зі
// спільним роздільником, зайва викидається ЦІЛКОМ». На проді це дало рівно те, на
// що поскаржився Вова: мітка `ПОВІДОМЛЕННЯ · ПРОДАМ БУДИНОК В ЖОРНИЩЕ` (323px при
// бюджеті 242) втратила НАЗВУ ОГОЛОШЕННЯ і лишилась голим словом «ПОВІДОМЛЕННЯ» —
// тобто зникла саме та частина, заради якої все робилось.
// 🛑 Два уроки, чому драбина краща за частини:
//   • викидання частини — рішення БЕЗ АЛЬТЕРНАТИВИ: або все, або нічого. Драбина
//     дозволяє РОЛІ сказати, чим саме жертвувати («МОЄ ОГОЛОШЕННЯ» → «МОЄ»);
//   • у частин зі спільним роздільником не буває чесного скорочення: викинеш
//     частину — лишиться висячий «·» або двокрапка, і це треба ловити окремо.
//     У драбині кожен щабель — готовий рядок, писаний людиною.
//
...
// Обрати рядок, який УМІСТИТЬСЯ.
//   variants — драбина готових рядків: [найповніший, …, найкоротший].
//   m        — знімок із `lineMetrics()`. Немає знімка → беремо найповніший.
// Повертає готовий рядок (порожній, якщо варіантів немає).
export function fitLine(variants, m) {
  const список = (variants || []).map(v => String(v == null ? '' : v).trim()).filter(Boolean);
  if (!список.length) return '';
  if (!m) return список[0];

  // 1. Перший варіант, який ВМІЩАЄТЬСЯ цілком.
  for (const в of список) if (textWidth(в, m) <= m.width) return в;

  // 2. Не вмістився жоден — ріжемо НАЙКОРОТШИЙ (останній) по межі СЛОВА.
  //    🔑 Саме останній, а не перший: він уже зведений до найпотрібнішого, тож
  //    обрізати в ньому найменш боляче.
  const слова = список[список.length - 1].split(/\s+/);
  ...
  let одне = слова[0];
  while (одне.length > 1 && textWidth(одне + '…', m) > m.width) одне = одне.slice(0, -1);
  return одне + '…';
}
