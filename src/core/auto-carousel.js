 ...
 * @param opts     { slideSel, cycleMs, onSlide, auto }
 *                 onSlide(i) кличеться щоразу, коли у вікні опиняється інший слайд.
 *                 auto: false — САМ не гортає, лише каже, який слайд у вікні.
...
export function startAutoCarousel(track, opts = {}) {
  ...
  // 🔑 20.09 — `auto: false`: доріжка, яку гортає ЛИШЕ людина.
  // Заведено для капсули автобуса, де два боки маршруту стоять слайдами. Сам
  // рух там не потрібен: капсула це відповідь, а не стрічка, і самочинна зміна
  // означала б, що потрібний бік може поїхати з екрана, поки людина дивиться в
  // інший бік — рівно та вада, через яку 17.08 прибрали циклічні капсули.
  // 🛑 Але лічильник слайда й тут мусить бути СПІЛЬНИЙ: крапки під капсулою
  // рахуються тим самим `visibleIndex()`, що й у каруселях. Власний лічильник у
  // споживача — це клас B-27, і в цьому файлі про нього написано двома абзацами
  // вище.
  if (opts.auto === false) return () => { track.removeEventListener('scroll', onScroll); };
