...
function applyOpen(open) {
  ...
  // вкладка — з минулого разу. Дешевше за підписки на кожне з цих джерел.
  if (open) { renderNav(); refreshCabinet(); }
}

// 🔴 10.08 — ЄДИНИЙ ПИСАР РОЗМІТКИ, І ВІН НЕ МІНЯЄ СТАН.
//
// Скарга Вови (повернення того самого бага): «відкрив бургер, натиснув Instagram,
// повернувся назад, відкриваю бургер — фон блюриться, меню зʼявляється і відразу
// зникає, а фон залишається».
//
// 🔑 СИМПТОМ ЗАВЖДИ ОДИН І ТОЙ САМИЙ: панель і затемнення РОЗІЙШЛИСЬ. Це вже
// третій захід на цей клас (зависле затемнення 09.08 · меню, що закривалось саме
// 09.08 · тепер це). Кожного разу лікували ПРИЧИНУ конкретного випадку — а клас
// лишався можливим, бо станів у розмітці ДВА і їх можна розвести.
//
// ➡️ Тому тепер стан пишеться рівно в одному місці й ОДНИМ заходом, а затемнення
// в CSS виводить свою видимість із класу ПАНЕЛІ (`:has(.sidebar--open)`): без
// відкритої панелі воно не показується взагалі, скільки б класів на ньому не
// залипло. Розійтись немає чому — і писар один, і правда одна.
//
// ⚠️ `paintState()` НЕ вирішує, відкрито меню чи ні, — він лише приводить розмітку
// у відповідність до `_open`. Саме тому його безпечно кликати на подіях фону: у
// цьому й був баг 09.08, коли подія фону САМА закривала щойно відкрите меню.
function paintState() {
  const { sidebar, overlay, toggle } = els();
  if (!sidebar || !overlay) return;
  if (_open) overlay.hidden = false;      // показати можна лише знявши атрибут
  // ⚠️ Третього стану (мітки на `body`) тут свідомо НЕМАЄ: що менше місць, де
  // живе «відкрито/закрито», то менше чому розходитись. Затемнення виводить свою
  // видимість із класу ПАНЕЛІ через `:has()` — див. `style/sidebar.css`.
  ...
  if (!_open) syncOverlay();              // закриття могло вже завершитись — звіримо одразу
}
...
export function initSidebar() {
  ...
  // 🔴 Повернення в застосунок ПЕРЕЗАСТОСОВУЄ розмітку (`paintState`), а не лише
  // ховає затемнення. Саме тут ламалось: після Instagram сторінка поверталась із
  // застиглим напівстаном, і наступне відкриття успадковувало його. Стан при
  // цьому НЕ змінюється — подія фону не має права закривати меню (урок 09.08).
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') paintState();
  });
