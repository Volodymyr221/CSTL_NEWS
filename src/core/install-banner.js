...
// 🔑 Пауза 400 мс після «Погоджуюсь» не косметична: банер згоди їде вниз 240 мс
// (`.consent-bar` transition), і поява другого блоку в ту саму мить читалась би як
// «одне перетворилось на інше». Даємо низу екрана спорожніти.
function уЧергу(показати, затримка) {
  if (!consentPending()) { setTimeout(показати, затримка); return; }
  document.addEventListener('cstl-consent-accepted',
    () => setTimeout(показати, 400), { once: true });
}
