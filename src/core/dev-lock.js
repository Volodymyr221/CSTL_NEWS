...
import { normalizeDevCode, devCodeHash, devCryptoReady } from './dev-code.js';
...
const ALLOWED_CODE_PBKDF2 = [
  ...
  // Другий код, доданий на прохання Вови 18.08 — обидва дійсні одночасно, порядок
  // у списку значення не має (перевірка йде через `includes`).
  // 🔴 ЦЕЙ ХЕШ СЛАБШИЙ ЗА ПЕРШИЙ: після нормалізації код 7 символів проти 9, і сам
  // він словниковий та описує механізм, до якого відкриває доступ. Вові сказано
  // прямо; рішення його. Заміна — один рядок:
  //   node tests/tools/dev-code-hash.mjs 'новий-довгий-код'
  '204928637b5a774d93856d4a21087c42ae77110c1df5604babd963c0733ec29c',
];
...
async function onSubmitCode(e) {
  ...
  // 🔴 25.08 — «НЕ МОЖУ ПЕРЕВІРИТИ» ≠ «КОД НЕ ТОЙ», І КАРАТИ ЗА ЦЕ НЕ МОЖНА.
  //
  // Скарга Вови зі знімком: «як так що з тел версії підходить код, а з компʼютера
  // ні». Код був ПРАВИЛЬНИЙ. Сайт відкрито по `http://` (у вкладці Safari видно
  // «Не захищений»), а там немає `crypto.subtle` — отже хеш не рахується взагалі.
  //
  // 🛑 Що робив замок до цього: писав «Код не той» І нараховував хибну спробу. Тобто
  // 1) брехав про причину, 2) через пʼять таких «спроб» видавав паузу 60 секунд
  // людині, яка **жодного разу не помилилась**. Саме це й було на знімку.
  // 🔑 Тому вихід стоїть ТУТ — до `codeAllowed()` і, головне, до `noteWrongTry()`.
  // ⚠️ Замок НЕ послаблюється: доступу так само немає. Змінюється лише те, що
  // людина дізнається СПРАВЖНЮ причину і не платить за неї паузою.
  if (!devCryptoReady()) {
    note('Сайт відкрито без захищеного зʼєднання — браузер не може перевірити код. Відкрий адресу з https://', 'bad');
    return;
  }
...
function showGate() {
  ...
  // 🔴 25.08 — КАЖЕМО ПРО НЕЗАХИЩЕНЕ ЗʼЄДНАННЯ ОДРАЗУ, А НЕ ПІСЛЯ НЕВДАЧІ.
  // 🔑 Підказка при невдалій спробі лікує наслідок; людина все одно спершу набирає
  // код, чекає «Перевіряю…» і аж тоді дізнається, що перевірити було неможливо.
  // Тут вона бачить причину ще до того, як торкнеться поля.
  if (!devCryptoReady()) {
    note('Сайт відкрито без захищеного зʼєднання — браузер не зможе перевірити код. Відкрий адресу з https://', 'bad');
  }
}
