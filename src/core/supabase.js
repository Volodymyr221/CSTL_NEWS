...
function logDbRefusal(err) {
  try {
    ...
    if (!code && !msg) return;
    // 🔑 Пишемо ВИПАДКОВИЙ НОМЕР ПРИСТРОЮ (`getAnonId`), а не акаунт — навіть коли
    // людина залогінена. Щоб побачити поломку, треба знати ЩО зламалось і чи це
    // трапляється в багатьох, а не В КОГО саме; прив'язка до акаунта нічого не
    // додала б до діагнозу і зробила б журнал збоїв журналом людей.
    ...
    logEvent(getAnonId(), 'db_refusal', { meta: { code, msg } });
