...
import { escapeHtml, fullName } from './utils.js';   // hydrateAvatars (безпечний <img src>) · склейка «імʼя + прізвище»
...
// 🔴 ЧИ Є В БАЗІ ПОДАННЯ-ЛІЧИЛЬНИКИ — ЗАПАМʼЯТОВУЄМО НА СЕАНС.
// `null` — ще не пробували, `true` — є, `false` — немає і питати не варто.
// Причина в коментарі всередині `fetchAllReactions`: промах по неіснуючому
// відношенню коштує дорого, і повторювати його на кожне відкриття не можна.
let поданняРеакцій = null;
let поданняЛайків  = null;
...
export async function fetchAllReactions(anonId) {
  ...
  // 🔴 24.09, ГАРЯЧЕ. ПОДАННЯ ПИТАЄМО НЕ БІЛЬШЕ ОДНОГО РАЗУ ЗА СЕАНС.
  // Міграція 0006 накочується руками, тож між деплоєм коду і накатом подання
  // `reaction_counts` у базі НЕМАЄ. Кожне звернення до неіснуючого відношення
  // PostgREST зустрічає промахом кеша схеми — а саме довге перечитування схеми
  // вже одного разу поклало застосунок (B-35, 22.09: `PGRST002`, вхід із 504).
  // Робити такий запит на КОЖНЕ відкриття Дошки означало б стукати в те саме
  // місце постійно.
  // ➡️ Тому перший промах запамʼятовується, і далі сеанс іде старою дорогою
  // без жодного зайвого запиту. Після накату 0006 перше ж відкриття знайде
  // подання і користуватиметься ним.
  const [лічильники, мої] = await Promise.all([
    поданняРеакцій === false
      ? Promise.resolve({ data: null, error: { message: 'подання немає (запамʼятовано)' } })
  ...
  if (лічильники.error && поданняРеакцій === null) поданняРеакцій = false;
  else if (!лічильники.error) поданняРеакцій = true;
  if (!лічильники.error) {
...
export function nameUid(uid, { short = false } = {}) {
  ...
  // 📐 Заміряно 04.09 на живій базі: 12 профілів із 14 мають прізвище. Тобто в
  // тісних місцях повне імʼя не вміщається БІЛЬШОСТІ людей, а не поодиноким.
  // ⚠️ Тут стояла мірка «7 із 12 записані як Імʼя Прізвище» — вона з 23.08,
  // тобто ДО того, як 29.08 анкета розділила поля (`account-ui.js`). Відтоді
  // «пробіл у `name`» більше не означає «є прізвище»: воно живе в `surname`,
  // а склеює їх `fullName()` з `core/utils.js`.
...
export function nameSlotStatic(text, official) {
  if (!official) {
    return '<span class="cstl-name"><span class="cstl-name-txt">' + text + '</span></span>';
  }
  // 🔴 05.09 — ЗНАК ПРИКЛЕЄНИЙ ДО ОСТАННЬОГО СЛОВА.
  //
  ...
  //
  // 🔑 Обидва місця, куди приходить цей варіант — ЗАГОЛОВКИ, що переносяться
  // (шапка спільноти, картка профілю). Там знак мусить іти в текстовому потоці,
  // як іще один символ, а не стояти збоку блока.
  // 🛑 ЧОМУ ЦЬОГО МАЛО БЕЗ СКЛЕЙКИ. Сам по собі потік ставить знак після
  // останнього символа — але якщо той рядок майже повний, знак переноситься
  // САМ і лишається на новому рядку самотньою синьою крапкою. Тому останнє
  // слово і знак живуть в одному вузлі `nowrap`: переносяться вони разом.
  // ⚠️ Трикрапки в цих двох місцях немає (заголовок переноситься, а не ріжеться),
  // тож знак усередині тексту нічим не ризикує. У рядкових місцях зі стрічки
  // діє інший шлях — `nameSlot`, і там структура лишилась незмінною.
  const i = text.lastIndexOf(' ');
  const голова = i > 0 ? text.slice(0, i + 1) : '';
  const хвіст  = i > 0 ? text.slice(i + 1) : text;
  return '<span class="cstl-name"><span class="cstl-name-txt">' + голова
    + '<span class="cstl-name-tail">' + хвіст + officialMarkHtml() + '</span>'
    + '</span></span>';
}
    ...
    const pr = (async () => {
      try {
        ...
        (data || []).forEach(r => {
          ...
          // 🆕 04.09 — у кеш лягає ПОВНЕ імʼя (`name` + `surname`). Саме звідси
          // його бере `hydrateNames()`, тобто ця склейка й доносить прізвище на
          // всі підписи одразу: коментарі, дописи, чати, списки розмов.
          // 🛑 Умова лишається на `r.name`, а не на склеєному рядку: профіль із
          // самим прізвищем і без імені — не привід стерти те, що вже знали.
          if (r.name) _nameCache.set(r.uid, fullName(r.name, r.surname));
...
export function netErrorText(err) {
  ...
  // ── ✉️ ВХІД ПОШТОЮ КОДОМ (29.08, переставлено 30.08) ────────────────────────
  // 🔴 ЦЕЙ БЛОК СТОЇТЬ ВИЩЕ ЗА `isTransientError`, І ЦЕ КУПЛЕНО ПОМИЛКОЮ.
  // 30.08 Вова тиснув «Надіслати код» — і бачив «Слабкий звʼязок». Логи сервера
  // казали інше: `/otp 200` (код пішов), далі `429 — можна повторити через 31 с`.
  // Тобто звʼязок був чудовий, сервер відповів ЗМІСТОВНО, а ми переклали це як
  // проблему мережі. 🛑 `isTransientError` вважає 429 тимчасовим — і правильно
  // робить ДЛЯ ПОВТОРІВ, але для ТЕКСТУ це брехня: людина йде перевіряти Wi-Fi
  ...
  // Скільки саме чекати — сервер каже точно, тож не округлюємо до «хвилини».
  const wait = (msg.match(/(\d+)\s*seconds/)   /* не `after (` — сторож імпортів читає це як виклик функції */ || [])[1];
  if (/rate_limit/.test(acode) || wait || /email rate limit/i.test(msg))
    return wait ? `Код уже надіслано — можна повторити через ${wait} с`
                : 'Код уже надіслано — зачекай хвилину';
...
export async function fetchPageReactions(userKey) {
  ...
  // Те саме запамʼятовування, що у `fetchAllReactions`. `page_reaction_counts`
  // на проді Є, тож тут це радше запобіжник на майбутнє, ніж лагодження.
  const [лічильники, мої] = await Promise.all([
    поданняЛайків === false
      ? Promise.resolve({ data: null, error: { message: 'подання немає (запамʼятовано)' } })
  ...
  if (лічильники.error && поданняЛайків === null) поданняЛайків = false;
  else if (!лічильники.error) поданняЛайків = true;
  if (!лічильники.error) {
