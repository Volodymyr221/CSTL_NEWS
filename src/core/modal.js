...
// 🔴 01.09 «c» — ШАПКА СТАЛА ОКРЕМИМ ЛИПКИМ ЯРУСОМ (`.app-modal-head`).
//
// 🗣️ Замовлення Вови: «стандартизувати всі модалки… шапку, щоб була ця рисочка і
// іконка хрестика… нам треба зробити базу… при скролі ця рисочка і іконка
// закриття не скролиться за рамки модалки».
//
// 🔬 ЗАМІРЯНО ДО ПРАВКИ (лист прокручено до кінця, 390×844): рисочка і ✕ їхали з
// `top 96.4` на **−838.6**, тобто **−935px — повністю за екран**.
// 🔑 Це була не вада однієї модалки, а БУДОВА примітиву: `.app-modal-sheet` сам є
// скролером (`overflow-y: auto`), рисочка лежала в ньому звичайним вузлом, а ✕ мав
// `position: absolute` — а абсолют усередині скролера прокручується разом із вмістом.
// Тобто ваду успадкували ВСІ 10 модулів, що кличуть примітив.
//
// ⚠️ ЧОМУ НЕ ЗРОБИВ ТРИ ЯРУСИ, ЯК У КОМПОЗЕРІ СТРІЧКИ (head/body/bar). Це було б
// красивіше, але зламало б двох викликачів, які СПИРАЮТЬСЯ на те, що скролером є
// сам аркуш: `community-modal.js` слухає `sheet.scroll` для класу `is-scrolled`, а
// `community-blocks.js` (погода) вішає власний свайп на `.app-modal-sheet`.
// Липка шапка всередині того самого скролера дає той самий результат і не чіпає
// нічого з цього.
//
// 💡 Взірець не вигаданий: рівно так уже працювала ОДНА модалка — «Подати
// оголошення» (`.app-modal--board-compose`), яка від 29.07 мала власні
// `position: sticky` для рисочки й ✕. Тобто рішення в проєкті було, але жило
// прибитим до однієї модалки. Тепер воно в базі, а та модалка свої копії віддала.
function buildSheet({ title, bodyHtml, dismissible }) {
  return `
    ...
    <div class="app-modal-sheet" role="dialog" aria-modal="true"${title ? ` aria-label="${escapeHtml(title)}"` : ''}>
      ${dismissible ? `<div class="app-modal-head"><div class="app-modal-handle"></div>${closeBtnHtml()}</div>` : ''}
