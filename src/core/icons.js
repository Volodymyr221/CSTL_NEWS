...
// stroke-width 1.75 — обрано Ромою 12.07 (sw1 занадто тонка/хайрлайн, sw2 забагато;
// 1.75 = золота середина). Заокруглені кінці/стики, 1em-масштаб під розмір тексту
// поруч. Змінити тут — переміняться ВСІ JS-іконки одним рядком.
const A = 'width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" class="cat-ico"';
...
export const ICONS = {
  ...
  // Крапля — ймовірність опадів у прогнозі погоди (19.08). Tabler `droplet`.
  droplet: `<svg ${A}><path d="M7.502 19.423c2.602 2.105 6.395 2.105 8.996 0c2.602 -2.105 3.262 -5.708 1.566 -8.546l-4.89 -7.26c-.42 -.625 -1.287 -.803 -1.936 -.397a1.376 1.376 0 0 0 -.41 .397l-4.893 7.26c-1.695 2.838 -1.035 6.441 1.567 8.546z"/></svg>`,
  // Замок / приватність
  ...
  // 🆕 17.08 — розділ «Збори» (меню + адмінка). У наборі не було нічого про
  // ДОПОМОГУ: `shield` уже означає «Для захисників» і «Правила Дошки»,
  // `usersGroup` — «Громада», монета читалась би як «витрати» (така іконка вже
  // стоїть на «Витрати AI»). Серце в долоні — про допомогу, а не про фінанси.
  // Той самий малюнок стоїть на плитці розділу в `admin.html` (ICO_HEART_HAND):
  // один знак в обох місцях, щоб розділ упізнавався.
  heartHand: `<svg ${A}><path d="M12 9.5l-.66 -.61a1.66 1.66 0 0 0 -2.35 0a1.75 1.75 0 0 0 0 2.44l3.01 2.67l3.01 -2.67a1.75 1.75 0 0 0 0 -2.44a1.66 1.66 0 0 0 -2.35 0l-.66 .61z"/><path d="M3 13h2l3 3h4a1.5 1.5 0 0 1 0 3h-3"/><path d="M5 16v5"/><path d="M14 19l4 -1a2 2 0 0 1 2 2v0"/></svg>`,
  ...
  // 🆕 31.08 — блок телефонів громади (`tabs/home-contacts.js`). До цього дня
  // категорії контактів були ЄДИНИМ місцем застосунку, де ще стояли емодзі
  // (🏛 🏥 ⚡ 📦 👮 📇), і поруч із векторною рештою вони читались як чужі:
  // в емодзі своя товщина, свій оптичний центр і колір, який не слухається
  // `currentColor`. Той самий висновок уже записаний у `back` про текстовий «←».
  // Медицина (Tabler stethoscope) — амбулаторія. ⚠️ Не «хрест»: хрест у наборі
  // означає ПЛЮС/додати, і на контакті читався б як «додати контакт».
  stethoscope: `<svg $${A}><path d="M6 4h-1a2 2 0 0 0 -2 2v3.5a5.5 5.5 0 0 0 11 0v-3.5a2 2 0 0 0 -2 -2h-1"/><path d="M8 15a6 6 0 1 0 12 0v-3"/><path d="M11 3v2"/><path d="M6 3v2"/><path d="M18 10a2 2 0 1 0 4 0a2 2 0 1 0 -4 0"/></svg>`,
  // Копіювати (Tabler copy) — кнопка «скопіювати номер». Було текстове «⧉»:
  // гліф, тобто літера зі своєю метрикою, поруч із векторною «на карті».
  ...
  // Установа / органи влади (Tabler building-bank) — сільрада, ЦНАП.
  // ⚠️ Не `community` (Tabler building-community): той уже носить категорію
  // «Для громади» на картці збору, і два різні сенси на одному значку
  // розходяться першою ж зміною. Та сама причина, що описана в `usersGroup`.
  bank: `<svg $${A}><path d="M3 21l18 0"/><path d="M3 10l18 0"/><path d="M5 6l7 -3l7 3"/><path d="M4 10l0 11"/><path d="M20 10l0 11"/><path d="M8 14l0 3"/><path d="M12 14l0 3"/><path d="M16 14l0 3"/></svg>`,
};

// ── ЗНАЧОК ВКЛАДКИ: ЧИТАЄМО З ТАБ-БАРУ, А НЕ ТРИМАЄМО КОПІЮ ──────────────────
//
// 🔴 ЧОМУ ФУНКЦІЯ, А НЕ ЩЕ ОДИН РЯДОК В `ICONS`. Значки вкладок живуть у
// `index.html` і переробляються найчастіше в проєкті: іконку «Питання» Вова
// перемальовував СІМ разів за серпень (історія — коментар над нею в `index.html`).
// Копія такого малюнка в JS розійшлася б із оригіналом при першій же редакції —
// і розійшлась би МОВЧКИ, бо обидві версії правильні самі по собі.
//
// ➡️ Тому екран, який веде на вкладку, БЕРЕ її значок із таб-бару. Джерело
// фізично одне, а не «домовились однакові». Переїхало сюди 06.09 з
// `core/sidebar.js`, коли споживачів стало двоє: бургер-меню і хаб «Збережені».
//
// ⚠️ РОЗМІР НОРМАЛІЗУЄ CSS СПОЖИВАЧА, і це не дрібниця: у таб-барі значок має
// ЖОРСТКІ 22px (`.tab-icon`), а значки з `ICONS` — 1em. Без правила на `svg`
// рядки списку стрибали б по висоті залежно від того, звідки прийшов значок.
// Приклади: `.sidebar-item-icon svg` і `.shub-cat-ic svg`.
//
// `fallback` обовʼязковий: таб-бару може не бути (стенд, який будує лише аркуш),
// і тоді екран мусить показати свій значок, а не порожнє місце.
export function tabIcon(tab, fallback) {
