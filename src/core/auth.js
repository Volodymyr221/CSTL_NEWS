...
export async function saveProfile(fields = {}) {
  ...
  if (_user) writeProfileCache(_user.id, { name: _profileName, avatar_url: _profileAvatar });
  // 🔴 17.09 — СКАЗАТИ ЗАСТОСУНКУ, ЩО ПРОФІЛЬ ЗМІНИВСЯ.
  // 🗣️ Скарга Вови: «оновлюю імʼя чи фотографію в кабінеті, виходжу на Громаду —
  // у привітанні старе. Треба закрити застосунок і зайти знову».
  // 🔬 Причина була саме тут: рядки вище чесно оновлюють і памʼять, і кеш на
  // пристрої — тобто ДАНІ свіжі одразу. Але ЕКРАН про це не дізнавався: привітання
  // перемальовує `updateGreetingName()`, а її кликали лише на старті вкладки і на
  // `onAuthChange`. Зміна імені — це не зміна входу, тож подія не наставала ніколи.
  // 🔑 Подія, а не прямий виклик: `auth.js` — ядро, воно не сміє знати про Громаду,
  // шапку чи бічне меню. Хто показує імʼя — той і слухає.
  try {
    window.dispatchEvent(new CustomEvent('cstl-profile-updated', {
      detail: { name: _profileName, avatar_url: _profileAvatar },
    }));
  } catch (_) {}
