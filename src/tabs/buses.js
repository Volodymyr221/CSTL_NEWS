...
function buildListTitleHtml(updatedStr) {
  return `<div class="bus-list-title">Розклад автобусних маршрутів<span class="bus-list-date-sub">${formatBusDayTitle()}</span><span class="bus-list-updated-sub">${updatedStr}</span>${buildThinDayHtml()}</div>`;
}

// ── 🔴 21.09 — «ДЖЕРЕЛО ПОКАЗУЄ МАЛО РЕЙСІВ» ────────────────────────────────
//
// 🔬 ЗАРАДИ ЧОГО, З ЖИВОГО ВИПАДКУ. 21.09 у VOPAS на цю дату стояв РІВНО ОДИН
// рейс, тоді як на 22.09 — двадцять чотири. Парсер відпрацював точно: узяв те,
// що є. Але застосунок написав «СЬОГОДНІ РЕЙСІВ БІЛЬШЕ НЕ ЗАПЛАНОВАНО» — тобто
// видав «ми не знаємо» за «рейсів немає». Людина читає це і не йде на зупинку,
// хоча автобус, найімовірніше, їде: перевізник просто не виставив день у
// системі продажу квитків.
//
// 🔑 ПОРІВНЮЄМО З СУСІДНІМИ ДНЯМИ ТОГО Ж ТИЖНЯ, А НЕ З ПОРОГОМ У ГОЛОВІ.
// Зашите число («менше 5 — попереджай») було б вигадкою: у неділю рейсів
// законно менше, у свято теж. А сусідній день — це мірка, яку дає сам розклад.
// 📐 Третина: 1 проти 24 — кажемо; 28 проти 30 — мовчимо.
//
// 🛑 ЧОМУ СІРИЙ РЯДОК, А НЕ ЧЕРВОНИЙ БАНЕР. Це не поломка, а невизначеність.
// Червоне навчить лякатись там, де просто бракує даних, — і за місяць його
// перестануть помічати разом зі справжніми тривогами.
const ТОНКИЙ_ДЕНЬ_ЧАСТКА = 1 / 3;

function buildThinDayHtml() {
  const дні = busData?.days;
  if (!дні) return '';
  const сьогодні = (дні[busDay]?.routes || []).length;
  if (!сьогодні) return '';          // порожній день — про нього каже інший екран

  // Сусіди того ж тижня: беремо найбільший, бо питання саме «чи бувало більше».
  const усі = Object.keys(дні).sort();
  const i = усі.indexOf(busDay);
  if (i < 0) return '';
  const сусіди = [усі[i - 1], усі[i + 1]]
    .filter(Boolean)
    .map(d => (дні[d]?.routes || []).length);
  if (!сусіди.length) return '';
  const найбільший = Math.max(...сусіди);
  if (!найбільший || сьогодні >= найбільший * ТОНКИЙ_ДЕНЬ_ЧАСТКА) return '';

  const [р, м, д] = busDay.split('-');
  const дата = `${д}.${м}.${р}`;
  const рейс = сьогодні === 1 ? 'рейс' : (сьогодні < 5 ? 'рейси' : 'рейсів');
  // 🔑 Посилання — СЛОВОМ, не голою адресою: так само, як у підвалі
  // (`buildSourceHtml`). Гола адреса в рядку тексту читається як помилка верстки.
  return `<span class="bus-list-thin">Джерело показує ${сьогодні} ${рейс} на ${дата}.
      Перевірте детальну інформацію на <a href="https://vopas.com.ua" target="_blank" rel="noopener">VOPAS</a></span>`;
}
...
function syncDropdownHeight() {
  ...
  // 🔴 ДРУГА ПОПРАВКА (10.08, знімок Вови): низ вікна — ще не низ ВІЛЬНОГО місця.
  // Знизу стоїть таб-бар, а по центру він має випуклий острівець із кнопкою
  // «Громада», який піднімається ще вище за смужку. Перша редакція рахувала до
  // краю екрана — і останні зупинки («Хорлупи», «Котів») ішли ПІД таб-бар.
  // Вова: «зроби так, щоб нижній край розташовувався над тим випуклим
...
function openDropdown(field) {
  ...
  // 🔴 ЗАМОК ПРОКРУТКИ СТОРІНКИ, поки список відкритий (10.08, знімок Вови).
  // Скарга: «коли натискаю на поле вводу, воно дьоргається» — шапка їде вгору й
  // обрізається. Причина: iOS на фокус у полі прокручує НАЙБЛИЖЧИЙ прокручуваний
  // предок, «щоб показати поле». Сам список `position: fixed`, тож прокручується
  // сторінка під ним. Забираємо в iOS предмет прокрутки — і смикати нічого.
  ...
  // (`pointerdown` → `preventDefault`) це дало РЕГРЕСІЮ, яку Вова описав так:
  // «вилазить згорнута модалка типу як ніби є клавіатура, хоча клавіатури
  // немає». Механіка: iOS відкриває клавіатуру на програмний `.focus()` лише
  // всередині вікна користувацького жесту. Раніше жестом був сам тап у поле-
  // кнопку; коли я цей фокус прибрав, єдиним жестом лишився `click` — а
  // `setTimeout` виносить виклик ЗА його межі, і система клавіатуру не дає.
  // Синхронний виклик лишається всередині обробника `click`, тобто в жесті.
  // 🛑 Не повертати таймер «щоб дати списку домалюватись»: список уже в DOM
  // рядком вище (`renderDropdownItems` + `hidden = false`).
...
export function openSavedRouteOnBuses(rid, date, from, to) {
  ...
  fromStop      = from || '';
  toStop        = to   || '';
  // 🔴 21.08 — ПАНЕЛЬ ПОШУКУ ТЕЖ ПЕРЕМАЛЬОВУЄТЬСЯ. Скарга Вови: «якщо я з
  // капсули переходжу на Олика → Жорнище, то в „Звідки“ також познач Олику, і
  // щоб можна було скинути».
  // 🔑 Причина була не в тому, що зупинку не передавали — її передавали й
  // ставили в `fromStop`. Не перемальовували ПОЛЕ: значення жило в пам'яті й
  // фільтрувало список, але людина бачила порожнє «Звідки» і кнопки «скинути»
  // не мала. Тобто фільтр діяв невидимо — гірший стан, ніж якби його не було.
  renderSearchPanel();
  renderWeekStrip();
  renderSmartRow();
  renderRouteList();
...
export async function initBuses() {
  ...
  try {
    // 🔴 24.09 — `?v=` ПРИБРАНО. Тут стояв cache-bust кожну хвилину:
    // `?v=${Math.floor(Date.now() / 60000)}`. Адреса мінялась щохвилини, тож ні
    // `ETag`, ні `304`, ні кеш Service Worker не спрацьовували жодного разу —
    // файл на **814 КБ** їхав мобільним інтернетом наново при кожному відкритті
    // вкладки в нову хвилину. Це на селі не дрібниця.
    // 🔑 Свіжість від цього не постраждала і не могла: `sw.js` тримає весь
    // `data/*.json` на «мережа першою» (`sw.js:239`), тобто запит однаково йде
    // в мережу, а кеш — лише запасний шлях при обриві. Cache-bust тут не давав
    // свіжості, він лише забороняв серверу відповісти «не змінилось».
    // 📐 Джерело оновлюється кілька разів на добу (`vopas-parser`), а качалось
    // щохвилини.
    // ⚠️ Заразом зникла розбіжність двох точок входу: капсула на Громаді
    // (`home-caps.js:444`) ходить за тим самим файлом БЕЗ `?v=` — тобто вона й
    // вкладка бачили різні копії того самого розкладу.
