...
function syncBoardBodyOffset() {
  ...
  // схованою) і зсув класу `--shown`. Прочитати власну висоту CSS не вміє, тому
  // віддаємо змінною.
...
function setupHeaderCollapse() {
  ...
  // 🔴 06.08 (ТРЕТІЙ ПІДХІД) — ШАПКУ БІЛЬШЕ НЕ РУХАЄ JS. Її прокручує браузер.
  //
  // Вова: «дьоргається все одно, зроби так, щоб на початку сторінки при скролі
  // воно ховалось так, ніби це одна сторінка».
  //
  // ЧОМУ ДВІ ПОПЕРЕДНІ СПРОБИ БУЛИ ПРИРЕЧЕНІ. Обидві рухали `fixed`-шапку з
  // обробника `scroll`: спершу через `rAF`, потім синхронно і без розмиття.
  // Причина дьоргання структурна: на iOS прокрутку виконує КОМПОЗИТОР в окремому
  // потоці, а події `scroll` доходять до JS із затримкою і не щокадру. Тобто
  // JS-driven рух «за пальцем» на мобільному не буває плавним у принципі —
  // скільки його не оптимізуй. Правило CLAUDE.md на такий випадок пряме: «дві
  // невдачі підряд = пре-мортем і стоп, а не третя спроба». Тому змінено саму
  // конструкцію, а не її налаштування.
  //
  // ЯК ТЕПЕР. `.bd-controls` для Дошки — `position: sticky` з ВІД'ЄМНИМ `top`
  // (див. `style/board.css`). Вона просто лежить у потоці й прокручується разом
  // зі сторінкою силами браузера, а дійшовши до верху — прилипає вже схованою.
  // Тобто «одна сторінка» дослівно, і дьоргатись нема чому: JS у цьому русі не
  // бере участі взагалі.
  //
  // ЩО ЛИШИЛОСЬ ЗА JS — рівно одна дискретна дія: показати сховану шапку, коли
  // людина повела пальцем УГОРУ з середини сторінки (клас `--shown`, разова
  // анімація 0.28с). Це не покадровий рух, тому воно й раніше не дьоргалось.
  //
  // ⚠️ Пороги лишились ті самі, що були до всіх переробок: ховати пізніше (110),
  // повертати швидко (70) — патерн Twitter/Instagram, обґрунтування нижче.
  const HIDE_AFTER = 110;  // px донизу від зміни напрямку → сховати (зняти показ)
  const SHOW_AFTER = 70;   // px вгору від зміни напрямку → показати
  let lastY = main.scrollTop;
  let accDown = 0, accUp = 0;   // накопичений шлях у кожному напрямку
  let ticking = false;

  // Висота шапки — ЛИШЕ щоб знати, чи вона вже поза екраном (нижче цієї межі
  // показ має сенс, вище шапку й так видно). Кешуємо: `offsetHeight` у обробнику
  // прокрутки — примусовий перерахунок розкладки.
  let cachedH = 0;
  const headerH = (el) => (cachedH ||= el.offsetHeight + 6);
  _invalidateHeaderH = () => { cachedH = 0; };

  // ⚠️ Стан НЕ тримаємо у змінній-прапорці: `renderAll()` перестворює
  // `.bd-controls`, і новий вузол приходить без класів — прапорець у замиканні
  // почав би брехати про DOM. `classList.toggle` з тим самим значенням
  // перемальовки не викликає, тож ідемпотентність тут безкоштовна.
  const setShown = (el, on) => el.classList.toggle('bd-controls--shown', on);

  // Кеш вузла шапки. Перевірка `isConnected` обов'язкова: `renderAll()`
  // перестворює `.bd-controls`, і старе посилання вказувало б на викинутий вузол —
  // клас лягав би в нікуди, а шапка застигла б показаною.
  let elCache = null;
  const controlsEl = () => {
  ...
  const apply = () => {
    ...
    // Поки шапка ще в межах свого місця у потоці — вона просто прокручується
    // разом зі сторінкою, і клас показу тут не потрібен: він опустив би її
    // НИЖЧЕ, ніж вона й так стоїть.
    if (y <= headerH(el)) {
      accDown = accUp = 0;
      setShown(el, false);
      return;
    ...
    if (dy > 0) {                 // рух вниз
      ...
      accUp -= dy; accDown = 0;
      if (accUp >= SHOW_AFTER) setShown(el, true);
    }
  };

  // `rAF`-throttle тут ДОРЕЧНИЙ і повернувся: JS більше не малює рух шапки, він
  // лише вирішує, чи вмикати клас показу. Кадрова точність для цього не потрібна,
  // а зайва робота на кожній події прокрутки — потрібна ще менше.
  main.addEventListener('scroll', () => {
    if (!ticking) { ticking = true; requestAnimationFrame(apply); }
  }, { passive: true });

  // Зміна розміру екрана (поворот, зміна вікна) → перерахувати геометрію шапки.
  // ⚠️ Заразом скидаємо кеш висоти: після повороту вона інша, а від неї залежить
  // і межа «шапка вже поза екраном», і `--bd-h`, на якій тримається `sticky`.
