...
// Відмінювання «питання / питання / питань» для рядка стану під пошуком.
function wordПитань(n) {
  const m10 = n % 10, m100 = n % 100;
  if (m10 === 1 && m100 !== 11) return 'питання';
  if (m10 >= 2 && m10 <= 4 && (m100 < 12 || m100 > 14)) return 'питання';
  return 'питань';
}
...
function renderHeader() {
  ...
  // 🔴 11.08, ЧЕТВЕРТИЙ ЗАХІД — ДОДАНО КОРОТКЕ ПОЯСНЕННЯ РОЗДІЛУ.
  // Пряме прохання Вови: «незрозуміло для чого це і що… треба якусь інструкцію,
  // може таку мінімальну, в одне речення чи в два».
  // 🔑 Претензія «незрозуміло, що це» звучала ТРИЧІ поспіль, і щоразу я лікував її
  // ВИГЛЯДОМ — банером, потім типографікою, потім щільністю списку. Жодного разу
  // не спрацювало, бо вигляд не може сказати те, що каже речення. Слоган
  // «Не знаєте — запитайте сусідів» кличе до дії, але не пояснює, ЩО тут відбувається
  // і що людина отримає.
  // 🛑 Два рядки, не банер: банер на пів екрана вже відхилено, і саме він з'їдав
  // третину висоти. Тут — звичайний текст під заголовком, ~40px.
  // ⚠️ Текст МУСИТЬ лягати у два рядки. Перша редакція була на 110 символів і
  // зайняла чотири — разом із Large Title і пошуком це відсунуло перше питання на
  // 36% висоти екрана, тобто повторило помилку банера, тільки текстом.
  const пояснення = `<p class="qa-hero-note">Питайте сусідів про місцеве — графіки, довідки, події. Знаєте відповідь — допоможіть.</p>`;

  // Рядок стану під пошуком: скільки всього і скільки чекає на допомогу.
  // Дає розділу відчуття живого — і показує, де людина потрібна. Малюється лише
  // коли питання є (на порожньому екрані «0 питань» — це не інформація).
  // 🔴 11.08, РЕДАКЦІЯ 8 — ЛІЧИЛЬНИК І ЧІПИ БІЛЬШЕ НЕ ДУБЛЮЮТЬ ОДНЕ ОДНОГО.
  // Скарга Вови зі знімка: «верхній блок занадто широкий, займає дуже велику
  // частину екрану». Одна з причин була буквальною: щойно питань стало 12,
  // з'явився ряд чіпів «Без відповіді 4», а на 60px нижче стояв рядок
  // «12 питань · 4 без відповіді» — ТЕ САМЕ ЧИСЛО двічі, двома рядками.
  // ➡️ Числа переїхали В ЧІПИ («Усі 12» · «Без відповіді 4»), а окремий рядок
  //    лишається лише тоді, коли чіпів немає (менше 8 питань) — там він єдине
  //    джерело «скільки тут усього».
  const лічильник = (усьогоПитань && !показатиЧіпи) ? `
    <p class="qa-hero-count">${усьогоПитань} ${wordПитань(усьогоПитань)}${
      unanswered ? ` · <span class="qa-hero-need">${unanswered} без відповіді</span>` : ''}</p>` : '';

  // 🔴 ШАПКУ РОЗДІЛЕНО НА ДВІ ЧАСТИНИ — і це головна правка редакції 8.
  //
  // Було: заголовок + пояснення + чіпи + пошук + лічильник жили ОДНИМ блоком
  // `.bd-controls`, а він у цій зоні `position: fixed` — тобто **242px висоти
  // (38% екрана) були приклеєні до верху назавжди**, і список їхав під ними.
  // Саме це Вова й описав: «дуже багато місця займає оце все зверху, а знизу
  // там десь скроляться карточки».
  //
  // 🔑 Дошці цю саму хворобу вилікували ще 06.08 (шапка поїхала в потік і зникає
  // при прокрутці), а Питання лишились зі старою конструкцією. Тобто прохання
  // «стандартизувати, як в інших випадках» — буквально про це.
  //
  // Стало, за стандартом iOS (Mail, Повідомлення, Налаштування):
  //   ЗНАЙОМСТВО (Large Title + пояснення) — у ПОТОЦІ списку, їде геть назавжди;
  //   ІНСТРУМЕНТИ (чіпи + пошук)           — лишаються зафіксованими.
  // Пояснення нікуди не зникло: людина, яка заходить уперше, бачить його першим
  // же кадром — а той, хто вже все знає, більше не платить за нього щоразу.
  ...
  // 🔴 05.09«b» — РЕЖИМ «ЗБЕРЕЖЕНІ» ОТРИМУЄ СВОЮ ШАПКУ.
  // 🗣️ Скарга Вови зі знімка: «вона ніби змінила вигляд, пошук притиснутий до
  // верху, UI UX взагалі немає».
  // 🛑 Він має рацію, і це МОЯ регресія того ж дня. Прибираючи дубль-екран
  // `openSavedAds`, я звірив, що режим Дошки вміє все ФУНКЦІОНАЛЬНО (пошук,
  // повні картки, зняття закладки) — і жодного разу не подивився на його ХРОМ.
  // А в ньому не було нічого: `showCategories` false → бордовий блок не
  // малювався взагалі, і в шапці лишався голий рядок пошуку заввишки 41px.
  // 📐 Заміряно на живому екрані: `.bd-controls` мала рівно одну дитину
  // (`bd-search-row`), тобто ні назви екрана, ні лічильника, ні ВИХОДУ.
  // Останнє найгірше: вийти з режиму було НІЯК — `activeType` скидався лише при
  // виході з акаунта. Людина заходила в «Збережені» й лишалась у глухому куті.
  const isSaved = activeType === 'saved';
  ...
  // Шапка «Збережених» — та сама бордова мова, що в Дошки, але зі своїм змістом:
  // ← (вихід), назва екрана і лічильник. Фільтрів категорії й локації тут немає
  // навмисно: це не каталог громади, а власна поличка людини.
  // 🔑 Лічильник рахує те, що ЗАРАЗ У СПИСКУ, а не всю поличку: під ним стоїть
  // пошук, і число «7» над двома видимими картками брехало б рівно так, як
  // брехали капсули з применшеним числом (`HOT_RULES` №12).
  const savedCount = isSaved ? getFilteredPosts().length : 0;
  const savedHeroHtml = isSaved ? `
    <div class="bd-hero bd-hero--saved">
      <button class="bd-hero-back" id="bd-saved-exit" type="button"
              aria-label="Вийти зі збережених — до всіх оголошень">${ICONS.back}</button>
      <div class="bd-hero-text">
        <h2 class="bd-hero-title">Збережені</h2>
        <p class="bd-hero-sub">${savedCount
          ? `${savedCount} ${pluralAds(savedCount)} · тап по закладці знімає`
          : 'Оголошення, які ви позначили закладкою'}</p>
      </div>
    </div>
  ` : '';
  ...
  // ⚠️ Бордовий блок тепер малюється і для «Збережених» — саме він тримає
  // заокруглені кути, тінь і білий текст пошуку. Без нього рядок пошуку лежав
  // голим на матовому склі, і екран не мав ні верху, ні імені.
  const titlebarHtml = (showCategories || isSaved) ? `
    <div class="bd-titlebar">
      ${showCategories ? heroHtml : savedHeroHtml}
      ${searchRowHtml}
    </div>
  ` : '';
  ...
  return `
    <div class="bd-controls">
      ${discHead}
      ${titlebarHtml}
      ${(showCategories || isSaved) ? '' : searchRowHtml}
    </div>
  `;
}
...
function emptyStateHtml() {
  if (activeType === 'saved') {
    // 🔑 Порожній стан із ДІЄЮ, а не тупик. Тут людина найменш схильна шукати,
    // куди тапнути, — і саме тут раніше не було ЖОДНОЇ кнопки, лише текст на
    // пів екрана. Кнопка веде туди, де закладку можна поставити.
    // ⚠️ Кнопка в ОКРЕМОМУ блоці, а не одразу після тексту: `.bd-empty-reset`
    // має `display: inline-block`, тож у потоці вона затікала в останній рядок
    // («…зʼявиться тут. [До всіх оголошень]»). Видно лише на екрані — у розмітці
    // це виглядає бездоганно.
    return `У «Збережених» поки нічого.<br>Натисніть закладку на оголошенні — і воно зʼявиться тут.
      <div><button class="bd-empty-reset" type="button" data-bd-saved-exit>До всіх оголошень</button></div>`;
  }
...
//
// 🔴 11.08, РЕДАКЦІЯ 8 — ДВІ СПРОБИ, І ПЕРША БУЛА НЕПРАВИЛЬНА. Спершу я виніс
// знайомство В ПОТІК СПИСКУ, лишивши зафіксованими чіпи й пошук. Числа стали
// добрі (шапка 242→94px), але знімок показав те, чого числа не бачать:
// **чіпи й пошук опинились НАД назвою розділу** — тобто спершу «Усі 12 · Без
// відповіді», і аж потім «Питання». Фільтр перед назвою того, що фільтруємо.
// ➡️ Правильна конструкція вже була в проєкті: у Дошки шапка лежить у ПОТОЦІ
// (`position: sticky` з відʼємним `top`) і їде геть цілком. Тоді порядок
// природний — назва, пояснення, інструменти, список, — а при прокрутці екран
// звільняється ПОВНІСТЮ, а не на 94px. Саме це Вова й називав «стандартизувати,
// як в інших випадках».
// 🔑 Функція лишається окремою і без стану, хоч зараз її кличе лише
// `renderHeader()`: тіло списку перемальовується САМЕ ПО СОБІ (`renderBodyOnly`
// на кожен символ пошуку), і повертати сюди приховану залежність «спершу шапка,
// потім тіло» не варто.
function qaHeroHtml() {
  if (activeType !== 'chat') return '';
  return `
    <div class="qa-hero">
...
function renderBody() {
  ...
  // 🔴 11.08, РЕДАКЦІЯ 3 — ПИТАННЯ ЙДУТЬ ОДНИМ СУЦІЛЬНИМ БЛОКОМ, А НЕ ОКРЕМИМИ
  // КАРТКАМИ. Це патерн «Налаштувань» і «Пошти» iOS (inset grouped list).
  //
  // Чому не картки. Заміряно на живих даних: при трьох питаннях картки лишали
  // **21% екрана порожнім**, і кожна забирала 124px. У громаді з пʼятьма
  // питаннями список карток виглядає бідним — і жоден стиль цього не лікує, бо
  // причина не в оформленні, а в кількості контенту. Суцільний блок дає
  // **5 питань на екран замість 3** при тій самій висоті рядка.
  // ⚠️ Це вже ТРЕТЯ редакція вигляду: банер із рамками Вова відхилив
  // («фейсбук 2006»), картки без рамок — теж («все одно не подобається»). Тому
  // цього разу міняється не оформлення, а САМА КОНСТРУКЦІЯ списку.
  //
  // «Збережені» (`saved`) лишаються стрічкою карток — там оголошення Дошки, у них
  // фото і ціна, у рядок вони не лягають.
  if (activeType === 'chat') {
    return `
      <div class="board-backdrop" id="board-backdrop"></div>
  ...
  // saved — вертикальна стрічка карток.
...
async function loadBoardData() {
  ...
  // 🔴 25.08 — ЧЕКАЄМО ФАКТ «ХТО Я» (беклог, пункт 0). Це та сама вада, що лікували
  // у Стрічці (PR #1020), тільки тут вона коштує більше, бо `uid` іде В ОБИДВА
  // персональні запити нижче — і вони для РІЗНИХ вкладок:
  //   • `fetchSavedPostIds(uid)` → `setSavedIds()` — це **ЗБЕРЕЖЕНІ ОГОЛОШЕННЯ ДОШКИ**.
  //     Без `uid` запит навіть НЕ ВИКЛИКАЄТЬСЯ (тернарний оператор нижче), множина
  //     лишається порожньою: позначка «збережено» згасла, таб «Збережені» порожній.
  //   • `fetchAllReactions(uid || getAnonId())` → `setDiscussionsData()` — це ❤️
  //     **«МЕНЕ ТЕЖ ЦІКАВИТЬ» У ПИТАННЯХ**, на ньому тримається `isLikedByMe()`.
  //     Без `uid` запит іде про АНОНІМА, тобто моє «цікавить» не знаходиться — і
  //     питання, яке я позначив, перестає бути «моєю справою». Наслідок видно не
  //     тут, а в Питаннях: підсвітка нової відповіді на нього мовчить.
  //
  // 🛑 ЩО ТУТ ЛЕГКО ПРОЧИТАТИ НЕПРАВИЛЬНО (виправлено 25.08 після зауваження Вови:
  // «які сердечка на дошці? там є тільки збереження, реакції ми давно вже забрали»).
  // Він має рацію, і це перевірено `grep`: у `board.js` НЕМАЄ ЖОДНОЇ згадки про
  // реакції — на картці оголошення їх не існує. Назва `fetchAllReactions` загальна,
  // а живе виклик тут лише тому, що Дошка й Питання ділять ОДИН запит на дві
  // вкладки. ⚠️ І скарга «мій лайк не підсвічується» була про СТРІЧКУ (інша
  // таблиця, `page_reactions`, полагоджено в PR #1020) — до цього рядка вона
  // стосунку не має. Перша редакція цього коментаря приписувала її Дошці.
  //
  // ⚠️ `initBoard()` кличе `renderBoard()` → сюди НА САМОМУ СТАРТІ, тобто перегони
...
export async function renderBoard() {
  ...
  // 🔑 `discOpen` розрізняє, ЧИЙ це корінь: Дошка і Питання ділять цей код, але
  // форми в них різні — картка оголошення 366×125 проти рядка питання 358×165.
  // Один кістяк на обидві означав би, що на одній із вкладок список підстрибне.
  paintLoading(el, 3, true, discOpen ? 'questions' : 'board');   // `true` — замінити застаріле «Тут поки порожньо»
...
function renderAll() {
  ...
  // 28.07: у Дошки прибрано ЗАТЕМНЕНІ КРАЇ (`.board-vignette` — градієнт
  // rgba(0,0,0,0.58) на 140px згори). Вони були частиною «темного корка» і на
  // новому світло-сірому фоні читались би як брудна смуга через екран.
  //
  // 🔴 19.09 — ВІНЬЄТКУ ЗНЯТО І З ПИТАНЬ (скарга Вови: «зверху над блоком шапки
  // падає якась тінь на питання. Її немає в жодній вкладці, тому її потрібно
  // забрати»).
  // 🔑 Вона дожила сюди не як рішення, а як ЗАБУТИЙ ХВІСТ. 28.07 її лишили з
  // обґрунтуванням «Обговорення мають теплий беж `#E7E1D2`, його не чіпаємо» —
  // а 11.08 Обговорення стали Питаннями, і фон їм поміняли на `--app-bg`.
  // Тобто підстава зникла того ж місяця, а сама тінь лишилась: затемнення
  // 58% поверх світло-сірого дає рівно ту брудну смугу, від якої її знімали
  // з Дошки. 📐 Заміряно контролем (bundle+css з `origin/main`): смуга 390×140
  // від самого верху при шапці 59px — тобто **81px затемнення просто над
  // картками**, 34% чорного на верхньому краї.
  ...
  // Вихід зі «Збережених» — і з шапки (←), і з порожнього стану. Делегування на
  // корені: обидві кнопки живуть у розмітці, яку `renderAll` перестворює.
  el.addEventListener('click', (e) => {
    if (!e.target.closest('#bd-saved-exit') && !e.target.closest('[data-bd-saved-exit]')) return;
    activeType = 'board';
    activeCategory = 'all';
    searchQuery = '';
    renderAll();
...
function playAskHint(зона = 'discussions') {
  ...
  // Вова побачив «Запитат». 🔑 Урок не про 2px: **формула, що дублює числа з CSS,
  // ламається щоразу, коли CSS міняють.** Тепер ширину кнопки рахує браузер
  ...
  askHintTimers.push(setTimeout(() => {
    ...
    requestAnimationFrame(() => {
      ...
      // 🔴 15.08 — ПОВНИЙ ОБЕРТ (360°), а не пів (180°). Скарга Вови: «іконка
      // повідомлення перевернута догори ногами… вона має ставати в свій
      // звичайний режим». Плюс симетричний, тому півоберту на ньому не видно —
      // а на Дошці при непрочитаних видно КОНВЕРТ, і 180° лишали його
      // перевернутим. Повний оберт лишає малюнок правильним ЗА ПОБУДОВОЮ, і
      // при цьому руху стає більше, а не менше (Вова просив помітніше).
      if (icon) icon.style.transform = 'rotate(360deg)';
    });
  }, 420));   // невелика пауза після входу — щоб рух не злився з появою сторінки

  // Крок 2 — тримаємо розгорнутою. 2.6с: менше не встигає прочитати НІХТО —
  // норма читання ~200 слів/хв дає на одне слово ~0.3с, а решта часу йде на те,
  // щоб помітити рух і перевести погляд. Більше — вже не підказка, а плашка,
  // від якої йшли.
  // _(12.08: розрахунок був підписаний «людина 60+»; Вова виправив аудиторію на
  // 18-40+. Число не змінюється: воно виведене зі швидкості читання, а не з віку.)_
  askHintTimers.push(setTimeout(() => {
    btn.classList.remove('qa-fab-wide');
    // Іконка докручується ВПЕРЕД до 720°, а не відкручується назад: зворотний
    // рух читався б як «кнопка передумала». Крок такий самий, як на розгортанні
    // (повний оберт), тож будь-яка іконка стоїть рівно на обох кінцях.
...
function syncBoardBodyOffset() {
  ...
  //
  // 🔴 11.08 — ПИТАННЯ ПЕРЕВЕДЕНО НА ТУ САМУ КОНСТРУКЦІЮ, ЩО ДОШКА.
  // До цього їхня шапка лишалась `fixed`, і це коштувало **242px = 38% екрана,
  // приклеєних назавжди** (заголовок + пояснення + чіпи + пошук + лічильник), а
  // список їхав під ними. Вова зі знімка: «верхній блок занадто широкий, займає
  // дуже велику частину екрану, а знизу там десь скроляться карточки».
  // Тепер обидві зони живуть однаково: шапка в потоці, при прокрутці їде геть
  // цілком, екран звільняється повністю. Тіло відступу під шапку більше не
  // тримає — вона й так займає своє місце.
  // ⚠️ `--bd-h` ставимо на КОРІНЬ, а не на саму шапку: `renderAll()` перестворює
  // `.bd-controls`, і змінна, записана на неї, зникала б разом із вузлом.
  root.style.setProperty('--bd-h', h + 'px');
  if (root.id !== 'board-content') body.style.paddingTop = '';
}
...
export function initBoard() {
  ...
  window.addEventListener('cstl-tab-changed', async () => {
    ...
    // підказку рівно тоді, коли людина прийшла нею скористатись.
    if (пішли_з === 'discussions' && вкладка !== 'discussions') markQuestionsSeenOnLeave();
    ...
    if (вкладка === 'discussions') {
      // 🛑 ПОРЯДОК ТУТ — ЧАСТИНА ПОВЕДІНКИ, А НЕ СТИЛЬ. Поріг успадковує
      // `chatSeenTs()`, а `markChatSeen()` цю саму позначку зрушує на «зараз».
      // Постав його першим — і поріг щоразу дорівнював би поточній миті, тобто
      // рівно та вада, яку лікуємо. Спіймано стендом: свіже питання ставало
      // «історією» від одного тапу по вкладці.
      markQaFloorOnce();
      markChatSeen();
