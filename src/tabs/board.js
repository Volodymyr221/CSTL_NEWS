...
async function loadBoardData() {
  ...
  // персональні запити нижче:
  //   • `fetchSavedPostIds(uid)` — без `uid` навіть НЕ ВИКЛИКАЄТЬСЯ, і `setSavedIds()`
  //     отримує порожню множину: сердечка згаслі, а таб «Збережені» порожній;
  //   • `fetchAllReactions(uid || getAnonId())` — без `uid` питає базу про АНОНІМА,
  //     тобто мої лайки чесно не знаходяться. Це буквально та скарга Вови 25.08:
  //     «я лайкав цей пост, але лайк мій не висвітлюється».
  // 🔑 І ЩЕ ОДНЕ, НЕОЧЕВИДНЕ: ці ж реакції їдуть у `setDiscussionsData()`, а на них
  // тримається `isLikedByMe()` — тобто «мене теж цікавить» у Питаннях. Без факту
  // «хто я» питання, яке я позначив, перестає бути моєю справою, і підсвітка нової
  // відповіді на нього мовчить. Один рядок тут закриває три поверхні одразу.
