...
import { attachRubberZone } from '../core/rubber-zone.js';
...
//   шапка (щогодини) → капсули → збір → НОВИНИ → стрічка →
//   оголошення (щодня) → автобуси → телефони (довідка).
// ⚠️ 04.08 (вечір) новини й стрічку ПОМІНЯЛИ МІСЦЯМИ на прохання Вови.
...
function renderSkeleton() {
  ...
  el.innerHTML = `
    <!-- ══ ФОТО НА ФОНІ ═══════════════════════════════════════════════════════
         ...
         🔴 ФОРМАТ ФОТО ТУТ — НЕ ДРІБНИЦЯ, А ГОЛОВНА ВИМОГА.
         Спершу стояло olyka.day-1.jpg — 1440×810, тобто ГОРИЗОНТАЛЬНИЙ кадр
         16:9. Екран телефона вертикальний (390×844 CSS = 1170×2532 реальних
         ...
         піксель ставав видимим навіть під розмиттям. Вова: «фото геть не під
         розмір екрану, воно розтягнуте».
         Проміжним кроком ставили day-2 (1167×1347, майже квадрат) — 2.0× і 47%.
         ✅ ЗАРАЗ: olyka-bg.jpg — **941×1672, вертикальне 9:16**, знімок
         справжнього Олицького костелу (дав Вова 04.08). Розтяг 1.5×,
    ...
    <!-- ══ ЗОНА ГУМОВОГО ВІДСКОКУ ═══════════════════════════════════════════
         🔴 10.08 — ОБГОРТКА ЗАВЕДЕНА ПІД ЗАМОВЛЕННЯ ВОВИ, а не «щоб було».
         Дослівно: «гумовий ефект має починатись від скляних міні-віджетів до
         телефонів громади», а бордовий блок привітання з погодою «не
         відривався».
         🔑 Рідна «гумка» iOS зсуває ВЕСЬ вміст прокрутника — виключити з неї
         ...
         🛑 Що НЕ входить сюди — і саме тому не може зрушити:
...
let _знятиГумку = null;   // зняти попередній жест перед повторним initCommunity()
...
export function initCommunity() {
  ...
  // 🔴 Гумовий відскок — ЛИШЕ на зоні після бордової шапки (замовлення Вови
  // 10.08). Чому не рідний відскок і чому це не повторить провал Дошки —
  // повністю в `core/rubber-zone.js`. Вішаємо після рендеру скелета: обидва
  // вузли вже в розмітці, а вміст секцій доїде пізніше й на жест не впливає.
  _знятиГумку?.();
  _знятиГумку = attachRubberZone(document.querySelector('.app-main'),
