...
export function initCommunity() {
  ...
  // 🛑 ВЛАСНИЙ ГУМОВИЙ ВІДСКОК ВИМКНЕНО 10.08 — і це рішення, а не забутий рядок.
  //
  // Зробили (PR #874-875), Вова перевірив на телефоні: «та гумка яку ти зробив
  // вона дьоргається, ти можеш зробити її такою самою плавною і живою,
  // природньою як і в стрічці наприклад чи в автобусах».
  //
  // 🔑 Відповідь — ні, не можу, і причина структурна. У Стрічці й Автобусах
  // гумка РІДНА: її веде композитор в окремому потоці, незалежно від скрипта.
  // Наша рахувалась у JS на кожен `touchmove`, тобто головним потоком. На iOS це
  // не буває таким самим гладким — рівно той висновок, який проєкт уже зробив
  // тричі на шапці Дошки («ТРЕТІЙ ПІДХІД» у `style/board.css`). Я вважав, що мій
  // випадок інший, бо не конкурую з живою прокруткою. Помилився: справа не в
  // конкуренції, а в тому, ХТО малює кадри.
  //
  // ⚠️ Тому НЕ вмикати назад із «оптимізаціями» (rAF, will-change, throttle) —
  // це була б четверта варіація того самого прийому. Правило CLAUDE.md: дві
  // невдачі підряд = пре-мортем і зміна конструкції, а не наступна спроба.
  //
  // ➡️ Єдиний шлях до РІДНОЇ плавності — щоб пружинив сам прокрутник. А він
  // зсуває весь свій вміст, отже бордова шапка мусить бути ПОЗА ним (тобто
  // прибита назавжди, без прокручування вгору). Це зміна дизайну — рішення Вови.
  // Обгортка `#hm-rubber` і модуль лишені саме під цей варіант: там уже рівно та
  // межа, яку Вова назвав («від скляних віджетів»).
  _знятиГумку?.();
  _знятиГумку = null;
