...
// ── 🔴 22.08 — КАПСУЛА ГРОМАДИ ДІЗНАЄТЬСЯ ПРО ВІДПОВІДЬ ОДРАЗУ ──────────────
//
// Замовлення Вови: «неможна зробити так, щоб відразу в капсулі підтягувало?».
// Було: капсула «відповіли під моїм дописом» перечитувалась лише при поверненні
// на Громаду, тож людина, яка стоїть на ній зараз, бачила рядок із запізненням.
//
// 🛑 ДРУГОЇ ПІДПИСКИ НЕ ЗАВОДИМО — і це не економія, а правило. Два канали на ту
// саму подію рано чи пізно розходяться; у проєкті це вже коштувало бага B-27
// (два лічильники коментарів показували різне). Тому подію кидає ТА САМА жива
// підписка, що вже малює коментарі, а капсули просто слухають — рівно як
// `cstl-unread-changed` для повідомлень і `cstl-discussions-data` для питань.
//
// 🔑 ФІЛЬТР ОБОВʼЯЗКОВИЙ. Підписка приносить КОЖЕН коментар у Стрічці — чужий
// під чужим теж. Без фільтра капсули перемальовувались би на кожну репліку в
...
const новийКадр = () => new Promise(r => requestAnimationFrame(() => requestAnimationFrame(r)));
...
export async function openFeedPage(pageId, focusPostId = null) {
  ...
  window.switchTab?.('shotam');
  // 🔴 26.08 — ЧЕКАЄМО, ПОКИ СТРІЧКА СПРАВДІ НАМАЛЮЄТЬСЯ, І ЛИШЕ ТОДІ ВІДКРИВАЄМО ШАР.
  // Скарга Вови: вихід СВАЙПОМ показував спершу Громаду, а потім перемикався на
  // Стрічку — «ніби технічний баг, типу блим». Виходом по стрілочці такого немає.
  // 🔑 Різниця саме в системній анімації iOS: жест «назад» малює ЗНІМОК попереднього
  // запису історії. Запис створює `openLayer` (`history.pushState`), і якщо він
  // ляже в ТОМУ САМОМУ кадрі, що й `switchTab`, браузер ще не встиг перемалювати —
  // у знімок потрапляє Громада. Далі шар зникає, DOM показує Стрічку, і людина
  // бачить два різні екрани поспіль.
  // 🛑 Кнопка «назад» цього не показує, бо там анімацію малюємо МИ (`animateOut`), а
  // не система, і жодного знімка не бере ніхто — тому вада виглядала вибірковою.
  // ⚠️ Два кадри, а не один: перший віддає стилі, другий гарантує, що кадр із новою
  // вкладкою вже вийшов на екран.
  await новийКадр();
  if (!loaded) { await loadData(); renderFeed(); }
...
// 🔴 08.08 — ПІГУЛКА КРІПИТЬСЯ ДО ШАПКИ ЗАСТОСУНКУ, А НЕ ДО РЯДУ СПІЛЬНОТ.
// Було: `top` рахувався від низу `.fd-topbar`. Поки бар був липкий, це працювало —
// він завжди стояв угорі екрана. Тепер бар їде зі сторінкою, тож його низ при
// прокрутці йде в мінус, і пігулка (вона `position: fixed`) поїхала б під шапку
// застосунку або взагалі за екран.
// 🔑 Тепер якір — нижня межа `.app-header`: вона фіксована і не залежить від того,
// куди прокручено сторінку. Тому й перераховувати при скролі більше не треба.
...
function layoutCircles() {
  ...
  if (!el) return;
  // 08.08: глушіння фаз згортання (--sh / --sh-tight) звідси прибрано — самих фаз
  // більше немає, ряд завжди в розгорнутому стані, тож замір і так натуральний.
  el.classList.remove('is-fit');                        // міряємо натуральну ширину
  if (el.scrollWidth <= el.clientWidth + 1) el.classList.add('is-fit');
  planCollapsedPad(el);
}

// 🗑 08.08 — `shrinkProgress()` і константи NAME_RANGE / TIGHT_RANGE ВИДАЛЕНО.
// Вони рахували дві фази згортання липкого топбару. Бар більше не липкий (рішення
// Вови: ряд спільнот їде разом зі сторінкою), тож рахувати нема чого.
// ⚠️ Коментар при них стверджував «винесено окремою чистою функцією, щоб
//    перевірятись тестом без браузера» — такого тесту в репозиторії НЕ БУЛО
//    (перевірено грепом по tests/ перед видаленням). Тобто твердження було
//    застаріле, і функція трималась ні на чому.
...
function commentRowHtml(c, reply = false, replyTo = null) {
  ...
  // 🔴 ЗАПАСНА НАЗВА, А НЕ ЗАПАСНИЙ ЛЮДСЬКИЙ ПІДПИС. Якщо сторінки чомусь немає в
  // памʼяті (не долетів список, чужий пост із deep-link), звичайне `find` віддало б
  // undefined — і рядок ТИХО намалювався б як особистий, тобто назвав би на весь екран
  // саме ту людину, яку вся ця робота навмисно не називає. Тому: є `as_page_id` —
  // голос спільноти, крапка; невідома назва це «Спільнота», а не імʼя автора.
  const asPage = c.as_page_id != null
    ? (pages.find(p => p.id === c.as_page_id) || { name: 'Спільнота', avatar_url: '' })
    : null;
  const nm = asPage
...
function renderCommentSheet() {
  ...
  // 🔴 16.08 — НЕ ПЕРЕМАЛЬОВУЄМО ЛИСТ, ЯКЩО РОЗМІТКА ТА САМА (аудит, P2-7).
  // Ця функція кличеться на КОЖЕН чужий коментар (realtime) і на кожну свою дію.
  // Безумовний `innerHTML` перестворював усі вузли: аватари співрозмовників
  // вантажились наново, і список помітно блимав саме тоді, коли розмова жвава.
  // 🔑 Той самий примітив, що вже прибрав блимання стрічки й Дошки 15.08
  // (`core/list-patch.js`): порівнюємо РЯДОК РОЗМІТКИ, тож однаковий рядок =
  // однакова картинка за визначенням, і DOM не чіпається взагалі.
  // ⚠️ Прокрутку це теж рятує: перестворений список губив місце читання.
  paintIfChanged(listEl, list.length
...
function openComments(postId, focusCommentId = null) {
  ...
  // ── ПІДКАЗКА ОДИН РАЗ НАЗАВЖДИ ────────────────────────────────────────────
  // 🔴 Скарга Вови: «взагалі не зрозуміло що там переключається акаунт».
  // Кільце каже СТАН, значок ⇄ каже ЩО ЦЕ КНОПКА — але ні те, ні те не каже, що
  // кнопка там узагалі є, поки людина її не помітила. Це третя, окрема задача,
  // і вирішує її не графіка, а одне речення в потрібну мить.
  //
  // 🔑 РІВНО ОДИН РАЗ. Підказка, яку показують щоразу, стає шумом і її перестають
  // читати — тоді вона не працює навіть тоді, коли справді потрібна.
  // ⚠️ Ключ ІМЕННИЙ (`uid`): на одному телефоні працюють два акаунти Вови, і
  // безіменний ключ означав би, що другий адміністратор підказки не побачить
  // ніколи. Той самий урок, що з account-scoped state 24.08.
  // 🛑 `try/catch` обовʼязковий: у приватному вікні `localStorage` кидає виняток
  // на самому ДОСТУПІ, і без нього впав би весь лист коментарів через підказку.
  if (мояКоманда && myUid) {
    const ключ = `cstl-voice-hint:${myUid}`;
    let показували = true;
    try { показували = localStorage.getItem(ключ) === '1'; } catch (_) { /* приватне вікно */ }
    if (!показували) {
      // Із затримкою: лист щойно виїхав, і тост поверх анімації читається гірше.
      setTimeout(() => {
        if (!openCommentSheet || openCommentSheet.postId !== postId) return;   // уже закрили
        showToast('Торкніться фото зліва, щоб відповісти від імені спільноти', 5000);
        try { localStorage.setItem(ключ, '1'); } catch (_) {}
      }, 700);
    }
  }
...
// Дочекатись, поки знімки всередині вузла отримають розмір. Порожній список і вже
// завантажені знімки дають готову обіцянку — тобто зайвого кадру не коштують.
// ⚠️ МЕЖА ЧАСУ ОБОВʼЯЗКОВА: мережа може не віддати знімок ніколи, а прокрутка через це
// не сміє не статись. Це той самий клас, що `serviceWorker.ready` — воно не падає,
// воно ВИСНЕ (урок у `HOT_RULES`).
function whenImagesReady(node, межа = 2500) {
  const imgs = [...node.querySelectorAll('img')].filter(i => !i.complete || !i.naturalHeight);
  if (!imgs.length) return Promise.resolve();
  return new Promise(resolve => {
    let лишилось = imgs.length;
    let готово = false;
    const крок = () => {
      if (готово) return;
      if (--лишилось > 0) return;
      готово = true; resolve();
    };
    imgs.forEach(i => {
      i.addEventListener('load', крок, { once: true });
      // Помилка знімка — теж відповідь: картка більше не виросте, вести можна одразу.
      i.addEventListener('error', крок, { once: true });
    });
    setTimeout(() => { if (!готово) { готово = true; resolve(); } }, межа);
  });
}
...
function focusInsideScreen(screen, postId) {
  ...
  const step = () => {
    ...
    if (el) {
      // 🔴 26.08 — ВЕРХ ДОПИСУ БІЛЬШЕ НЕ ХОВАЄТЬСЯ ПІД ЛИПКУ ШАПКУ.
      // Скарга Вови зі знімка: у спільноті з багатьма дописами наведення обрізало
      // допису шапку (аватар · назва · дата) — вона опинялась ПІД `.fd-screen-title`,
      // який тут `position: sticky; top: 0`.
      // 🔑 Причина — `block: 'center'`: він ставить у центр вікна САМ ЕЛЕМЕНТ, і для
      // високого допису (знімок на всю ширину + текст) верх виїжджає вище видимої
      // зони. Липка шапка при цьому не «накриває» — вона просто там і стоїть.
      // ✅ Тому `block: 'start'` плюс `scroll-margin-top` на висоту шапки: браузер
      // сам віднімає цей відступ, і допис стає рівно ПІД нею, з видимим заголовком.
      // ⚠️ Висоту МІРЯЄМО, а не вписуємо числом: шапка стискається при прокрутці
      // (`--p` у `feed.css`), і будь-яке зафіксоване число розійшлося б із живим.
      // 🔴 26.08, ДРУГА ПРИЧИНА ТІЄЇ САМОЇ ВАДИ — І ГОЛОВНА.
      // Прокрутка рахується від ПОТОЧНИХ висот, а знімок допису на той момент ще
      // не завантажений: місця він займає мало, картка низька. Коли знімок приходить,
      // картка виростає на кілька сотень пікселів і ПІДШТОВХУЄ все, що нижче, — те,
      // що ми старанно поставили під шапку, виїжджає під неї вгору.
      // 🔑 У стенді цього не видно ніколи: там знімок або локальний, або `data:`, тобто
      // готовий у той самий кадр. На живому телефоні він іде з мережі — і саме тому
      // вада була видна Вові й невидна всім моїм замірам.
      // ✅ Тому чекаємо знімки САМЕ ЦІЄЇ картки, і лише тоді ведемо до неї.
      const прокрутити = () => {
        // Висоту шапки міряємо в останній момент: вона стискається при прокрутці
        // (`--p` у `feed.css`), і взяте наперед число розійшлося б із живим.
        const шапка = screen.querySelector('.fd-screen-title');
        const h = шапка ? Math.round(шапка.getBoundingClientRect().height) : 0;
        // 📐 26.08 — ПРОСВІТ 22px, І ЦЕ НЕ ЧИСЛО «НА ОКО».
        // 🗣️ Вова по знімку: «хай трошки нижче, щоб було видно верхній край, але щоб не
        // було видно наступного поста — чуть-чуть буквально».
        // 🔑 22 = зазор між картками (8px) + радіус заокруглення картки (14px). При
        // колишніх 12px верхній край різався рівно на заокругленні, і картка читалась
        // «прилиплою» до шапки; тепер заокруглення видно повністю.
        // ⚠️ ВЕРХНЯ МЕЖА ТУТ ЖОРСТКА, і саме вона тримає другу половину прохання: просвіт
        // більший за зазор ПОКАЗУЄ низ попередньої картки. Заміряно: зазор 8px, тож усе,
        // що більше, — це вже чужа картка в кадрі. 22px лишає видимою смужку без вмісту
        // (низ попередньої — порожнє поле картки), і лише коли попередня взагалі є:
        // з віджета ведемо на НАЙСВІЖІШИЙ допис, а він у списку спільноти перший.
        // 🛑 Побільшуєш це число — спершу зміряй зазор ще раз: він задається в `feed.css`
        // і від його зміни залежить, з якого моменту в кадр лізе сусідня картка.
        el.style.scrollMarginTop = (h + 22) + 'px';
        el.scrollIntoView({ behavior: 'smooth', block: 'start' });
        el.classList.add('fd-card--flash');
        setTimeout(() => el.classList.remove('fd-card--flash'), 1600);
      };
      whenImagesReady(el, 2500).then(прокрутити);
      return;
    }
...
function openComposer(pageId, editPost = null) {
  ...
  sendBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // 🔴 17.09 — ВІДЖЕТ СТРІЧКИ НА ГРОМАДІ ДІЗНАЄТЬСЯ ПРО НОВИЙ ДОПИС.
        // 🗣️ Скарга Вови: «коли я новий пост викладаю, він мене відразу вибиває в
        // стрічці, що я його опублікував. Але віджет стрічки в громаді не оновлюється».
        // 🔬 Так і було: `renderFeed()` нижче перемальовує САМУ Стрічку, а віджет на
        // Громаді живе в іншому модулі (`home-feed.js`) і свій список читає лише при
        // побудові вкладки. Тобто до перезапуску застосунку він показував стрічку
        // БЕЗ щойно опублікованого допису — свого ж, щойно написаного.
        // 🔑 Подія, а не прямий виклик: композер не має знати, ХТО показує стрічку.
        // Поруч уже живе той самий патерн для подій (`cstl-events-changed`).
        window.dispatchEvent(new CustomEvent('cstl-feed-changed', {
          detail: { post_id: res.post?.id, edit: !!edit },
        }));
  ...
  // 🔴 04.09 — КЛАВІАТУРУ ВІДКРИВАЄ БУДЬ-ЯКЕ ПОЛЕ АРКУША, А НЕ ЛИШЕ ТЕКСТ ДОПИСУ.
  //
  // 🗣️ Скарга Вови зі знімка (композер події): «коли відкривається клавіатура,
  // модалка з подією не підтягується… я натисну вибрати місце проведення, і воно
  // не підтягується, клавіатура перекриває це все».
  //
  // 🔑 ПРИЧИНА, І ВОНА НЕ У ФОРМУЛІ ВИСОТИ. Гейт «клавіатура відкрита» тримався
  // на ОДНОМУ полі — тексті допису (`input: textEl`). Фокус у даті, часі чи місці
  // модуль не бачив узагалі: аркуш не стискався, і поля лишались під клавіатурою.
  //
  // ✅ Лікує `multiFieldFocus` — адаптер, який У ПРОЄКТІ ВЖЕ БУВ (його використовує
  // `setupSheetShell` для редактора сторінки й команди). Композер кличе
  // `attachKeyboardSheet` напряму, тож адаптер до нього просто не доїхав.
  // 🛑 САМ `core/keyboard.js` НЕ ЗМІНЕНО ЖОДНИМ СИМВОЛОМ — це зона, де два фікси
  // вже провалились (HOT_RULES №9 і №10), і лізти в її формулу висоти не було
  // потреби: зламаний був гейт у СПОЖИВАЧІ.
  // ⚠️ Перехід між полями не рахується за закриття — адаптер сам звіряє
  // `relatedTarget` (клавіатура при цьому нікуди не зникає, і смикати аркуш
  // униз-угору було б тим самим «дьорганням», від якого страждав лист коментарів).
  const compFocus = multiFieldFocus(compSheet);
  compFocus.addEventListener('focus', kbAnim);
  compFocus.addEventListener('blur', () => { if (!closing) kbAnim(); });
  detachKb = attachKeyboardSheet(back.querySelector('.fd-sheet-vp'), compSheet, {
    input: compFocus, minHeight: 220,
    kbClass: 'fd-comp--kb', overlayClass: 'fd-sheet-vp--kb',
    expandTop: readTopGap(compSheet),
  });

  // 🔴 04.09 — ПОЛЕ, В ЯКЕ ЛЮДИНА ТИЦЬНУЛА, ПІДТЯГУЄТЬСЯ У ВИДИМУ ЗОНУ.
  //
  // 🗣️ Скарга Вови (композер події): «я натисну вибрати місце проведення, і воно
  // не підтягується, клавіатура перекриває це все».
  //
  // 🔑 ЦЕ ДРУГА ПОЛОВИНА ФІКСУ, і без неї перша нічого не варта. Аркуш тепер
  // стискається на фокус у БУДЬ-ЯКОМУ полі (гейт вище отримав `multiFieldFocus`
  // замість одного `textEl`) — але поля дати, часу й місця лежать НИЖЧЕ в тілі,
  // тож саме стиснення їх не показує: воно лише звільняє місце.
  // ⚠️ Скролер тут — `.fd-comp-body`, а не сам аркуш: три яруси композера
  // (нерухома шапка · тіло, що скролиться · кнопка дії) саме для цього й зроблені.
  //
  // 🛑 `revealInScroller` рухає список ЛИШЕ якщо поле справді не видно, і мовчить
  // при розбіжності менш як 1px — зайвий рух гірший за його відсутність.
  // 🔑 Чому на `focusin`, а не на `click`: фокус приходить і з клавіатурного
  // переходу між полями, і з тапу; клік ловив би лише друге.
  // ⚠️ Затримка 300мс: доти клавіатура ще виїжджає, аркуш не встиг стиснутись, і
  // будь-який вимір «чи видно поле» відповідав би про СТАРУ розкладку — тобто ми
  // підтягли б поле не туди. Той самий урок, що в шапці `keyboard.js`: не міряти,
  // поки розкладка ще їде.
  // ⚠️ `bodyEl` уже оголошений вище (рядок ~3551) — беремо його, а не заводимо
  // другий покажчик на той самий вузол.
  compSheet.addEventListener('focusin', (e) => {
    const поле = e.target.closest('input, textarea, select');
    if (!поле || !bodyEl || !bodyEl.contains(поле)) return;
    setTimeout(() => revealInScroller(bodyEl, поле, 16), 300);
  });
...
export async function initFeed() {
  ...
  if (root && !root.dataset.fdWired) {
    ...
    // писав `--sh` / `--sh-tight` і стискав липкий бар. Замовлення Вови: ряд
    // спільнот тепер просто їде разом зі сторінкою, як в Instagram, — тобто
    // стискати нема чого, і слухач скролу на `.app-main` більше не потрібен.
    // ⚠️ Пігулку «нові публікації» перепозиціонувати при скролі ТЕЖ не треба: вона
    //    більше не прив'язана до низу бару (див. `positionNewPostsPill`).
    positionNewPostsPill();
