...
import { focusFeedPost, openEventsScreen } from './feed.js';   // 🗓 тап по події → сама подія; «Усі →» → афіша
...
export async function renderWeatherBlock() {
  ...
  try {
    ...
    //
    // 🔴 УМОВА ТУТ ЖОРСТКА, І ЦЕ ВИПРАВЛЕННЯ ВЛАСНОЇ ВАДИ, ЗРОБЛЕНОЇ ГОДИНУ
    // ТОМУ. Перша версія писала при `!picked` — тобто щоразу, коли місто не
    // обране руками. Але за відмови в дозволі (чи за таймаутом) `getCoords()`
    // віддає ЗАПАСНУ Олику з полем `city: 'Олика'`, і ця підстановка лягала б
    // у сховище як «людина визначено тут». Наслідок: кожен, хто не дав дозволу,
    // назавжди прикипав би до Олики — причому сильніше за власну анкету.
    // Це рівно та вада, яку я лікував учора («запасні координати — не місце
    // людини»), відтворена в новому місці.
    //
    // ➡️ Тому пишемо ЛИШЕ коли позиція справжня (`geo.city == null` — так
    // `getCoords` позначає живу відповідь GPS) і назву дало саме зворотне
    // геокодування, а не запасний рядок.
    const відНавігатора = !picked && geo && geo.city == null;
    const назваЗГеокодера = відНавігатора && (await cityP);   // null, якщо не встиг
    if (назваЗГеокодера) rememberDetectedPlace(назваЗГеокодера);
...
function attachWeatherFold(root) {
  ...
  function setOpen(next) {
    ...
    // ⚠️ Спостерігач вмикається ДО гілки «зменшити рух»: вона виходить раніше, і
    // при системному налаштуванні автозгортання не працювало б узагалі — тобто
    // фіча мовчки зникала б саме в тих, хто просив менше руху на екрані.
    стежити(open);
  ...
  // ── АВТОЗГОРТАННЯ, КОЛИ ВІДЖЕТ ПІШОВ ЗА КРАЙ ЕКРАНА (20.08) ───────────────
  ...
  //
  // 🔑 ТРИ РІШЕННЯ, І КОЖНЕ ВІДПОВІДАЄ НА СВОЮ ЧАСТИНУ ВИМОГИ.
  //
  // 1. `IntersectionObserver`, а не слухач `scroll`. Слухач працює на КОЖЕН
  //    кадр прокрутки і рахує геометрію — це і є те «дьоргання». Спостерігач
  //    будить нас рівно один раз, коли межу перетнули.
  //
  // 2. Згортаємо МИТТЄВО, без анімації. Анімувати те, чого не видно, означає
  //    змушувати сторінку рухатись під пальцем усі 300мс. Один кадр замість
  //    тридцяти — і рухається він поза екраном.
  //
  // 3. 🛑 ГОЛОВНЕ: КОМПЕНСУЄМО ПРОКРУТКУ. Віджет стоїть ВИЩЕ того місця, куди
  //    людина дивиться, тож зникнення його висоти підтягнуло б увесь вміст
  //    угору — саме той стрибок, якого Вова просив уникнути. Віднімаємо рівно
  //    ту висоту, що зникла, у ТОМУ САМОМУ кадрі: око не бачить руху взагалі.
  //
  // ⚠️ ЧЕСНА МЕЖА: якщо прокрутки менше за висоту панелі, компенсувати НЕМА ЧИМ
  // — `scrollTop` упреться в нуль і сторінка стрибне попри всі старання. У
  // такому разі просто не згортаємо. Той самий урок уже записаний у
  // `collapseNode` (`core/list-patch.js`): «коли контенту над людиною меншає,
  // якір безсилий за БУДЬ-ЯКОЇ реалізації».
  //
  // 📐 Запас 64px — це «трохи більше ніж треба» зі скарги: поки віджет лише
  // торкнувся краю, він ще ваш; згортаємо, коли пішов помітно далі.
  const ЗАПАС_PX = 64;
  let io = null;
  let зняти = null;   // прибрати слухачі спокою разом зі спостерігачем

  // Перший видимий блок НИЖЧЕ віджета — за ним і звіряємо, чи не поїхав екран.
  // Прихованих пропускаємо: у `display: none` рамка нульова, і «зсув» вийшов би
  // вигаданий (та сама пастка, що зі згортанням тексту в стрічці 20.08).
  function якір() {
    let n = (root.closest('header') || root).nextElementSibling;
    while (n) {
  ...
  // 🔴 20.08, ДРУГИЙ ЗАХІД — І ЦЕ БУВ МІЙ БАГ, А НЕ ДОРОБКА.
  // Скарга Вови: «коли віджет ховається, сторінка ЗУПИНЯЄТЬСЯ; тільки тоді як
  // зупинилась можна скролити далі».
  //
  // 🔑 Причина не в тому, ЩО я рахував, а в тому, КОЛИ я це робив. Будь-який
  // запис у `scrollTop` під час інерційної прокрутки на iOS обриває інерцію:
  // палець уже відпущений, сторінка летить — і мій рядок компенсації гасить
  // політ на місці. Тобто сторінку зупиняв не браузер, а моє ж «плавне»
  // згортання. Жодна точність компенсації цього не лікує: сам факт дотику до
  // `scrollTop` під час польоту і є вада.
  //
  // ➡️ Тому згортаємо не «щойно перетнули межу», а КОЛИ ПРОКРУТКА ВЩУХЛА. Це
  // нічого не коштує оку: віджет на той момент давно за краєм екрана, і людина
  // просто не бачить, у яку саме мить він склався. Зате політ пальця більше
  // ніхто не чіпає.
  //
  // ⚠️ Спостерігач лишається `IntersectionObserver` — він і далі будить нас один
  // раз на перетині межі, а не щокадру. Доданий слухач `scroll` — ПАСИВНИЙ і без
  // геометрії: він лише перезапускає таймер спокою, тож тим «дьорганням», проти
  // якого писався спостерігач, не стає.
  const СПОКІЙ_МС = 140;      // стільки без подій прокрутки = політ скінчився
  let ждун = null;
  let палецьНаЕкрані = false;

  function колиСпокійно(скролер, дія) {
    if (ждун) clearTimeout(ждун);
    const тик = () => {
      // Палець ще на склі — людина тримає сторінку, це не спокій. Чекаємо далі:
      // згорнути під притиснутим пальцем означало б смикнути її саме тоді, коли
      // вона керує рухом.
      if (палецьНаЕкрані) { ждун = setTimeout(тик, СПОКІЙ_МС); return; }
      ждун = null;
      дія();
    };
    ждун = setTimeout(тик, СПОКІЙ_МС);
  }

  function автозгорнути() {
    const скролер = root.closest('.app-main') || document.querySelector('.app-main');
    const висота = panel.getBoundingClientRect().height;
    if (!скролер || !висота) return;
    if (скролер.scrollTop < висота) return;   // компенсувати нічим — краще лишити як є

    // 🛑 Перевіряємо ще раз УЖЕ ПІСЛЯ спокою, а не лише на перетині межі: за час
    // польоту людина могла повернутись угору, і згортати те, на що вона зараз
    // дивиться, — це рівно той стрибок, від якого лікуємось.
    if (root.getBoundingClientRect().bottom >= 0) return;

    // 🔴 ЧОМУ ЗВІРЯЄМОСЬ ІЗ ЯКОРЕМ, А НЕ ВІДНІМАЄМО ВИСОТУ НАОСЛІП.
    // Перша версія робила `scrollTop -= висота`. Контрольний прогін показав, що
    // Chrome МАЄ власне scroll anchoring: він компенсує зникнення висоти сам, і
    // тоді моє віднімання стало б ДРУГОЮ компенсацією — екран поїхав би на ту
    // саму висоту, тільки в інший бік. А Safari на iPhone scroll anchoring НЕ
    // підтримує, тобто без компенсації стрибок буде саме там, де на нього
    // скаржаться. Обидва випадки лікує один прийом: дивимось, де якір був і де
    // опинився, і рухаємо рівно на РІЗНИЦЮ. Якщо браузер упорався сам, різниця
    // нульова і ми не робимо нічого.
    const я = якір();
    const було = я ? я.getBoundingClientRect().top : null;

    open = false;
    root.classList.remove('hm-wx--open');
    toggle.setAttribute('aria-expanded', 'false');
    toggle.setAttribute('aria-label', 'Показати прогноз на 7 днів');
    if (timer) { clearTimeout(timer); timer = null; }
    panel.style.height = '0px';               // миттєво: цього кадру вже не видно

    if (я && було != null) {
      const стало = я.getBoundingClientRect().top;
      const різниця = стало - було;
      if (різниця) скролер.scrollTop += різниця;
    }
    стежити(false);
  }

  function стежити(треба) {
    if (!треба) { io?.disconnect(); io = null; зняти?.(); зняти = null; return; }
    if (io || !('IntersectionObserver' in window)) return;
    const скролер = root.closest('.app-main') || document.querySelector('.app-main');
    io = new IntersectionObserver(([en]) => {
      // Стежимо ЛИШЕ за виходом угору. Якщо віджет не видно тому, що людина
      // ще не догорнула до нього знизу, згортати його безглуздо.
      if (!en.isIntersecting && en.boundingClientRect.bottom < 0) {
        колиСпокійно(скролер || document.scrollingElement, автозгорнути);
      }
    }, { root: скролер || null, rootMargin: `${ЗАПАС_PX}px 0px 0px 0px`, threshold: 0 });
    io.observe(root);

    // Кожна подія прокрутки відсуває мить згортання: поки сторінка рухається,
    // ми до неї не торкаємось. Пасивний слухач без обчислень — саме тому він і
    // не суперечить причині, з якої тут стоїть спостерігач.
    const відсунути = () => { if (ждун) колиСпокійно(скролер || document.scrollingElement, автозгорнути); };
    (скролер || window).addEventListener('scroll', відсунути, { passive: true });
    const вниз = () => { палецьНаЕкрані = true; };
    const вгору = () => { палецьНаЕкрані = false; };
    window.addEventListener('touchstart', вниз, { passive: true });
    window.addEventListener('touchend', вгору, { passive: true });
    window.addEventListener('touchcancel', вгору, { passive: true });
    зняти = () => {
      (скролер || window).removeEventListener('scroll', відсунути);
      window.removeEventListener('touchstart', вниз);
      window.removeEventListener('touchend', вгору);
      window.removeEventListener('touchcancel', вгору);
      if (ждун) { clearTimeout(ждун); ждун = null; }
    };
  }
...
// Українська плюралізація (1 день, 2 дні, 5 днів). ⚠️ Була «копією з events.js» —
// того модуля більше немає (видалений 04.09), тож копія стала оригіналом.
...
export async function renderEventBlock({ force = false } = {}) {
  ...
  try {
    ...
    // 🔴 04.09 (третій захід) — «УСІ →» ВЕДЕ В АФІШУ, А НЕ В ЗАГАЛЬНУ СТРІЧКУ.
    // 🗣️ Вова: «продумай логіку віджетів подій та віджетів стрічки… ти сам не
    // розумієш, для чого ти це робиш».
    // 🔑 Віджет — це ЗРІЗ (три найближчі), і кожен зріз на Громаді веде у своє
    // повне місце: новини → новини, оголошення → Дошка. У подій такого місця не
    // існувало: кнопка кидала в Стрічку, де подія лежить серед дописів за датою
    // ПУБЛІКАЦІЇ, тобто відповіді «що буде в громаді» там немає в принципі.
    const усі = document.getElementById('hm-events-all');
    if (усі && !усі.dataset.wired) {
      усі.dataset.wired = '1';
      усі.addEventListener('click', () => openEventsScreen());
    }
...
// 🆕 26.08: 7 → 10 секунд (потік /byyou, крок 2). Не «щоб було спокійніше» —
// картка стала вищою і в ній зʼявився анонс, тобто тексту на сторінці більше в
// півтора раза, а часу на нього лишалось стільки ж.
// ⚠️ ЧОМУ КАРУСЕЛЬ ВЗАГАЛІ ЛИШИЛАСЬ. Два зовнішні помічники радили прибрати
// авто-гортання («для новин це поганий UX»). Рішення Вови 26.08 — лишити, і
// підстава в тому, чого поради не бачили: рух тут уже вимикається сам —
// `document.hidden`, поза екраном, `prefers-reduced-motion` і ДОТИК ПАЛЬЦЕМ
// (див. `core/auto-carousel.js`). Тобто самочинного руху «під рукою», проти
// якого й заперечують, у нас не буває.
const NEWS_CYCLE_MS = 10000;
...
// 🔴 01.09 — БЕЙДЖ НАЗИВАЄ СВІЙ РОЗДІЛ. Скарга Вови: «вчора прочитав 8 нових
// статей, сьогодні зранку заходжу, нові не зʼявились, а пише так само 8 нових,
// це фігня якась а не лічильник».
//
// 🔬 ВІДТВОРЕНО ПРИЛАДОМ `tests/tools/news-badge-geo-probe.mjs` — і виявилось,
// що памʼять лічильника ЦІЛА (мітка переживає перезахід), а вада інша:
//   • число рахує ЛИШЕ «Громаду» (`countNewCommunity` → `NEWS_GEO_GROUPS[0]`),
//     і це свідоме звуження: Громада дає ~0.26 статті на день, Волинь ~24,
//     «Україна та Світ» ~30 — рахувало б усе, писало б «54 нових» щоранку;
//   • але віджет це КАРУСЕЛЬ, і підпис «НОВИНИ · ___» гортається разом із
//     картками, а бейдж — ні.
// ➡️ Людина стоїть на ВОЛИНІ, бачить «8 нових», читає ті вісім статей Волині —
// і число не рухається, бо воно ніколи й не було про Волинь. Заміряно:
// читання на Громаді дає 20 → 17, читання на Волині 17 → 17.
//
// 🛑 ЧОМУ НЕ «ХОВАТИ БЕЙДЖ ПОЗА ГРОМАДОЮ» — перше, що напрошується.
// Карусель гортається САМА (`startAutoCarousel`, `NEWS_CYCLE_MS`), тож бейдж
// зʼявлявся б і зникав без жодної дії людини. Це рівно той клас автоматичного
// руху, який прибирали з головної аудитом 03.08 («сторінка ніколи не стоїть на
// місці»). Мовчазне блимання гірше за неточний підпис.
//
// ✅ Тому число носить свій розділ ІЗ СОБОЮ. Текст статичний, не залежить від
// того, яка сторінка зараз у вікні, і не бреше на жодній із них.
// 🔑 Клас вади той самий, що в B-27: число, яке стосується не того, що
// підписано поруч. Розбір — `docs/ALGORITHMS.md`, розділ «Е».
function paintNewsBadge(arts) {
  ...
  // 📐 ЧОМУ «20 У ГРОМАДІ», А НЕ «20 НОВИХ У ГРОМАДІ» — заміряно, не на око.
  // Повна форма дає 119px, і шапка на 390px розсипається на два рядки:
  // сума 323px при доступних 322, висота 28px → 40px. Коротка форма — 83px,
  // сума 298px, висота лишається 28px.
  // 🔑 Слово «нових» не втрачене: воно живе в `aria-label`, тобто читач екрана
  // дістає повне формулювання, а на екрані стоїть те, що влазить. Той самий
  // прийом, що вже застосований у капсулах (мітка коротшає, `aria` — ні).
  const html = `<span class="cm-news-new" aria-label="${n} ${pluralNew(n)} у розділі Громада">${n} у Громаді</span>`;
