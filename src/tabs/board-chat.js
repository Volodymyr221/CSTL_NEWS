...
// 🆕 03.08 — те саме число, що вже лежить у `_unreadChats`, віддане назовні для
// капсули «ЗАРАЗ» на головній. Свідомо ГЕТЕР, а не власний запит: другий
// підрахунок непрочитаного став би другим джерелом правди і колись розійшовся б
// із бейджем — у проєкті це вже ставалось (дві копії списків антиспаму,
// два різні лічильники коментарів у B-27). Мережі тут немає: головна малює те,
// що порахував `refreshUnreadBadge()` на своїх подіях.
export function unreadChatsCached() { return isLoggedIn() ? _unreadChats : 0; }
...
export async function refreshUnreadBadge() {
  ...
  paintUnreadBadge();
  // 🆕 03.08 — головна теж показує непрочитане (капсула у смузі «ЗАРАЗ»), а
  // дізнатись про перерахунок їй нізвідки: `_unreadChats` живе тут. Подія, а не
  // прямий виклик, — щоб board-chat не знав про існування головної (інакше два
  // модулі почали б імпортувати один одного).
