...
export async function openThreadById(threadId) {
  ...
  // ⚠️ ЧЕСНО ПРО ПЛАВНІСТЬ: Вова просив «без блимання і ривків», але приписати це
  // перемиканню вкладки НЕ МОЖНА — заміряно `tests/tools/notif-exit-smooth.mjs`,
  // і стара версія теж давала **0 кадрів без вмісту**, бо перемикання відбувалось
  // під заставкою і на екран не потрапляло. Тут змінено МІСЦЕ ПРИЗЕМЛЕННЯ, а не
  // плавність; плавність була ціла і лишається такою (сторож її тепер стереже).
