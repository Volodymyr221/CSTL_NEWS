...
import {
  currentUserId, currentUserName, isLoggedIn, requireAuth, getProfile, onAuthChange, authReady,
} from '../core/auth.js';
import {
...
export async function refreshUnreadBadge() {
  // 🔴 25.08 — ЧЕКАЄМО ФАКТ «ХТО Я» ДО ПЕРШОГО ПОКАЗУ (беклог, пункт 0).
  //
  // `initBoardChat()` кличе цю функцію ОДРАЗУ при старті, а `initAuth()` в `app.js`
  // йде без `await`. Тому на холодному старті `isLoggedIn()` тут ще `false` — і
  // гілка нижче чесно малює вигляд ГОСТЯ: бейджа немає, крапки на вкладці немає,
  // капсула «ПОВІДОМЛЕННЯ» на Громаді мовчить. Людина з непрочитаним листуванням
  // відкриває застосунок і не бачить жодної ознаки, що їй писали.
  //
  // 🔑 Це та сама вада, що лікувалась у Стрічці (PR #1020), тільки симптом інший:
  // там не підсвічувався лайк, тут не горить крапка. Причина одна — перегони.
  //
  // ⚠️ Ставити тут `if (!isLoggedIn()) return` НЕ МОЖНА: функція кличеться ще й на
  // повернення у вкладку та на вихід з акаунта, і тоді нуль — правильна відповідь,
  // яку треба намалювати. Тому чекаємо ФАКТ, а рішення лишаємо тому самому рядку.
  // ⏱ Після першого разу очікування коштує нуль: гарантія памʼятає, що вже
  // відповіла, і має власну межу часу — підвиснути тут не можна за побудовою.
  await authReady();
  if (!isLoggedIn()) {
...
export async function openThreadById(threadId) {
  ...
  // 🔴 25.08 — ГАРАНТІЯ ЗАМІСТЬ ПЕРЕГОНІВ (беклог, пункт 0).
  //
  // Саме тут описаний вище розрив («хеш зʼїдається на ~465мс, вхід зʼявляється на
  // ~500мс») закривається В КОРЕНІ, а не наслідком: чекаємо відповідь на «хто я»,
  // і рішення нижче ухвалюється вже за фактом, а не за тим, хто встиг першим.
  //
  // 🛑 ВІДКЛАДЕНИЙ НАМІР НИЖЧЕ ЛИШАЄТЬСЯ І НЕ ЗАЙВИЙ. Він покриває інший випадок:
  // людина справді була гостем, тапнула сповіщення і ввійшла руками — гарантія про
  // це нічого не знає, бо на момент очікування відповідь «гість» була ЧЕСНА.
  // Тобто гарантія прибирає перегони, намір доводить справжній вхід — різні речі.
  await authReady();
  if (!isLoggedIn()) {
  ...
  // 🔴 15.08, ТРЕТЯ СКАРГА ВОВИ (два знімки): «відкриває тільки цю сторінку, і коли
  // я свайпом виходжу назад — вибиває сторінку завантаження і пусту сторінку… а має
  // перекидати мене назад в усі повідомлення, і звідти в застосунок, у Дошку».
  //
  // Заміряно `tests/tools/notif-backstack-probe.mjs`: після тапу під чатом стояла
  // ЗАСТАВКА, вкладка лишалась `community`, а «назад» вело з застосунку геть —
  // бо запис в історії був РІВНО ОДИН (сам чат).
  //
  // Лікуємо двома кроками:
  //   1) чекаємо заставку — інакше «назад» показує саме її («сторінка завантаження»);
  //   2) під розмову кладемо «Повідомлення» — це і є другий запис історії.
  //
  // 🔴 15.08, УТОЧНЕННЯ ВОВИ: шлях назад має бути **чат → Повідомлення → Громада**.
  // Тому вкладку ТУТ БІЛЬШЕ НЕ ПЕРЕМИКАЄМО ВЗАГАЛІ (був `switchTab('board')`).
  // На холодному старті вкладка й так `community` (Громада), тож не робити нічого —
  // це і є потрібна поведінка.
  // 🔑 Заразом лікується друге: при ВЖЕ ВІДКРИТОМУ застосунку перемикання
  // ВИКРАДАЛО людину з тієї вкладки, де вона стояла, — закривши чат, вона
  // поверталась не туди, звідки прийшла.
  // 🔴 І ТРЕТЄ, ЗАМІРЯНЕ: перемикання давало ВИДИМУ ПЛЯМУ, а не лише не те місце.
  // `.app-main` має `transition: background-color 0.3s`, а фони вкладок різні
  // (Громада `#1A0A0E` майже чорна ↔ Дошка `#E6E6E3` світла). Перемикання
  // спрацьовує ПІСЛЯ `whenSplashGone()`, тобто рівно тоді, коли заставка вже
  // зійшла — і тло повзло з темного у світле в людини на очах.
  // 📐 `tests/tools/notif-journey-frames.mjs`, стара версія: **18 значень тла**
  // за один прохід (`rgb(26,10,14)` → … → `rgb(230,230,227)`); тепер **1**.
  // 🛑 УРОК ПРО САМУ МІРКУ: перший прилад (`notif-exit-smooth`) показав тут НУЛЬ і
  // «спростував» цю гіпотезу — бо починав знімати кадри ВЖЕ ПІСЛЯ заставки, коли
  // перетікання закінчилось. Тобто прилад був чесний, а вікно спостереження —
  // замале. Міряючи плавність, починай знімати ДО події, а не після.
  await whenSplashGone();
  // ⚠️ Список кладемо, ЛИШЕ якщо жодного екрана чатів ще немає. Інакше тап по
  // сповіщенню з уже відкритих «Повідомлень» поклав би другий такий самий список.
  if (!document.querySelector('.pm-screen')) openThreadsList();
