...
// («вузла .hm-cap2 немає»), і це була справжня вада, а не проблема сцени:
// перша версія робила `await getCoords()` ПЕРЕД побудовою капсул. Поки людина
// не відповіла на запит дозволу — а вона може не відповісти взагалі — уся смуга
// не малювалась. Тобто заради уточнення одного слова («Метельне» замість
// «Олика») ми ховали ВСІ капсули, включно з тими, що з геолокацією не пов'язані.
// 🔑 Правило, яке з цього лишається: **уточнення не має права затримувати
// показ.** Чекаємо коротко; не встигло — малюємо з анкети, а координати, коли
// приїдуть, лягають у зліпок і працюють із наступного перемальовування (вхід на
// вкладку, повернення в застосунок). Людина бачить смугу одразу і в найгіршому
// разі один раз — зі своїм селом з анкети.
...
// 🔴 21.08 — «ДАЛЕКО» І «НЕВІДОМО» — РІЗНІ СТАНИ, і плутати їх не можна.
// Порожній `_гдеЯ` раніше означав обидва: і «координати є, але я поза громадою»,
// і «дозволу немає». Для капсули це протилежні випадки: у першому я точно НЕ
// можу сісти на автобус зі свого села (я за 35 км), у другому — цілком можу.
let _далеко = false;

async function поточнеСело() {
  if (_гдеЯ !== null) return _гдеЯ || null;
  if (!_гдеЯПошук) _гдеЯПошук = обчислитиСело();
  // Гонка з коротким таймером: перемагає той, хто швидший. Пошук при цьому НЕ
  // скасовується — він домалює зліпок сам, просто вже без нас.
...
async function обчислитиСело() {
  try {
    ...
    if (c && c.city == null) {                 // справжня позиція, не запасна
      _гдеЯ = (await nearestSettlement(c)) || '';
      _далеко = !_гдеЯ;                        // координати є, а села громади поруч немає
    } else { _гдеЯ = ''; }
  } catch { _гдеЯ = ''; }
  return _гдеЯ || null;
}
...
async function nowCapsule() {
  ...
  // ── 🔴 Я НЕ ВДОМА: КОЛИ АВТОБУС ДОДОМУ (21.08) ───────────────────────────
  //
  // Скарга Вови зі знімка: капсула показувала «Олика → Жорнище», коли він
  // фізично був у Луцьку. І це не дрібниця: на той автобус він не міг сісти
  // ніяк — між ним і зупинкою 35 км.
  //
  // 🔑 Перебування ПОЗА громадою — це не «немає інформації», а сильна
  // інформація: людина поїхала і захоче повернутись. Тому питання міняється з
  // «коли наступний автобус із мого села» на «коли я зможу дістатись додому».
  //
  // 🛑 І ТУТ ГОЛОВНЕ — НЕ ВИГАДУВАТИ, ДЕ САМЕ ЛЮДИНА. Спокуса була: взяти назву
  // міста з погоди (`wx_place_v1`) і написати «Луцьк → Олика». Але той запис
  // ставиться і РУЧНИМ вибором міста в погоді — тобто людина, яка глянула
  // погоду Луцька сидячи в Олиці, отримала б капсулу про подорож, якої немає.
  // ➡️ Тому початкову зупинку беремо з РОЗКЛАДУ (звідки цей рейс іде), а не з
  // здогадки про місце людини. Якщо вона в Луцьку — напис точний; якщо деінде —
  // він однаково відповідає на «коли можна дістатись додому» і ні про що не бреше.
  if (_далеко) {
    const дім = await mySettlement();
    if (дім) {
      let додому = null;
      for (const r of live) {
        const stop = findStopOnRoute(r, дім);
        if (!stop) continue;
        const m = getStopMins(r, stop.name);
        if (m == null || m - nowMin < 0) continue;
        if (!додому || m < додому.m) додому = { r, m };
      }
      if (додому) {
        const хв = ((додому.m % 1440) + 1440) % 1440;
        const час = `${String(Math.floor(хв / 60)).padStart(2, '0')}:${String(хв % 60).padStart(2, '0')}`;
        const звідки = додому.r.stops?.[0]?.name || '';
        const лишилось = додому.m - nowMin;
        return {
          key: 'now', icon: ICONS.bus,
          // Бал за тією ж шкалою, що й звичайний рейс: дорога додому не
          // терміновіша за інші справи, вона просто про інше.
          rank: лишилось <= 45 ? RANK.BUS_SOON : RANK.BUS_LATER,
          labelVariants: [`ДОДОМУ · О ${час}`, 'ДОДОМУ'],
          valueVariants: [`${звідки} → ${дім}`.trim(), дім],
          aria: `Автобус додому о ${час}, ${звідки} до ${дім}`,
          tap: () => {
            if (typeof window.switchTab === 'function') window.switchTab('buses');
            openSavedRouteOnBuses(додому.r.id, todayISO, звідки, дім);
          },
        };
      }
    }
