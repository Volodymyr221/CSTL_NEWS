...
const RANK = {
  ...
  // 🔴 31.08 — BUS_SOON ПІДНЯТО ВИЩЕ ЗА FEED_REPLIES. Це не зміна смаку, а
  // наслідок видалення віджета автобусів із Громади (рішення Вови).
  //
  // 🔬 ЯК ЦЕ БУЛО ДО 31.08 і чому мусило змінитись. Тут стояло `FEED_REPLIES: 65`
  // вище за `BUS_SOON: 60`, і обґрунтування було дослівно таке: «рейс за 16-45 хв
  // людина побачить ІЩЕ Й У ВІДЖЕТІ АВТОБУСІВ на цій самій сторінці». Тобто бал
  // спирався на існування віджета — на другу поверхню, яка підстраховувала.
  // Віджета більше немає, підстраховки теж, і стара розстановка означала б, що в
  // людини з трьома іншими подіями рейс за 20 хвилин просто не показується ніде.
  //
  // 🔑 Тепер обґрунтування з обох боків стоїть на самій шкалі, без опори на верстку:
  //   • BUS_SOON ВИЩЕ за FEED_REPLIES, бо рейс СПЛИВАЄ: не встиг — поїхав, і
  //     наступний може бути через годину. Коментар під дописом не спливає ніколи.
  //   • BUS_SOON НИЖЧЕ за MSG (70), бо там конкретна людина чекає відповіді зараз.
  //   • FEED_REPLIES лишається вище за все, що нижче за 60, з тієї ж причини, що й
  //     була: другої поверхні, яка показує «під вашим дописом відповіли», у
  //     застосунку немає — дзвіночок Стрічки це підписка на push, а не список
  //     непрочитаного.
  BUS_SOON:      68,   // рейс 16-45 хв — ще встигаєш зібратись, але воно спливає
  FEED_REPLIES:  65,
...
async function nowCapsule() {
  ...
  // ── 🔴 СКАСОВАНИЙ РЕЙС — НА СВОЄМУ МІСЦІ В ЧЕРЗІ (20.08 · переписано 26.09) ─
  //
  // Було 20.08: `live` відсіює `cancelled` на самому початку, тобто про
  // скасування людина з головної не дізнавалась НІКОЛИ. Заміряно на живому
  // розкладі: 17 рейсів із 350 скасовано — це 5%, не рідкість. Тому блок
  // завели, і він стояв ПЕРЕД усіма іншими гілками з власним `return`.
  //
  // 🔴 26.09 — САМЕ ЦЕЙ РАННІЙ `return` І БУВ ВАДОЮ, ЗНАЙШОВ ВОВА НА ПРОДІ.
  // Знімок 12:04: він відстежує «Луцьк → Личани» з посадкою о 12:40, а капсула
  // каже «РЕЙС О 14:25 СКАСОВАНО · наступний о 12:25». Тобто скасування за дві
  // з половиною години перебило рейс, який людина САМА позначила і на який їй
  // виходити через 36 хвилин. 📐 Корінь: блок не мав ЖОДНОГО порівняння з
  // часом — ні стелі, ні звірки з рештою кандидатів. Будь-яке скасування до
  // кінця доби вигравало все.
  //
  // 🛑 Це ламало правило №12 («через скільки рядок стане непотрібним»): рейс
  // о 12:40 стає непотрібним через 36 хв, скасування о 14:25 — через 141.
  // Показувати друге замість першого — прямо проти міри.
  //
  // 🗣️ Вова: «коли я натискаю відстежити будь-який маршрут, він має в капсулі
  // мати найвищу пріоритетність, а скасований рейс має відображатися в тій
  // самій послідовності… потім минає цей час — і повертається до звичайного
  // відображення по черзі».
  //
  // ✅ Тепер скасування — не перебивач, а ЗВИЧАЙНИЙ КАНДИДАТ У ЧЕРЗІ: воно
  // показується рівно тоді, коли САМЕ ВОНО найближче з мого (рішення нижче,
  // після того як пораховані відстежуваний рейс і боки від моєї зупинки), і
  // зникає, щойно його час минув. Бал `BUS_CANCELLED` (110) лишається: коли
  // рядок таки вийшов, він каже «не йди на зупинку» і мусить стояти вище за
  // все — але тепер він не крадає чергу в рейсу, на який ще можна встигнути.
  //
  // ⚠️ ТІЛЬКИ МІЙ. Скасування чужого рейсу — це новина, а не капсула. «Мій» =
  // відстежую АБО він іде з моєї зупинки в бік, який я обрав.
  // 🔑 Зупинку посадки для ВІДСТЕЖУВАНОГО беремо з самої позначки (`t.from`):
  // людина могла позначити рейс із Дерна, сидячи в Луцьку, і питати про час
  // посадки в Луцьку означало б порахувати не той рейс (або не знайти його).
  let скасованийМій = null;
  {
    const трекНаСьогодні = new Map(getSavedRoutesForUI()
      .filter(t => t.trackDate === todayISO).map(t => [t.routeId, t]));
    for (const r of routes) {
      if (r.status !== 'cancelled') continue;
      const трек = трекНаСьогодні.get(r.id);
      const звідки = трек?.from || myStop;
      if (!трек && !boardable(r, звідки)) continue;   // не мій — це новина, не капсула
      const m = getStopMins(r, findStopOnRoute(r, звідки)?.name);
      if (m == null || m - nowMin < 0) continue;   // те, що вже минуло, нічого не змінює
      if (!скасованийМій || m < скасованийМій.m) скасованийМій = { r, m, звідки };
    }
  }
  ...
  // ── 🔴 ЧЕРГА: ЧИ СКАСУВАННЯ ЗАРАЗ НАЙБЛИЖЧЕ (26.09) ──────────────────────
  //
  // Тут і вирішується те, чого блокові скасування бракувало з самого початку:
  // звірка з рештою черги. Вище вже пораховано все, що капсула показала б
  // інакше — відстежуваний рейс (`hit`) або найближчий бік від моєї зупинки
  // (`hit` / `далекийСьогодні`). Скасування виходить, ЛИШЕ якщо воно настає
  // не пізніше за них.
  //
  // 📐 На знімку Вови 12:04 це рахується так: відстежуваний «Луцьк → Личани»
  // дає 36 хв, скасований «Луцьк → Рівне» о 14:25 — 141 хв. 141 > 36, отже
  // виходить відстежуваний. О 14:00 буде навпаки (25 хв проти решти черги), і
  // скасування стане на своє місце. О 14:26 воно зникне само — його відсіє
  // `m - nowMin < 0` вище.
  //
  // 🔑 ПОРІВНЮЄМО ЗА ПРАВИЛОМ №12, а не за часом відправлення: для поїздки, що
  // вже триває, лічба йде до ВИСАДКИ (`пріоритет`). Інакше людина в автобусі
  // побачила б чуже скасування замість «виходити через 5 хвилин».
  // ⚠️ Нічого попереду немає (нижче буде гілка «а коли наступний?» із рейсом
  // на завтра) — тоді скасування на сьогодні однозначно ближче, і воно виходить.
  if (скасованийМій) {
    const доЧерги = hit ? пріоритет(hit)
      : далекийСьогодні ? далекийСьогодні.left
      : null;
    const доСкасування = скасованийМій.m - nowMin;
    if (доЧерги == null || доСкасування <= доЧерги) {
      const звідки = скасованийМій.звідки;
      const чч = м => `${String(Math.floor((((м % 1440) + 1440) % 1440) / 60)).padStart(2, '0')}:${String((((м % 1440) + 1440) % 1440) % 60).padStart(2, '0')}`;
      // 🛑 КАПСУЛА МУСИТЬ ЗАКРИВАТИ ПИТАННЯ, А НЕ ВІДКРИВАТИ. «Рейс скасовано»
      // без продовження — погана новина, а не сервіс. Тому поруч стоїть те, що
      // людина побачила б наступним: спершу її власна черга (`hit` —
      // відстежуваний або бік від зупинки), і лише якщо її немає — перший
      // живий рейс від цієї ж зупинки.
      // 🔑 26.09: раніше тут завжди стояв `pickFor(myStop, '')` — окремий
      // перебір, що не знав ні про відстеження, ні про вибраний бік, і міг
      // назвати не той рейс, який капсула покаже через хвилину.
      // ⚠️ Поїздка, що ВЖЕ ТРИВАЄ (`mode === 'ride'`), сюди не годиться: її час
      // посадки позаду, і «наступний о 08:50» був би не порадою, а безглуздям.
      const черга = hit?.mode === 'ride' ? null : hit;
      const запасний = черга || далекийСьогодні || pickFor(звідки, '');
      const мХв = запасний
        ? getStopMins(запасний.r, findStopOnRoute(запасний.r, запасний.from || звідки)?.name)
        : null;
      const наступний = мХв == null
        ? 'інших рейсів сьогодні немає'
        : `наступний о ${чч(мХв)}`;
      const кінець = кудиЇде(скасованийМій.r);
      return {
        key: 'now', icon: ICONS.bus,
        rank: RANK.BUS_CANCELLED,
        labelVariants: [`РЕЙС О ${чч(скасованийМій.m)} СКАСОВАНО`, 'РЕЙС СКАСОВАНО'],
        valueVariants: [`${звідки} → ${кінець || ''} · ${наступний}`.trim(), наступний],
        aria: `Рейс о ${чч(скасованийМій.m)} скасовано, ${наступний}`,
        tap: () => {
          if (typeof window.switchTab === 'function') window.switchTab('buses');
          openSavedRouteOnBuses(скасованийМій.r.id, todayISO, звідки, '');
        },
      };
    }
  }
