...
//
// ⚠️ 11.08 — ФУНКЦІЯ НЕ ЗМІНИЛАСЬ, ЗМІНИВСЯ ЇЇ НАСЛІДОК, і це варто записати.
// Тут стояло: «прибирати гео СКРІЗЬ не можна — у розділі "Україна та Світ" мітка
// каже, що саме це, Україна чи Світ». Після розділення цього розділу надвоє мітка
// збігається з назвою вкладки в УСІХ чотирьох розділах, тож знімається всюди.
// 🔑 Правило лишилось те саме («ховаємо збіг»), і саме тому воно пережило зміну
// продукту без жодної правки — на відміну від перевірки, яка стерегла НАСЛІДОК
// («у Україні та Світі мітка ЛИШАЄТЬСЯ») і від цієї зміни впала.
...
// Перша новина списку — ВЕЛИКОЮ карткою (патерн Apple News / Google News).
//
// 🔴 26.08 — УМОВУ «ТІЛЬКИ ЯКЩО Є ФОТО» ЗНЯТО (потік /byyou, крок 5).
// Тут стояло: «герой без знімка — це просто роздутий блок тексту, який зʼїдає
// екран і нічого не додає». Це було правдою, поки велика картка малювала слот
// 358×200 під фотографію: без знімка там лишалась порожня плита.
// 🗑 З кроку 4 такого слоту немає — картка без фото переходить у стан
// `.nc--noimg` і лишається великою за ТЕКСТОМ (більший заголовок, довший анонс).
// Тобто підстава умови зникла, а сама умова лишалась і давала гіршу поведінку:
// хаб то мав велику першу картку, то не мав — залежно від того, чи трапилось
// фото в найсвіжішої новини. Для читача це виглядало як випадковість.
//
// ⚠️ ЩО ЦЕ ЛАМАЄ, названо вголос: у розділах, де фото рідкі (Волинь — 69% статей
// з фото, у перших шести 4 з 6), перша картка тепер завжди велика. Це навмисно:
// «найсвіжіше — головне» тепер правило, а не збіг у даних.
//
...
// 🔑 `nc--noimg` вішаємо ТУТ лише для «фото немає в даних». Випадок «фото є, але
// не доїхало» закриває `handleImgError` — і саме він на проді частіший.
function markLead(list, all) {
  const first = list.firstElementChild;
  if (!first || !all.length) return;
  first.classList.replace('nc--row', 'nc--lead');
  if (!all[0].image) first.classList.add('nc--noimg');
}
