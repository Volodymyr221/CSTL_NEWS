...
// 🔎 УВЕСЬ ТЕКСТ ВІДПОВІДЕЙ — ДЛЯ ПОШУКУ (06.09, замовлення Вови: «в питаннях
// зроби таке, можливо хтось буде шукати якесь питання»).
//
// 🔑 Чому відповіді взагалі мусять бути в пошуку. Питання в громаді практичні
// («чи працює амбулаторія», «коли вивозять сміття»), і людина, яка вже бачила
// цю розмову, згадує не формулювання ПИТАННЯ, а те, що в ній ВІДПОВІЛИ: назву
// установи, годину, імʼя майстра. Слова відповіді в тексті питання немає ніде.
//
// ⚠️ Беремо ВСІ живі відповіді, не лише кореневі — на відміну від `answersCount`
// і картки. Там строгість потрібна (число і цитата описують «відповіді на
// питання»), тут — навпаки: репліка під чужою відповіддю це такий самий текст,
// у якому людина могла запамʼятати слово. Строгість тут лише губила б збіги.
// 🛑 Видалені не беремо — пошук не має воскрешати те, що прибрали з екрана.
export function answersText(postId) {
  return activeComments(postId).map(c => c.text || '').join(' ');
}
...
// ⚠️ ДРУГИЙ АРГУМЕНТ — ОБʼЄКТ, А НЕ РЯДОК, і це навмисно. Вище вже стоїть
// попередження, що `.map(renderQuestionCard)` кликати не можна: `map` передав би
// другим аргументом ІНДЕКС. З обʼєктом така помилка нешкідлива — `(2).why` це
// `undefined`, тобто підпис просто не намалюється; з рядком у картку надрукувалось
// би голе число. Захист коштує нуль, а клас помилки в проєкті вже спрацьовував.
export function renderQuestionCard(p, opts = {}) {
