...
const LS_QA_FLOOR_BASE = 'cstl-qa-floor-v1';
...
export function qaFloorTs() {
  ...
  // 🔴 ЗАПАСНИЙ ПОРІГ — `chatSeenTs()`, і він не «про всяк випадок».
  // Поріг ставиться при першій появі у вкладці. Але капсула «нове питання» живе
  // на ГРОМАДІ й може спрацювати РАНІШЕ, ніж людина взагалі відкриє Питання —
  // саме тоді, коли вона найпотрібніша. Без запасного значення поріг був би
  // нулем, і капсула мовчала б рівно для того, хто у вкладку ще не заходив.
  // 🔑 Беремо ту позначку, яка вже відповідає на це питання в решті застосунку
  // (Новини · Дошка · Стрічка мають таку саму), і не заводимо для Питань
  // ЧЕТВЕРТУ модель «баченого». Щойно людина зайде у вкладку — поріг
  // зафіксується власним ключем і рухатись перестане.
  return chatSeenTs();
}

/** Поставити поріг, якщо його ще немає. Ідемпотентно — кличеться на кожному вході у вкладку. */
export function markQaFloorOnce() {
  if (qaFloorTs()) return;
  lsSet(LS_QA_FLOOR_KEY(), Date.now());
}
