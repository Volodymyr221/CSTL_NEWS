...
// 🔴 03.09 — ДРУГИЙ ПРИХОВАНИЙ ВХІД ТЕЖ ПРИБРАНО.
// 🗣️ Вова: «Другий прихований вхід прибираємо. Входи можемо залишити тільки з
// бургер-меню адміністратора або власника… самого додатку, не спільноти».
// Тут стояли пʼять тапів по ЛІЧИЛЬНИКУ ВЕРСІЇ (`.deploy-stamp`) → `admin.html`.
//
// 🔑 ЧОМУ ЦЕ БЕЗПЕЧНО ПРИБРАТИ, А НЕ ВТРАТА ДОСТУПУ. Вхід у кабінет лишається в
// бургер-меню (`kind: 'cabinet'` у `sidebar.js`), і він показується лише команді —
// пункт ховається за `is_team_member()`, тобто за **security-definer функцією в
// базі**, а не за клієнтською перевіркою. Це рівно ті ролі, які назвав Вова:
// адміністратор або редактор ЗАСТОСУНКУ, не спільноти.
// 📐 Заміряно 03.09 на прохання Вови «чи можна це зламати»: сервісного ключа в
// клієнті немає, доступ вирішує `is_team_member()`, запис стережуть RLS на
// `is_admin()`. Сторонній, що набере `/admin.html`, побачить порожній інтерфейс.
// ➡️ Тобто прихований жест не був захистом — він був лише зручністю. Прибрано.
//
// ⚠️ Що з цим зникає: у ВСТАНОВЛЕНОМУ додатку немає адресного рядка, тож набрати
// `./admin.html` руками там не вийде. Єдиний шлях тепер — бургер-меню. Це і є те,
// чого Вова просив.
