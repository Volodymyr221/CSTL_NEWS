...
- **Блок 3 (цей):** чіткий поділ двигунів контенту + памʼять + семантичний дедуп (див. §9).
- **Далі (плагіни):** конектори (telegram/facebook), генерація фото (flux), нові sink-и.

## 9. Двигуни контенту (Блок 3)

Хто що наповнює — за принципом «чи треба ПИСАТИ»:

| Поверхня | Двигун | Режим | Куди |
|---|---|---|---|
| Волинь / Україна / Світ | `parse_rss.py` (cron 30хв) | ПАРСИНГ (є звідки) | `articles.json` авто-живе |
| Громада (табло) | `ai_news_agent.py` (лише ця місія) | AI ПИШЕ оригінал | чернетка → кабінет → **ревʼю Алли** |
| Свята («Шо в селі») | `editor/` місія holidays | AI ПИШЕ статтю+фото | чернетка → кабінет → **ревʼю Алли** |

- **`active_missions`** у `hromada_config.json` = `["Громада"]`. Волинь/Україна-Світ вимкнено (RSS дає безкоштовно) → економія веб-пошуків.
- **Памʼять** `data/hromada_memory.json` — персистентний лог написаного (заголовок, категорія, джерела, ключ). Агент бачить її в промпті («вже писали — бери НОВІ кути»). МʼЯКА (не жорсткий блок).
- **Семантичний дедуп** `editor/core/dedup.py` — AI-кластер (1 виклик Anthropic, заголовок+лід) ловить «та сама історія іншими словами». Вшитий у `enqueue()` після дедупу-за-словами. МʼЯКИЙ (лише явні повтори), **фейл-софт** (без ключа/помилка → нічого не дропає). Вимикач: env `SEMANTIC_DEDUP=0`.
- **Написане = через людину:** обидва AI-джерела (Громада+свята) йдуть у чернетки; Алла ревʼю/публікує. Живе з ключем Вови (`SUPABASE_SERVICE_ROLE_KEY`); без нього — fallback у `data/editor_drafts.json` (тримається, НЕ авто-публікується).
- `news_queue.json`/`publish_queue.py`/`news-publisher.yml` — **дормантні** (не видалені; черга порожня → публікатор no-op).
