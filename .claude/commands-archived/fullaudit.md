> 🔒 **ЗАБЛОКОВАНО (14.07.2026) — не запускати.** Активувати тільки коли Рома явно скаже що проект на фінішній прямій/поліровці перед релізом. Дизайн (13 вимірів + severity-верифікація) готовий і лишається актуальним.
>
> _Причина блокування: застосунок ще активно будується (Рома + Вова паралельно), повний аудит зараз занадто дорогий відносно користі (один прогін ≈ 3.6М токенів, ~63 хв, 91 агент). Файл навмисно винесено з `.claude/commands/` у `.claude/commands-archived/`, щоб команда `/fullaudit` фізично НЕ реєструвалась. Щоб знову увімкнути — `git mv .claude/commands-archived/fullaudit.md .claude/commands/` і прибрати цей банер._
