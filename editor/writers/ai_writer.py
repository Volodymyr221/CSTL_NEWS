...
для стрічки «Шо в селі» + видає ТОЧНИЙ англ. image_query (урок тесту:
...
class AIWriter(Writer):
    def write(self, item, cfg):
        ...
        prompt = (
            ...
            '"ТОЧНИЙ короткий АНГЛ. запит для фото з Wikimedia — конкретний обʼєкт/символ '
            'свята (напр. Saint Sophia Cathedral Kyiv), НЕ загальні/неоднозначні слова"}'
        )
        payload = {
