...
MAX_MONTH_USD = float(os.environ.get("AI_MAX_MONTH_USD", "6.4"))   # 4→5 17.09; 🔴 5→6.4 21.09
# 📐 6.4 = кишені місій $1.60 (свята $0.4 + OLYKA CASTLE $0.6 + Історія $0.6)
# + новини $4.80. 🛑 Сума кишень НЕ СМІЄ перевищити цю стелю — інакше вона
# фіктивна, а «резерв для дешевих агентів» існує лише на папері. Стереже
# `tests/ai-budget.mjs`.
MAX_DAY_USD = float(os.environ.get("AI_MAX_DAY_USD", "0.50"))      # 🔴 1.20→0.50 17.09: $1.20 це 24% місяця за добу — саме так місяць і згорав за дві доби


def cost_of_run(r: dict) -> float:
    """Скільки цей прогін коштує за ЧИННИМИ цінами (не за тими, що були в день запису).

    🔑 Кожен запис journal несе власний `model` і всі лічильники токенів, тож
    перерахувати його можна будь-коли й відтворювано.
    """
    p = PRICES.get(r.get("model")) or PRICES["claude-sonnet-5"]
    # 🛑 `suspect_usd` — НЕ рахунок від моделі, а наша ОЦІНКА списання за прогін,
    # який обірвався мережею (`ai_news_agent.record_spend(extra_usd=…)`): токенів у
    # ньому нуль, бо відповіді ми не побачили. Її не можна вивести з токенів, тож
    # переносимо як є.
    # 🔬 Спіймано сторожем `ai-budget` 17.09: без цього рядка перерахунок бачив
    # «записано $0.30, за токенами $0» і зчитував усі 30 центів як завищення —
    # тобто МОВЧКИ СТИРАВ підозру з місячної суми, і стеля переставала її бачити.
    return round(
        float(r.get("input_tokens", 0)) / 1_000_000 * p["in"]
        + float(r.get("output_tokens", 0)) / 1_000_000 * p["out"]
        + float(r.get("cache_read", 0)) / 1_000_000 * p["cache_read"]
        + float(r.get("cache_write", 0)) / 1_000_000 * p["cache_write"]
        + float(r.get("web_searches", 0)) / 1000 * PRICE_SEARCH_1K
        + float(r.get("suspect_usd", 0) or 0), 4)


def month_spend_recalc(month: str = "") -> float:
    """🔴 17.09 — ВИТРАТИ МІСЯЦЯ ЗА ЧИННИМИ ЦІНАМИ, а не за записаними.

    🔬 ЗАРАДИ ЧОГО. 02.09 виправили ціни Sonnet 5 ($3/$15 → правильні $2/$10), але
    записи, зроблені ДО фіксу, лишились пораховані за старим тарифом — а саме їх
    читали місячна стеля й лінія темпу. 📐 Заміряно 17.09 на вересні: журнал казав
    **$2.7061**, чесний перерахунок дав **$2.0742** — завищення **30.5%**.
    🛑 Наслідок не косметичний: агент новин зупиняв себе за числом, якого не існує —
    845 прогонів поспіль «пропущено», а чернетки в кабінеті стояли з 01.09.

    🔑 ЧОМУ ПЕРЕРАХУНОК, А НЕ ПРАВКА ЗАПИСІВ. `runs` — журнал ФАКТІВ (що відправили
    і що відповіли); його не переписуємо. `months[*].cost_usd` — похідний АГРЕГАТ,
    і він має право бути перерахованим, щойно ми дізнались правильну ціну.

    🛑 РАХУЄМО ПОПРАВКУ, А НЕ СУМУ — І ЦЕ НЕ ПРИСКІПЛИВІСТЬ. Перша версія цієї
    функції просто підсумовувала `runs` за місяць. Сторож `ai-budget` її завалив, і
    слушно: журнал підрізається до `KEEP_RUNS`, а в пісочниці стенда `runs` взагалі
    порожній — сума давала **$0**, тобто стеля мовчки переставала спиняти виклики.
    ⚠️ Занижена стеля небезпечніша за завищену: завищена гальмує агента, занижена
    дає йому витратити чужі гроші.
    ➡️ Тому: беремо записану суму місяця і віднімаємо поправку РІВНО за ті прогони,
    які справді бачимо в журналі. Немає прогонів — поправка нульова, число лишається
    те саме, що було. Гірше вже не стане ніколи.
    """
    month = month or time.strftime("%Y-%m", time.gmtime())
    d = _load()
    записано_місяць = float((d.get("months", {}).get(month, {}) or {}).get("cost_usd", 0) or 0)
    записано_видимих, чесно_видимих = 0.0, 0.0
    for r in d.get("runs", []):
        try:
            if time.strftime("%Y-%m", time.gmtime(float(r.get("ts", 0)) / 1000)) != month:
                continue
            записано_видимих += float(r.get("cost_usd", 0) or 0)
            чесно_видимих += cost_of_run(r)
        except Exception:
            continue
    поправка = записано_видимих - чесно_видимих      # >0 коли записане було завищене
    return round(max(записано_місяць - поправка, 0.0), 4)


def month_spend_usd() -> float:
    """Витрачено цього місяця — усіма агентами разом, за ЧИННИМИ цінами."""
    try:
