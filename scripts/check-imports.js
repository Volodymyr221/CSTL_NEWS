...
// 🔴 24.08 — ТА САМА МІНА ВИБУХНУЛА ВДРУГЕ, ТЕПЕР У `core/board-shared.js`.
// У шапці цього файлу (рядки 6-7) стоїть рядковий коментар «імпортує ЛИШЕ з
// core/* — ніколи з tabs/*». Для регулярки нижче `/*` там — ПОЧАТОК блок-
// коментаря. Поки у файлі не було жодного `*/`, збігу не траплялось і все
// мовчало ЧОТИРИ МІСЯЦІ. Щойно там з'явився перший звичайний `/* … */` —
// регулярка «закрила» коментар на ньому і викинула ВЕСЬ проміжок разом з
// імпортами й оголошеннями: 3 хибні «called but not imported/declared», і
// жодне з названих чисел рядків не вказувало на справжнє місце.
//
// 🔑 29.07 корінь полікували ЛИШЕ з боку рядків у подвійних лапках
// (`blankDoubleQuoted`) — а джерел у `/*` два, і друге це РЯДКОВИЙ КОМЕНТАР.
// Тому тепер гасимо маркери блок-коментаря і в них.
//
// ⚠️ Гасимо САМЕ МАРКЕРИ (`/*`, `*/`), а не рядок цілком, і лише там, де рядок
// ПОЧИНАЄТЬСЯ з `//`. Такий рядок не може містити коду за визначенням, тож
// зіпсувати аналіз нічим; а якби ми стирали рядок повністю, то зачепили б
// рідкісний випадок «рядок усередині шаблонного рядка, що починається з //».
// ⚠️ Довжину зберігаємо (пробіли замість символів) — номери рядків і колонок
// мусять лишитись тими самими.
function defuseMarkersInLineComments(content) {
  return content.split('\n').map(line =>
    /^\s*\/\//.test(line) ? line.replace(/\/\*|\*\//g, '  ') : line
  ).join('\n');
}
function stripBlockComments(content) {
  return blankDoubleQuoted(defuseMarkersInLineComments(content))
    .replace(/\/\*[\s\S]*?\*\//g, '');
}
...
function extractImports(content) {
  ...
  for (const m of content.matchAll(/import\s*\{([\s\S]+?)\}\s*from\s*['"][^'"]+['"]/g)) {
    // 🔴 04.09 — КОМЕНТАРІ ВСЕРЕДИНІ СПИСКУ ЗНІМАЮТЬСЯ ДО РОЗБОРУ.
    // Без цього рядка прилад БРЕХАВ: список ріжеться по комі, тож у шматку
    // «renderEventBlock,   // пояснення\n  renderCommunityNews» після коми
    // лишається коментар РАЗОМ із наступним іменем, регулярка його не впізнає —
    // і цілком імпортоване імʼя оголошується «not imported».
    // 🛑 Ловилось це найгірше з можливих способів: збірка падала на файлі, у
    // якому все правильно, і вказувала на РЯДОК ВИКЛИКУ, а не на імпорт.
    // ⚠️ У проєкті коментар у списку імпортів — звичайна річ (там пояснюють,
    // навіщо саме цей модуль), тож правити треба прилад, а не прибирати
    // пояснення: це вже вкотре випадок «перша підозрювана — сама перевірка».
    const тіло = m[1].replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/[^\n]*/g, '');
    for (const part of тіло.split(',')) {
