...
--
-- ✅ НАКАТАНО 09.08.2026 (накотив Вова через Supabase SQL Editor — запис у прод
--    із сесії блокує сторож дозволів середовища, читання дозволене).
--
-- 📐 ДОВЕДЕНО ЖИВИМ РЕГРЕСОМ ПІСЛЯ НАКАТУ:
--    роль `anon`          — рядків 11 → **0**, унікальних uid 5 → **0**
--    подання              — 8 постів, сума лайків **11** (не втрачено жодного)
--    роль `authenticated` — 11 рядків, свій лайк на місці (1), подання теж доступне
--    Тобто гість утратив рівно uid і нічого крім них.
--
