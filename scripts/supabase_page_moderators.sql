...
-- ═══════════════════════════════════════════════════════════════════════════
-- ДРУГА ІТЕРАЦІЯ (24.07) — системне виправлення додавання людей
-- ✅ Накатано міграціями page_team_invites_and_role_fix + fix_team_list_order
--
-- ЩО БУЛО ЗЛАМАНО (ламалось у КОЖНОГО користувача, не лише в однієї пошти):
--   1) 🔴 `page_admins_role_check` дозволяє ролі 'owner'/'admin', а RPC вставляв
--      'moderator' → порушення обмеження → «Не вдалося додати» ЗАВЖДИ, для будь-кого.
--      Помилка моя: писав RPC, не звіривши наявні обмеження таблиці.
--   2) Пошук ішов по `auth.users`, а зовнішній ключ `page_admins.uid` дивиться на
--      `profiles(uid)`. Якщо профілю ще нема — вставка впала б.
--   3) Людину, яка ЩЕ НЕ заходила в додаток, додати було неможливо взагалі —
--      глухий кут для реального сценарію «друг скинув пошту, додай мене».
--
-- ЩО СТАЛО:
--   • роль 'moderator' дозволена обмеженням;
--   • пошук по `profiles` (там і пошта, і потрібний ключ) + запасний шлях;
--   • пошта нормалізується (пробіли, великі літери) і перевіряється на формат;
--   • НОВА таблиця `page_invites` — запрошення для незареєстрованих. Спрацьовує
--     автоматично тригером `trg_profiles_claim_invites` при появі профілю;
--   • `claim_my_page_invites()` — підстраховка з клієнта при вході;
--   • `list_page_moderators` показує і запрошених (status='invited');
--   • `cancel_page_invite` — скасувати запрошення.
--
-- ВІДПОВІДІ add_page_moderator: 'ok' | 'invited' | 'already' | 'bad_email' + помилка прав.
--
-- Перевірено наживо (від імені власника сторінки):
--   зареєстрована пошта → ok · та сама з пробілами й ВЕЛИКИМИ → already
--   незареєстрована → invited · «не-пошта» → bad_email

-- ═══════════════════════════════════════════════════════════════════════════
-- ТРЕТЯ ІТЕРАЦІЯ (24.07) — ЗАПРОШЕННЯ ПРИБРАНО (рішення Вови)
-- ✅ Накатано міграцією drop_page_invites_registered_only
--
-- ЧОМУ ПРИБРАНО: запрошення видавало права РЯДКУ-пошті, а не перевіреній людині.
-- Одна помилка в літері — і сторонній, хто пізніше зареєструє цю адресу, мовчки
-- став би модератором офіційної сторінки. Підтвердження не було (листів не шлемо),
-- тож це слабше за Slack/GitHub, на які я подумки орієнтувався. Суперечило
-- базовому правилу додатка: без авторизації — лише читання.
--
-- СТАЛО: у команду додається ЛИШЕ той, у кого вже є акаунт.
-- Пошук іде по profiles.email АБО auth.users.email (пошта може бути записана лише
-- в одному з них), але завжди серед тих, у кого Є профіль — бо page_admins.uid
-- посилається саме на profiles(uid).
--
-- Відповіді add_page_moderator: 'ok' | 'not_found' | 'already' | 'bad_email'.
--
-- ПЕРЕВІРЕНО НА ВСІХ АКАУНТАХ (не вибірково): для кожного з 10 зареєстрованих
-- користувачів пошук знаходить його і за звичайною поштою, і за поштою з
-- ВЕЛИКИМИ літерами та пробілами. Не знайдено: 0.
