...
from editor.core.facts import (                       # noqa: E402
    причина_застарілості, факт_недатоване_число,      # noqa: F401
    факт_відносний_час, факт_посада_без_дати,         # noqa: F401
    факти_без_застарілих)                             # noqa: F401
...
MAX_SEARCHES_PER_MISSION = 4        # 🔴 6→4 17.09: пошук це 24% ціни прогону ($0.01 за запит). 4 лишає запас на тему, 6 витрачались рідко
...
# 🔴 21.09 — 3.4 → 4.8. РІШЕННЯ ВОВИ: «агенти мають працювати, а не стояти».
# 📐 ЗАМІРЯНО В МОМЕНТ РІШЕННЯ, і це не те, що здавалось: агент стояв НЕ через
# вичерпану стелю. Витрачено $2.5877 із $3.40 — до стелі лишалось $0.81. Спиняло
# його ГАЛЬМО РІВНОГО ТЕМПУ: на 21-й день місяця дозволено $3.40 × 21/30 = $2.38,
# а витрачено вже $2.5877.
# 🔑 Тому стелю підняли, а гальмо лишили недоторканим. Послабити гальмо було б
# простіше на вигляд — і повернуло б рівно те, від чого його ставили 20.08:
# $2.35 з'їдені за дві доби 01-02.09, після чого агент мовчав два тижні.
# ➡️ Нова лінія темпу на той самий 21-й день: $4.80 × 21/30 = $3.36, тобто запас
# $0.77 і негайний виїзд.
MAX_MONTH_COST_USD = float(os.environ.get("AI_NEWS_MAX_MONTH_USD", "4.8"))  # стеля (15→4 08.07; 4→3 20.08; 3→3.4 17.09; 🔴 3.4→4.8 21.09)
# 📐 РІШЕННЯ ВОВИ 17.09: спільний кошик усіх агентів піднято до $5/міс. Новинам іде
# те, що лишається після кишень місій ($1.60: свята $0.4 + OLYKA CASTLE $0.6 +
# Історія $0.6), тобто $3.40. 🛑 Спокуса дати новинам усі $5 спинена сторожем
# `ai-budget`: сума кишень не сміє перевищувати спільну стелю, інакше стеля
# фіктивна і «резерв для стрічки» існує лише на папері.
# 🔑 Виграш не в самій стелі, а в тому, що поруч виправлено: журнал завищував
# витрати на 30.5%, тож прогін «коштував» $0.30 замість чесних ~$0.21.
# ➡️ Разом: було ~10 прогонів, з'їдених за дві доби; стало ~16 РІВНОМІРНО за місяць.
...
from editor.core.spend import PRICES, PRICE_SEARCH_1K, month_spend_recalc   # noqa: E402
...
def record_spend(mission: str, usage: dict, found: int, note: str = "", extra_usd: float = 0.0):
    """Рахує вартість одного запуску за usage і дописує у data/ai_spend.json.

    🔴 02.09 — `extra_usd` ЗАВЕДЕНО, ЩОБ «ПІДОЗРА НА СПИСАННЯ» ПЕРЕЖИЛА ПРОГІН.
    Було так: мережа рвала виклик, Anthropic міг списати гроші server-side, а
    клієнт бачив `usage = 0`. Оцінку `SUSPECT_CHARGE_USD` додавали ЛИШЕ до
    `run_cost` — змінної в памʼяті. У журнал ішов нуль.
    📐 Наслідок вимірюваний: контейнер раннера вмирав, і разом із ним зникала
    єдина згадка про ці гроші. Місячна стеля наступного прогону рахувала суму
    без них — тобто **реальні витрати були вищі за ті, що бачив запобіжник**.
    Це один із каналів, яким серпень закрився на $3.3587 при стелі $3.00.
    ⚠️ Оцінка навмисно ПЕСИМІСТИЧНА (переоцінити безпечніше, ніж недоврахувати):
    якщо списання не було, ми лише трохи раніше пригальмуємо темп.
    """
    cost = round(
...
def month_spend_usd() -> float:
    """Скільки вже витрачено цього місяця (UTC) — за ЧИННИМИ цінами.

    🔴 17.09 — РАХУЄМО ПЕРЕРАХУНКОМ, а не накопиченим полем `months`. Причина
    заміряна того дня: ціни Sonnet 5 виправили 02.09, але записи ДО фіксу лишились
    за старим тарифом ($3/$15 замість $2/$10), і місячне поле показувало **$2.7061
    замість чесних $2.0742 — завищення 30.5%**. Агент через це гальмував себе за
    неіснуючим числом: 845 прогонів «пропущено», чернетки в кабінеті стояли з 01.09,
    і саме це Вова побачив як «парсер не працює».
    ⚠️ Журнал прогонів (`runs`) НЕ переписуємо — перераховуємо лише похідний агрегат.
    Деталі й запобіжник «місяць має бути повністю в журналі» — `editor/core/spend.py`.
    """
    try:
        return month_spend_recalc()
    except Exception:
...
def month_pace_left_usd() -> float:
    """Скільки ще можна витратити СЬОГОДНІ, щоб бюджету вистачило РІВНО до кінця місяця.

    🔴 02.09 — ЗАВЕДЕНО ПІСЛЯ ПРЯМОГО ПИТАННЯ ВОВИ: «нам потрібно, щоб агенти
    працювали цілий місяць».

    🔬 ЩО ЗАМІРЯНО, І ЧОМУ СТАРИХ ДВОХ СТЕЛЬ БУЛО МАЛО. Місячна ($3.00) і добова
    ($1.20) не суперечать одна одній на папері, але разом дозволяють спалити
    місяць за **2.5 доби**: $3.00 ÷ 30 діб = $0.10/добу, тобто добова стеля
    рівно у **12 разів** більша за темп, який місяць витримує.
    📐 Це не гіпотеза: станом на 2 вересня журнал показував **$1.99 з $3.00 за
    дві доби** ($0.99/добу). Стеля вичерпалась би 3 вересня, і агент замовк би
    на 27 днів. Саме це Вова й побачив.

    🔑 РІШЕННЯ — не третя стеля, а ЛІНІЯ ТЕМПУ. Дозволено стільки, скільки вже
    «набігло» від початку місяця: на 2-гу добу з 30 доступно 2/30 бюджету.
    Витратив швидше — чекаєш, поки лінія дожене. Витратив менше — залишок сам
    переноситься вперед, бо рахується від початку місяця, а не від доби.
    ⚠️ Це НЕ економія: за місяць агент витратить рівно ту саму стелю. Змінюється
    тільки РІВНОМІРНІСТЬ, тобто те, чи буде він живий 28 числа.

    🛑 Добову стелю НЕ прибираємо — вона лишається другим рубежем «не зʼїсти все
    за один день» (рішення 20.08). Лінія темпу відповідає на інше питання:
    «чи вистачить до кінця місяця».
    """
    import calendar
    зараз = time.gmtime()
    діб_у_місяці = calendar.monthrange(зараз.tm_year, зараз.tm_mon)[1]
    # Поточна доба враховується ПОВНІСТЮ: інакше 1-го числа дозволено було б нуль
    # і агент не зміг би зробити жодного прогону в перший день місяця.
    минуло = min(зараз.tm_mday, діб_у_місяці)
    дозволено = MAX_MONTH_COST_USD * минуло / діб_у_місяці
    return round(дозволено - month_spend_usd(), 4)
...
def build_prompt(mission_name: str, cfg: dict, existing: list, target: int = None) -> str:
    ...
    if mission_name == "Громада":
        ...
        body = (
            ...
            "числа взагалі.\n"
            "🔴 ЧАС НАЗИВАЙ ДАТОЮ, А НЕ ВІДНОСНО СЕБЕ. Заборонено «нещодавно», "
            "«цьогоріч», «торік», «цього року», «останнім часом»: пост читають "
            "через місяці, і ці слова тоді означають уже інше. Пиши «у 2024 році», "
            "«у серпні 2025-го». Коли саме — не знаєш, краще не згадувати час.\n"
            "🔴 ПОСАДУ ПИШИ З ДАТОЮ або не пиши взагалі: «очолює громаду» правдиве "
            "лише до наступних виборів, а текст лишається. Треба — «станом на "
            "2024 рік очолював».\n"
...
def _факти_з_тексту(текст: str, межа: int = 8) -> list:
    ...
    for шматок in (текст or "").replace("\n", " ").split("."):
        ...
        if len(ш) > 25:
            ...
            elif причина_застарілості(ш):
                # Рядок, що постаріє мовчки: число без року, час відносно
                # моменту написання або посада без дати. Викидаємо РЯДОК, а не
                # тему: решта фактів лишається, і пост виходить біднішим, але
                # правдивим. Це дешевше за спростування.
                print(f"  ✂ {причина_застарілості(ш)}: «{ш[:70]}…»")
            else:
                вихід.append(ш + ".")
        if len(вихід) >= межа:
            break
...
def write_agent_status(стан: str, причина: str = "", коли_відновиться: str = "") -> None:
    """🔴 17.09 — АГЕНТ САМ ПИШЕ, ЧОМУ ВІН МОВЧИТЬ.

    🗣️ Замовлення Вови 17.09: «якщо є якісь проблеми з агентами, то потрібно це
    позначати».

    🔬 ЗАРАДИ ЧОГО, ДОСЛІВНО З ЖИТТЯ. Два тижні кабінет показував чернетки від
    01.09 і жодної нової. Агент при цьому СТАРТУВАВ щогодини — 845 прогонів, усі
    зелені, — доходив до перевірки бюджету і мовчки виходив. Причина існувала
    рівно в одному місці: рядок у лозі GitHub Actions, куди Вова не дивиться і не
    мусить. З кабінету це виглядало як «парсер зламався».

    🔑 ЧОМУ ПИШЕ САМЕ АГЕНТ, А НЕ РАХУЄ КАБІНЕТ. Кабінет міг би оцінити стан із
    журналу витрат сам — але тоді правило жило б у ДВОХ місцях (тут і в JS), і
    перше ж уточнення стелі розвело б їх. Хто ухвалює рішення — той його й
    записує; кабінет лише показує.

    ⚠️ Fail-soft: не змогли записати стан — прогін через це не падає. Стан це
    зручність, а не робота агента.
    """
    try:
        d = load_spend()
        d["agent_status"] = {
            "стан": стан,                       # "працює" | "пауза"
            "причина": причина,
            "відновиться": коли_відновиться,
            "ts": int(time.time() * 1000),
            "витрачено_міс": round(month_spend_usd(), 4),
            "стеля_міс": MAX_MONTH_COST_USD,
        }
        save_spend(d)
    except Exception as e:
        print(f"  ⚠ стан агента не записався ({e}) — на роботу не впливає")


def дата_коли_догнати() -> str:
    """Коли лінія темпу дожене вже витрачене (щоб сказати Вові число, а не «пізніше»)."""
    import calendar
    try:
        зараз = time.gmtime()
        діб = calendar.monthrange(зараз.tm_year, зараз.tm_mon)[1]
        витрачено = month_spend_usd()
        for д in range(зараз.tm_mday, діб + 1):
            if MAX_MONTH_COST_USD * д / діб - витрачено > 0:
                return f"{д:02d}.{зараз.tm_mon:02d}"
        return f"01.{(зараз.tm_mon % 12) + 1:02d}"      # аж наступного місяця
    except Exception:
        return ""
...
def main():
    ...
    if not args.dry_run:
        ...
        if spent_month >= MAX_MONTH_COST_USD:
            ...
            write_agent_status("пауза", f"місячна стеля ${MAX_MONTH_COST_USD} вичерпана",
                               f"01.{(time.gmtime().tm_mon % 12) + 1:02d}")
            return
        # 🔴 02.09 — ЛІНІЯ ТЕМПУ. Замовлення Вови: «щоб агенти працювали цілий місяць».
        # Дві стелі вище цього НЕ гарантували: $3.00/міс при $1.20/добу спалюються
        # за 2.5 доби (заміряно 02.09: $1.99 з $3.00 за дві доби).
        запас = month_pace_left_usd()
        if запас <= 0:
            print(f"⏳ темп випереджає бюджет: цього місяця вже витрачено ${month_spend_usd()} "
                  f"з ${MAX_MONTH_COST_USD}, а на сьогодні за рівним темпом мало бути менше. "
                  f"Прогін пропущено, щоб бюджету вистачило до кінця місяця.")
            write_agent_status("пауза", "темп випереджає бюджет — вирівнюється, щоб вистачило до кінця місяця",
                               дата_коли_догнати())
            return
    ...
    for name in missions:
        ...
        while len(кошик) < ліміт and batch_no < 4:
            # 🔴 02.09 — МІСЯЦЬ ПЕРЕВІРЯЄМО І ТУТ, А НЕ ЛИШЕ НА СТАРТІ (знахідка аудиту гонок).
            # Було: стеля місяця читалась ОДИН раз до циклу, далі прогін витрачав до
            # $1.20 без жодної повторної перевірки — тобто перевищення було закладене
            # в конструкцію, і воно сталося: серпень закрився на $3.3587 при стелі $3.00.
            # ⚠️ `month_spend_usd()` читає журнал, який ЦЕЙ прогін уже дописав
            # (`record_spend` пише після кожного виклику), тож перевірка бачить свіже.
            if not args.dry_run and month_spend_usd() >= MAX_MONTH_COST_USD:
                print(f"⛔ місячна стеля ${MAX_MONTH_COST_USD} досягнута ПОСЕРЕД прогону — пакети зупинено")
                break
            ...
            # 🔴 02.09 — ПІДОЗРА НА СПИСАННЯ ЙДЕ В ЖУРНАЛ ОДРАЗУ, разом із записом
            # прогону. Раніше вона додавалась окремим рядком нижче — але тільки до
            # `run_cost`, тобто жила до кінця прогону і вмирала з контейнером.
            # Тепер її бачать і місячна стеля, і лінія темпу, і добове вікно.
            підозра = SUSPECT_CHARGE_USD if not ok else 0.0
            ...
            if not ok:
                print(f"  ⚠ мережевий збій — +${SUSPECT_CHARGE_USD} записано в журнал як підозра; НЕ переписую промпт (idempotent backoff уже був у call_agent)")
