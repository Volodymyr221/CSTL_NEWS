...
-- 4) ЗАПРОСИТИ РЕДАКТОРА — підстав СПРАВЖНЮ пошту людини перед запуском.
--    ⚠️ Має йти ПЕРЕД бекфілом (крок 5), щоб залінкувало її в цей же запуск,
--    якщо вона вже створила акаунт.
-- 🛑 РЕАЛЬНУ ПОШТУ ТУТ НЕ ЛИШАТИ. Репозиторій ПУБЛІЧНИЙ — адреса живої людини
--    в цьому рядку стає загальнодоступною. До 18.08.2026 тут стояла справжня
--    пошта редакторки, яка вийшла з проєкту ще 26.07; знайдено аудитом на ПД.
--    Підставляй адресу в момент запуску скрипта і не комітируй її назад.
INSERT INTO public.editor_invites (email, name, can_create, can_publish, can_events)
VALUES ('editor@example.com', 'Редактор', TRUE, TRUE, TRUE)
