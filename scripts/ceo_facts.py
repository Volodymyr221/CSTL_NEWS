...
def контент():
    ...
    # ── 🔴 ЧОМУ ТУТ ЩЕ ОДИН ЗАМІР (знайшов агент CEO 21.09) ──────────────────
    # Він помітив, що «усього 400» і «за 7 днів 352» ТОЧНО ті самі, що тиждень
    # тому, і написав: «щоб і за 7 днів не змінилось при рухомому вікні — так не
    # буває. Або лічильник застиг, або рахується не те». Він мав рацію по суті,
    # але причина інша: `articles.json` має СТЕЛЮ (`MAX_ARTICLES` у парсері), і
    # потік Волині/України її забиває.
    #
    # 📐 Заміряно 21.09: потік — 348 статей, найстаріша додана **4.3 дні тому**;
    # Громада — 52 статті при власній квоті, найстаріша **56.8 дня тому**.
    # ➡️ Отже «за 7 днів» для потоку фізично не може бути правдою: файл стільки
    # не зберігає. Для Громади може — її квота не витісняється.
    #
    # 🛑 Тому числа не викидаю, а ПІДПИСУЮ. Мовчазне число, яке виглядає як обсяг
    # виробництва, а насправді є стелею файлу, — це те саме фальшиве
    # підтвердження, лише в цифрах.
    гром = [a for a in статті if "Громада" in str(a.get("geo", ""))]
    потік = [a for a in статті if "Громада" not in str(a.get("geo", ""))]

    def _глибина(група):
        """Скільки діб історії РЕАЛЬНО лежить у файлі для цієї групи."""
        додані = [a["added_ts"] for a in група if isinstance(a.get("added_ts"), (int, float))]
        if not додані:
            return None
        return round((datetime.now(timezone.utc).timestamp() * 1000 - min(додані)) / 86400000, 1)

    глибина_потоку = _глибина(потік)
    д["глибина_потоку_днів"] = глибина_потоку if глибина_потоку is not None else НЕ_ЗАМІРЯНО
    д["громади_у_файлі"] = len(гром)
    д["глибина_громади_днів"] = _глибина(гром) or НЕ_ЗАМІРЯНО
    if глибина_потоку is not None and глибина_потоку < 7:
        д["⚠️"] = (f"«статей_за_7_днів» НЕ міряє виробництво: файл тримає лише "
                   f"{глибина_потоку} дні потоку (стеля `MAX_ARTICLES`). Чесно "
                   f"міряється лише Громада — у неї власна квота.")
    return д
...
def аналітика():
    ...
    # 🔴 ПАРАМЕТРИ ЕКРАНУЄМО, А НЕ СКЛЕЮЄМО (вада знайдена 21.09 другим прогоном
    # агента). `isoformat()` дає «+00:00», а ПЛЮС в адресі означає ПРОБІЛ — база
    # отримувала «…517217 00:00», не могла розібрати мітку часу і чесно віддавала
    # `HTTP 400`. Замір, який це доводить:
    #     що складаємо:    2026-09-14T20:25:37.517217+00:00
    #     що читає сервер: 2026-09-14T20:25:37.517217 00:00
    # 🔑 Лагоджу не один рядок, а СПОСІБ: `urlencode` екранує будь-який параметр,
    # тож наступний доданий фільтр не наступить на те саме. Латка на «+» лишила б
    # ту саму пастку відкритою для кожного нового поля.
    запит = f"{url}/rest/v1/analytics_events?" + urllib.parse.urlencode({
        "select": "event_type", "created_at": f"gte.{від}", "limit": "10000",
    })
