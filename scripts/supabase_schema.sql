...
-- (модерація → admin підвищить до 'published')
CREATE POLICY "Anyone can submit a pending post"
  ON posts FOR INSERT
  WITH CHECK (status = 'pending');

-- Тільки admin (з whitelist у таблиці admins) може оновлювати (тобто approve/reject)
CREATE POLICY "Admins can update posts"
  ON posts FOR UPDATE
  USING (auth.email() IN (SELECT email FROM admins))
  WITH CHECK (auth.email() IN (SELECT email FROM admins));

-- Admin може видалити пост
CREATE POLICY "Admins can delete posts"
  ON posts FOR DELETE
  USING (auth.email() IN (SELECT email FROM admins));

-- Admin бачить ВСІ пости (включно pending) — окремий SELECT policy
...
-- Публічно видно тільки активну і непротухлу рекламу
...
-- Будь-хто може писати у ad_events (для трекінгу показів/кліків)
...
-- Тільки існуючі адміни можуть бачити/міняти список адмінів
...
CREATE POLICY "Admins can delete photos"
  ...
  USING (bucket_id = 'community-photos' AND auth.email() IN (SELECT email FROM admins));
