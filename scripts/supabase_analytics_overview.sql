...
-- ══════════════════════════════════════════════════════════════════════════════
-- ЖУРНАЛ ЗБОЇВ З ІМЕНАМИ: `admin_failures(p_period)` ✅ НАКАТАНО 26.08.2026
--
-- 🗣️ Замовлення Вови: «перероби так, щоб ці всі баги я міг скопіювати однією кнопкою.
-- І позначай імʼя акаунта, на якому це сталося… якщо в двох акаунтів, то ще імʼя
-- акаунта… і кількість їх».
--
-- 🛑 ЦЕ СКАСОВУЄ ПОПЕРЕДНЄ СВІДОМЕ РІШЕННЯ. У `logDbRefusal` стояло: «пишемо випадковий
-- номер пристрою, а не акаунт… прив'язка до акаунта нічого не додала б до діагнозу і
-- зробила б журнал збоїв журналом людей». Аргумент чинний, але власник вирішив інакше:
-- без імені він не може ні відтворити ваду, ні переслати її на виправлення.
-- 📐 Заміряно: у ВСІХ збоїв у базі стояло `акаунтів: 0` — імені не було з чого взяти.
-- ⚠️ Тому разом із цією міграцією змінено і КЛІЄНТ: `logDbRefusal` та `js_error` тепер
-- пишуть `analyticsUid() || getAnonId()`. Місток `setAnalyticsUid()` наповнює `auth.js` —
-- прямий виклик `currentUserId()` дав би кільце імпортів.
--
-- 🔑 ГРУПУЄМО ЗА ПОЛОМКОЮ, А НЕ ЗА ЛЮДИНОЮ: ключ це вид + код + текст + місце; імена —
-- доповнення, а не заголовок. Питання лишається «що зламано і в скількох».
-- ⚠️ Перша редакція впала з `subquery uses ungrouped column`: імена бралися підзапитом,
-- який тягнувся до `meta` вже ПІСЛЯ групування. Полагоджено нормалізацією в CTE — обидва
-- зрізи (по поломці й по акаунту) рахуються від одних і тих самих чотирьох полів, тож
-- розійтись не можуть.
--
-- Віддає: total · groups[] { kind, code, message, where, count, guests, accounts_n,
--          accounts[{name,count}], first_at, last_at }.
-- ✅ Перевірено транзакцією з відкотом: під адміном 4 групи / 14 випадків; без адміна
--    {"error":"not admin"}.
