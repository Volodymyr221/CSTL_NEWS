...
-- 🔴 24.09.2026 — НАЗВА ПОЛІТИКИ В ЦЬОМУ ПЛАНІ НЕ ЗБІГАЄТЬСЯ З ПРОДОМ.
-- Звірка з живою базою: політики «Public can read published posts» там НЕМАЄ.
-- Є одна політика читання — `posts read`:
...
-- Тобто три політики з плану нижче давно зведені в одну, і «зняти публічну» —
-- означає переписати `posts read`, а не видалити окремий рядок.
-- ⚠️ Хто піде знімати за старою назвою — не знайде її, вирішить, що вже зроблено,
--    і лишить діру відкритою. Саме тому цей абзац тут.
--
-- 📐 ЗАМІР 24.09 ВІД РОЛІ `anon` (свіжіший за той, що в шапці файлу):
--    `set role anon; select count(*), count(contact) from posts;` → 12 і 6.
--    Тобто шість телефонів, а не чотири: діра не стоїть на місці, вона росте
--    разом із Дошкою.
--
-- 🔑 ЧАСТИНА А ВЖЕ НА ПРОДІ. `get_post_contact()` у базі є (SECURITY DEFINER,
--    search_path виставлений) — шапка файлу з «⛔ НЕ НАКАТАНО» застаріла.
--    Лишилась саме ЧАСТИНА Б: закрити колонку.
--
-- Після переписування `posts read` під вітрину лишається те саме питання: ПОКУПЕЦЬ у
