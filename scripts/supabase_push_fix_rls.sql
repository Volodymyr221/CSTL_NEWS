...
--
-- 🛑 ЦЕЙ ФАЙЛ БІЛЬШЕ НЕ ДЗЕРКАЛИТЬ ПРОД — НЕ НАКОЧУВАТИ (01.08.2026).
--    Політики нижче (`USING (true)` для anon) ЗАМІНЕНО міграцією
--    `push_subscriptions_own_rows_only`. Причина: `USING true` на DELETE
--    означало, що будь-хто може видалити ЧУЖІ рядки — тобто стерти всі
--    підписки на автобусні сповіщення одним запитом.
--    ➡️ Чинний стан: `scripts/supabase_anon_lockdown.sql` секція 3.
--    Якщо накотити цей файл — дірка відкриється назад.
-- ============================================================================
