...
const VOLATILE_ALWAYS = [
  ...
  // ⚠️ ГІЛКА — ТЕЖ САМОПОСИЛАЛЬНА, і це знайшлось із ТРЕТЬОГО заходу (26.09).
  // Той самий цикл, що з хешем і номером PR, лише поверхом вище. Потік у CSTL
  // двогілковий: пульс ЗАВЖДИ пишеться на робочій гілці і ЗАВЖДИ їде в `main`
  // окремим PR. Тобто в мить запису тут стоїть `vova/щось`, а після злиття
  // сторож дивиться з `main` — і бачить розбіжність, якої не існує:
  //   пише `vova/X` → мерджимо → на `main` гілка вже `main` → пульс «відстав».
  // 📐 Заміряно: повний прогін 26.09 дав 179/180, і єдиним червоним був саме
  // цей рядок; перегенерація його не лікує, бо наступне ж злиття повторює те
  // саме. Сторож, який червоніє після КОЖНОГО деплою і не може стати зеленим
  // у звичайному потоці, перестають читати — а це гірше за його відсутність,
  // і в цьому файлі так сказано вже двічі.
  // 🔑 Рядок лишається у файлі як інформація (наступний чат читає, де лежить
  // робота) — летючою стає лише ЗВІРКА, рівно як для хеша і номера PR.
  /^\| гілка \|.*$/m,
];
const VOLATILE_SOFT = [
