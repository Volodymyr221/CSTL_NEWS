...
console.log('\n── Порожнє «Куди» = кінцева зупинка ──');

// 🔴 16.08 — скарга Вови: «вибираю Дерно → Рівне — закладку підсвічує; вибираю
// просто Дерно, кінцеву не чіпаю — той самий рейс уже НЕ підсвічує».
// Корінь: у запис писалось `toStop || null`, тобто те, що людина натиснула, а не
// те, що це ОЗНАЧАЄ. Порожнє «Куди» = «до кінця маршруту» — застосунок це вже
// знав (`getEffectiveTo`), але тільки для розрахунку часу.
check('сегмент нормалізується до фактичних зупинок',
  /function normSegment\(/.test(buses) &&
  /from: from \? byName\(from\) : stops\[0\]\.name/.test(buses) &&
  /to:\s+to\s+\? byName\(to\)\s+: stops\[stops\.length - 1\]\.name/.test(buses),
  'немає нормалізації — «Дерно→порожньо» і «Дерно→Рівне» знову стануть різними');

check('пошук збереженого рейсу порівнює нормалізовані зупинки',
  /function findTrackedEntry[\s\S]{0,400}normSegment\(route, t\.boardingStop, t\.alightingStop\)/.test(buses),
  'порівняння знову буквальне — старі записи з null не знайдуться');

check('порівняння зупинок стійке до варіантів написання',
  /const sameStop = \(a, b\) => normalizeStopName/.test(buses),
  'пряма рівність рядків: «Хорлупи пов.» проти «Хорлупи» дадуть різні сегменти');

check('у запис зберігаються фактичні зупинки, а не null',
  /const seg\s+= normSegment\(route, fromStop \|\| null, toStop \|\| null\)/.test(buses),
  'знову пишемо null — вада повернеться, і серверу теж доведеться здогадуватись');
...
// 🔴 Жива помилка з журналу збоїв (Катерина, 09.09, 2 випадки):
//     42501 new row violates row-level security policy (USING expression)
// Унікальний індекс — (endpoint, route_id, track_date), БЕЗ власника, а
// `endpoint` — адреса ПРИСТРОЮ. Забутий рядок попереднього акаунта на
// цьому телефоні відправляв upsert у гілку DO UPDATE — і та падала на USING
// політики UPDATE. Людина не могла відстежити рейс УЗАГАЛІ.
//
// ✅ Поведінку ДОВЕДЕНО НЕ ТУТ, а експериментом на живій базі 17.09: симптом
//    відтворено слово в слово і показано, що після захоплення та сама послідовність
//    проходить. Звіт записано в `scripts/supabase_bus_push_claim.sql`. Цей файл стежить
//    лише за тим, щоб рішення не зникло з коду мовчки (межі — у кінці файлу).

const claimSql = read('scripts/supabase_bus_push_claim.sql');

check('функція захоплення endpoint збережена в репо',
  /create or replace function public\.claim_bus_push_endpoint/.test(claimSql),
  'рішення живе тільки в базі — після збою відновлювати ні з чого');

check('захоплення чіпає ЛИШЕ чужі рядки цього endpoint',
  /where endpoint = p_endpoint[\s\S]*?user_uuid is distinct from \(auth\.uid\(\)\)::text/.test(claimSql),
  'без цих двох умов SECURITY DEFINER-функція стає діркою в RLS');

check('захоплення закрите від аноніма',
  /grant execute on function public\.claim_bus_push_endpoint\(text\) to authenticated/.test(claimSql)
    && /auth\.uid\(\) is null/.test(claimSql),
  'гість зміг би стерти чужі підписки по відомому endpoint');

// Порядок важливий: захоплення ПЕРЕД записом. Після — означало б перший
// тап завжди втрачений і тост «не вдалось» при кожній зміні акаунта на пристрої.
const saveBody = saveFn ? saveFn[0] : '';
check('клієнт забирає endpoint собі ПЕРЕД записом підписки',
  /забратиАдресуПристрою\([\s\S]*?\.upsert\(/.test(saveBody),
  'перший тап після зміни акаунта на пристрої все одно впаде в 42501');

check('на 42501 клієнт повторює захоплення і запис',
  /rawError\?\.code === '42501'/.test(saveBody),
  'обрив мережі на захопленні — і людина без сповіщень без жодного шляху виправити');

// 🛑 Зворотне правило: захоплення не сміє йти в базу на КОЖЕН запис:
// `selfHealPushSubscriptions()` кличе збереження при кожному відкритті вкладки
// і на кожен відстежуваний рейс. Endpoint у сеансі один — вистачить разу.
check('захоплення не повторюється на кожен запис (пам’ять на сеанс)',
  /_busEndpointClaimed\.has\(endpoint\)/.test(supabase),
  'зайвий запит у базу на кожен рейс при кожному відкритті вкладки');

check('невдале захоплення НЕ запам’ятовується',
  /if \(r\.ok\) _busEndpointClaimed\.add\(endpoint\)/.test(supabase),
  'один обрив мережі — і до кінця сеансу захоплення вважається зробленим');
