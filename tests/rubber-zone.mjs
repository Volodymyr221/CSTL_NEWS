...
// ── 🛑 НИЖНІЙ КРАЙ — ТЕЖ ВИМКНЕНИЙ ──────────────────────────────────────────
// Тут стояли перевірки «знизу зона теж відтягується»: їх додали на питання Вови
// «а знизу віджетів громади чому гумка не працює?», і вони були зелені.
// Разом із самим відскоком вони втратили сенс — обидві межі тепер мовчать.
// Історія лишена, бо вона показує, що механіка була доведена до кінця, і знято
// її не через недоробку, а через плавність (див. блок вище).
