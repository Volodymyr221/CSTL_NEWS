...
// шапку меню (немає замку; лічильник версії саме тут) зеленітимуть на будь-якому коді.
...
// Іконка замку дублювала центральну кнопку таб-бару — прибрана, сторож стежить.
//
// 🔴 18.09 — ПРО ЛІЧИЛЬНИК ВЕРСІЇ ЦЯ ПЕРЕВІРКА РОЗВЕРНУТА НАВПАКИ.
// Було: «лічильника в меню НЕМАЄ — він уже стоїть у шапці». Замовлення Вови 18.09:
// лічильник ліз у кожен знімок екрана, а знімки йдуть у презентацію на грант і
// в рекламу — тому він переїхав сюди.
//
// 🔑 АЛЕ НАМІР СТОРОЖА ЛИШИВСЯ ТОЙ САМИЙ, і це головне. 10.08 він стеріг не
// «лічильника тут бути не може», а «його не може бути ДВА». Тому тепер міряємо
// саме це: лічильник у застосунку РІВНО ОДИН, і він у меню. Просто перевернути
// умову означало б втратити те, заради чого сторож заводили.
// ⚠️ Де він видимий і чи не замерз текст — `tests/version-stamp-place.mjs`.
...
ok('🔴 лічильник версії стоїть у шапці меню',
   !!шапка && шапка.версія, шапка ? '' : 'шапки меню немає');

// 🛑 Те, заради чого сторож існував із 10.08: лічильників не може бути ДВА.
// Два розійдуться (деплой підміняє рядок по шаблону) і показуватимуть різне —
// а це гірше за відсутність лічильника: два числа не дають відповіді «чи доїхав».
const лічильників = await p.evaluate(() => document.querySelectorAll('.deploy-stamp').length);
ok('🔴 лічильник у застосунку РІВНО ОДИН', лічильників === 1, `вузлів: ${лічильників}`);
...
// 🔴 10.08, друга редакція — ПІДВАЛ РІВНО ТАКОЇ Ж ВИСОТИ, ЯК ШАПКА.
// Замовлення Вови: «занадто вертикально високий… треба зробити максимально такої
// висоти, як верхній блок». Було 85 проти 71. Міряємо ПАРУ, а не число: якщо
// колись зміниться шапка, підвал мусить піти за нею, а не лишитись на 71.
// ⚠️ Тап-цілі кнопок міряються окремо нижче (44px, Apple HIG) — саме щоб ніхто не
// «полагодив» висоту, зменшивши кнопки.
const висоти = await p.evaluate(() => ({
  шапка: Math.round(document.querySelector('.sidebar-head').getBoundingClientRect().height),
  підвал: Math.round(document.getElementById('sidebar-foot')?.getBoundingClientRect().height || 0),
}));
ok('🔴 підвал такої ж висоти, як шапка (не важчий за неї)',
   висоти.підвал > 0 && Math.abs(висоти.підвал - висоти.шапка) <= 1,
   `шапка ${висоти.шапка} · підвал ${висоти.підвал}`);

// 🆕 11.08 — КРАЇ МЕНЮ: тінь на обох смугах + заокруглений верх підвалу.
// Замовлення Вови: «добав верхньому блоку та нижньому блоку трохи тіні… верхній
// край нижнього блоку заокругли, так як… тапбар».
// 🔑 Радіус міряємо ПАРОЮ з таб-баром, а не числом «20px»: Вова назвав його
// взірцем, тож правильна вимога — «як у таб-барі», і вона мусить лишитись
// правдою, навіть якщо таб-бар колись перемалюють. Те саме, що з висотою підвалу.
const краї = await p.evaluate(() => {
  const тінь = s => getComputedStyle(document.querySelector(s)).boxShadow;
  const рад = s => getComputedStyle(document.querySelector(s)).borderRadius;
  return { шапкаТінь: тінь('.sidebar-head'), підвалТінь: тінь('#sidebar-foot'),
           підвалРадіус: рад('#sidebar-foot'), таббарРадіус: рад('.tab-bar') };
});
ok('🆕 у шапки меню є тінь', краї.шапкаТінь !== 'none' && /rgba/.test(краї.шапкаТінь), краї.шапкаТінь);
ok('🆕 у підвалу меню є тінь', краї.підвалТінь !== 'none' && /rgba/.test(краї.підвалТінь), краї.підвалТінь);
// 🔴 04.09 — КРИТЕРІЙ ПЕРЕПИСАНО НА ЗНАК ЗСУВУ, І ЦЕ ВИПРАВЛЕННЯ СТОРОЖА, НЕ ПОСЛАБЛЕННЯ.
// Стояло `/-4px/` і `/0px 4px/` — тобто перевірка трималась за ТОЧНІ ПІКСЕЛІ тіней,
// які були в коді того дня. Вимога ж у самій назві перевірки інша й ширша: підвал
// світить УГОРУ, шапка — УНИЗ. Дизайн-аудит 03.09 звів тіні в шкалу `--elev-*`, шапка
// дістала двошарову `0 2px 4px + 0 8px 20px` — обидва шари вниз, вимога ціла, а сторож
// почервонів. Тепер міряється те, що перевірка й обіцяла: ЗНАК вертикального зсуву
// кожного шару. Стеля старіє з кожним рішенням про вигляд, напрямок — ні.
// ⚠️ Шар тіні це ТРИ-ЧОТИРИ довжини поспіль (x · y · розмиття · розтяг), тож брати
// «кожну другу довжину» не можна — пара «розмиття + розтяг» прочиталась би як ще
// один шар із зсувом 0. Ловимо шар цілком і беремо з нього ДРУГЕ число.
const зсувиY = т => [...т.matchAll(/(-?[\d.]+)px\s+(-?[\d.]+)px\s+(-?[\d.]+)px(?:\s+(-?[\d.]+)px)?/g)]
  .map(m => parseFloat(m[2]));
const підвалY = зсувиY(краї.підвалТінь), шапкаY = зсувиY(краї.шапкаТінь);
ok('🆕 тінь підвалу спрямована ВГОРУ, а шапки — ВНИЗ (не одна й та сама)',
   підвалY.length > 0 && шапкаY.length > 0 &&
   підвалY.every(y => y < 0) && шапкаY.every(y => y > 0),
   `підвал ${підвалY.join('/')}px · шапка ${шапкаY.join('/')}px`);
ok('🆕 верх підвалу заокруглено ТАК САМО, як таб-бар (взірець Вови)',
   краї.підвалРадіус === краї.таббарРадіус,
   `підвал ${краї.підвалРадіус} · таб-бар ${краї.таббарРадіус}`);
...
// ⚠️ 11.08: «Обговорення» → «Питання». Не косметика — вкладка перейшла з моделі
// «тема → чат» на «питання → відповіді», і підпис у меню мусить збігатися з
// таб-баром (перевірку на це робить сам стенд рядком вище).
// 🆕 17.08 — у групі зʼявились «Збори» (розділ громади з формою заявки).
// ⚠️ Перевірку НЕ послаблено до «містить»: точний список — це і є її сенс.
// Вона ловить два різні зриви, і обидва тихі: (1) пункт зник із меню разом із
// чужою правкою `SECTIONS`; (2) пункт заїхав СЮДИ, хоча за змістом належить до
// «Моє» — і людина шукає його не там. Тому список лишається точним, а нова
// назва додається СВІДОМО, разом із рішенням про місце пункту.
// 🔑 «Збори» стоять ПІСЛЯ «Новин», у кінці: це наймолодший розділ, а порядок
// перших шести дзеркалить таб-бар — його чіпати не можна (записане рішення 10.08).
ok('перша група — вкладки таб-бару + Новини + Збори, Громада першою',
   звірка.порядок.join(' · ') === 'Громада · Стрічка · Питання · Дошка · Автобуси · Новини · Збори',
   звірка.порядок.join(' · '));
...
const контраст = await p.evaluate(() => {
  ...
  const out = [];
  // ⚠️ `.sb-social-cap` прибрано зі списку 10.08 разом із самим підписом «Слідкуйте
  // за нами» — посилання на неіснуючий селектор мовчки нічого не перевіряє.
  for (const sel of ['.sb-cap', '.sb-card--me .sb-card-sub', '.sb-card--admin .sb-card-sub']) {
