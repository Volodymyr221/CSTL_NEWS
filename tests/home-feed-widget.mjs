...
// Картка під рядом — допис ПЕРШОЇ спільноти, тобто ряд і картка не розходяться.
ok('картка показує допис першої спільноти в ряду',
   зріз.авторКартки === ОЧІКУЮ[0], зріз.авторКартки);
