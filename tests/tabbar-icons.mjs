...
//
// ⚠️ ТОВЩИНА ЛІНІЇ РАХУЄТЬСЯ З УРАХУВАННЯМ `scale`, хоча самого `scale` в іконках
// уже немає (він заборонений окремою перевіркою нижче). Рахунок лишається як
// СТРАХОВКА: якщо колись повернуть групу з масштабом, видима товщина = `stroke-width`
// × масштаб, і перевірка самого лише атрибута пропустила б рівно ту помилку, від
// якої тут захист (1.75 × 1.08 = 1.89 — жирніше за сусідні).
//
...
//   HTML_REV=6c35e8ca node tests/tabbar-icons.mjs   → 16/18, cy=10.7 (зміщена вгору)
//   HTML_REV=e9e042cf node tests/tabbar-icons.mjs   → 17/18 (трималась на scale)
...
const icons = await p.evaluate(() => {
  ...
  document.querySelectorAll('.tab-bar .tab-item:not(.tab-item--home)').forEach(item => {
    ...
    out.push({
      ...
      hasTransform: !!svg.querySelector('[transform]'),
    });
  });
  return out;
});
...
// 2b. 🔴 ЖОДНОГО `transform` УСЕРЕДИНІ ІКОНКИ.
// `scale` множить не лише геометрію, а й товщину лінії — і знак мовчки стає
// жирнішим за сусідні. Одна редакція іконки «Питання» трималась саме на
// `scale(1.08)` з ручною компенсацією `stroke-width`; конструкція робоча, але
// крихка: досить комусь поправити масштаб і не зачепити компенсацію.
// ➡️ Правило простіше і надійніше: координати одразу фінальні.
ok('жодна іконка не тримається на transform', icons.every(i => !i.hasTransform),
   icons.filter(i => i.hasTransform).map(i => i.tab).join(', ') || 'чисто');
