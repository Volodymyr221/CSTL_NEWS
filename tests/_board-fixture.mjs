      ...
      const q = (table) => {
        ...
        const self = {
          then(res) {
            ...
            // 🔴 25.09 — ЗАТРИМКА, НАЗВАНА НА ТАБЛИЦЮ posts, МУСИТЬ ЛОВИТИ Й ВІТРИНУ.
            // Стенди сповільнюють джерело, щоб побачити екран ОЧІКУВАННЯ (кістяк).
            // Коли Дошку перевели на posts_public, правило slow з ключем posts
            // перестало на неї діяти, відповідь почала приходити миттєво — і
            // skeleton-shapes почервонів на СПРАВНОМУ застосунку: кістяка Дошки
            // просто не встигало бути видно. Стенд стеріг НАЗВУ, а не джерело.
            // Тому вітрина успадковує затримку таблиці, з якої виведена; окремий
            // ключ posts_public у SLOW, якщо він є, лишається головнішим.
            //
            // ⚠️ І ВДРУГЕ ЗА ДВА ДНІ ТУТ ЖЕ: зворотні лапки в цьому коментарі
            // ламають файл, бо весь цей блок живе всередині шаблонного рядка.
            // Тому назви таблиць тут пишуться голими, без жодного оформлення.
            const ms = SLOW[table] ?? (table === 'posts_public' ? SLOW.posts : 0) ?? 0;
            const pr = ms
