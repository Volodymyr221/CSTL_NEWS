...
const sql = projectFile('scripts/supabase_visibility_model.sql')
  ...
  // 24.08 — третя міграція тієї самої сімʼї: політика ЧИТАННЯ ламала не
  // вставку (як 22.08), а мʼяке ВИДАЛЕННЯ. Див. блок «свій видалений рядок».
  + '\n' + projectFile('scripts/supabase_soft_delete_visibility.sql');
...
// ── 🔴 24.08 — АВТОР МУСИТЬ БАЧИТИ СВІЙ ВЛАСНИЙ ВИДАЛЕНИЙ РЯДОК ─────────────
//
// СКАРГА ВОВИ З ПРОДА: «написав відповідь на питання і не можу видалити».
// Тост казав «Текст порожній» (антиспам на порожньому тексті), але то був лише
// ВЕРХНІЙ шар. Доведено виміром на живій базі:
//     update … set author = author      → ПРОЙШЛО
//     update … set deleted_at = now()   → ВПАЛО, 42501 RLS
// Тобто падало саме `deleted_at`: ставлячи мітку, автор робив рядок невидимим
// ДЛЯ СЕБЕ за політикою читання (`deleted_at is null`), і RLS відхиляв UPDATE.
//
// 🔑 ТА САМА СІМʼЯ, ЩО 22.08 — «політика ЧИТАННЯ ламає ЗАПИС». Там був
// `INSERT … RETURNING`, тут звичайний `UPDATE`. Тому перевірка живе поруч.
//
// 🛑 МАСШТАБ БУВ ШИРШИЙ ЗА СКАРГУ: так само мовчки не працювало видалення
// коментаря у «Стрічці» (`page_comments`) — про це не знав ніхто. Не зламаним
// лишався тільки `page_posts`, і саме він дав зразок правильного правила
// (`can_edit_page` бачить свій рядок навіть видаленим).
//
// ⚠️ Міряємо ФОРМУ (текст міграції), і це свідома межа: стенд не має доступу до
// живої бази. Наслідок доводився вручну транзакціями з `rollback` — записано в
// шапці `scripts/supabase_soft_delete_visibility.sql`. Форма тут ловить
// РЕГРЕС (хтось перепише політику назад), а не первинну ваду.
{
  const свіжі = останніПолітикиSelect(sql);
  const власник = { comments: /sender_uid = auth\.uid\(\)/, page_comments: /author_uid = auth\.uid\(\)/ };
  for (const [табл, re] of Object.entries(власник)) {
    const п = свіжі.filter(p => p.табл === табл).pop();
    ok(`🔴 «${табл}»: автор бачить СВІЙ видалений рядок (інакше видалення падає з 42501)`,
       !!п && re.test(п.вираз), п ? п.вираз.slice(0, 90) : '(політики немає)');
  }
}

// Момент видалення не має проганятись через антиспам: рядок цим самим
// оператором стає невидимим для інших, перевіряти в ньому нема чого.
// (Без цього `text: ''` від клієнта падав ще до RLS — це був верхній шар вади.)
ok('🔴 мʼяке видалення не проганяється через антиспам',
   /new\.deleted_at is not null and old\.deleted_at is null/.test(sql));

// 🛑 І окремо — щоб виправлення не перетворилось на дірку: ЧУЖЕ видалене
// лишається невидимим. Умова мусить бути прив'язана до `auth.uid()`, а не
// «пускати всіх залогінених».
ok('🛑 пускаємо саме АВТОРА, а не будь-кого залогіненого',
   !/deleted_at is not null and auth\.uid\(\) is not null/.test(sql)
   && !/or auth\.role\(\) = 'authenticated'/.test(sql));
