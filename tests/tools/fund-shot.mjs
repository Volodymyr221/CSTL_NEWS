...
console.log(JSON.stringify(await p.evaluate(() => {
  ...
  return {
    ...
    фактів: sec.querySelectorAll('.hm-fund-fact').length,
    кнопка: go?.textContent.replace(/\s+/g,' ').trim(),
    // Напис у кнопці мусить лишатись В ОДИН РЯДОК: перенос посеред бордової
    // плашки виглядає як зламана верстка. Міряємо ВИСОТОЮ тексту, а не довжиною
    // рядка — це наслідок, а не припущення про ширину шрифту.
    кнопкаОдинРядок: (() => {
      const t = sec.querySelector('.hm-fund-go-tx');
      if (!t) return null;
      const lh = parseFloat(getComputedStyle(t).lineHeight) || 16;
      return t.getBoundingClientRect().height < lh * 1.6;
    })(),
