...
const blocks = await page.evaluate(() => {
  ...
  // `hm-bg` — зафіксоване фото на фоні. Це не блок сторінки: воно не займає
  // місця в потоці і не зсуває нічого, тож у вимірі висот йому не місце.
  }).filter(b => b.h > 4 && !/hm-bg/.test(b.name));
});
