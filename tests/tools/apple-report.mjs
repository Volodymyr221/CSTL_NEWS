...
if (ЩО === 'material') {
  ...
  if (!склоНаСклі.size) console.log('   (жодної — матеріал на матеріал у застосунку не кладуть)');

  // Слабший випадок: тонування на тонуванні (без блюру). Скіл теж про це каже —
  // два напівпрозорі шари один на одному зʼїдають контраст тексту.
  const тонНаТоні = new Map();
  for (const [екран, д] of екрани) for (const m of д.матеріали) {
    if (!m.прозореНаПрозорому) continue;
    const k = m.вузол + ' ← ' + m.прозореНаПрозорому;
    if (!тонНаТоні.has(k)) тонНаТоні.set(k, { m, екран });
  }
  console.log(`\n── Напівпрозоре на напівпрозорому (без блюру) — ${тонНаТоні.size} пар ──`);
  n = 0;
  for (const [k, { m, екран }] of тонНаТоні) {
    if (n++ >= 12) break;
    console.log(`   ${k}   тло ${m.тло}  [${екран}]`);
  }

  // §12: «Bigger surfaces should read as thicker» — більша площа має нести
  // сильніший блюр. Перевіряємо, чи це так, чи блюр роздано без звʼязку з розміром.
  const пари = [];
  const бачив = new Set();
  for (const [, д] of екрани) for (const m of д.матеріали) {
    if (!m.blur || бачив.has(m.вузол)) continue;
    бачив.add(m.вузол);
    const px = parseFloat((m.blur.match(/blur\(([\d.]+)px\)/) || [])[1] || 0);
    пари.push({ вузол: m.вузол, px, площа: m.площа });
  }
  пари.sort((a, b) => b.площа - a.площа);
  console.log('\n── Блюр проти площі поверхні (скіл: більша поверхня = товщий матеріал) ──');
  for (const p of пари.slice(0, 12)) {
    console.log(`   площа ${String(p.площа).padStart(7)}px²   blur ${String(p.px).padStart(4)}px   ${p.вузол}`);
  }
}
...
if (ЩО === 'a11y') {
  ...
  // 🔴 ДВА НЕЗАЛЕЖНІ ДЖЕРЕЛА, і це не перестраховка.
  // Вимір браузером показав ✅ на трьох екранах для `prefers-reduced-transparency`,
  // хоча цієї медіа-фічі в стилях проєкту немає ЖОДНОЇ. Тобто Δ там — залишковий
  // шум (годинник, погода, живі лічильники), який один замір шуму не спіймав.
  // Тому вирок виносить ДЖЕРЕЛО (чи існує правило взагалі), а вимір лише каже,
  // де воно реально спрацьовує. Розбіжність між ними друкуємо вголос.
  const { readdirSync } = await import('fs');
  const STYLE = join(ROOT, 'style');
  const css = readdirSync(STYLE).filter(f => f.endsWith('.css'))
    .map(f => ({ f, t: readFileSync(join(STYLE, f), 'utf8') }));
  const ФІЧІ = ['prefers-reduced-motion', 'prefers-reduced-transparency', 'prefers-contrast'];
  console.log('\n── ДЖЕРЕЛО: чи існує правило в стилях узагалі ──');
  const уДжерелі = {};
  for (const ф of ФІЧІ) {
    const файли = css.filter(c => c.t.includes(ф));
    уДжерелі[ф] = файли.length;
    console.log(`   ${ф.padEnd(30)} ${файли.length ? `${файли.length} файл(и) з ${css.length}: ` + файли.map(c => c.f).join(', ') : '🔴 0 файлів — НЕ ІСНУЄ В ПРОЄКТІ'}`);
  }

  console.log('\n── ВИМІР: Δ = скільки вузлів змінили обчислені стилі ──');
  console.log('«шум» = те саме порівняння стану З САМИМ СОБОЮ. Δ ≤ шум = підтримки немає.\n');
  let провалів = 0, розбіжностей = 0;
  for (const [екран, д] of екрани) {
    const p = д.prefers || {};
    const ряд = ФІЧІ.map(ф => {
      const k = ф.replace('prefers-', '');
      const δ = p[k]?.змінилось ?? '?';
      const живе = typeof δ === 'number' && δ > (p.шум || 0);
      if (!живе) провалів++;
      let мітка = живе ? '✅' : '❌';
      if (живе && !уДжерелі[ф]) { мітка = '⚠️ Δ є, а правила в CSS НЕМАЄ → шум'; розбіжностей++; }
      return `${k.padEnd(22)} Δ${String(δ).padStart(3)} ${мітка}`;
    });
    console.log(`${екран}\n   шум ${p.шум}\n   ${ряд.join('\n   ')}`);
  }
  console.log(`\nПровалів (Δ не перевищує шум) — ${провалів} з ${екрани.length * 3}`);
  console.log(`Розбіжностей «вимір каже є, джерело каже немає» — ${розбіжностей} (усі читати як шум)`);
}
