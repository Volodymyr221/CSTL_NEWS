...
const надБаром = await p.evaluate(() => {
  ...
  if (!dd || !бар) return null;
  return { низСписку: Math.round(dd.getBoundingClientRect().bottom),
           верхБару: Math.round(бар.getBoundingClientRect().top) };
});
ok('🔴 низ списку НАД таб-баром (останні зупинки не ховаються під ним)',
   !!надБаром && надБаром.низСписку <= надБаром.верхБару,
   надБаром ? `низ ${надБаром.низСписку}px · верх бару ${надБаром.верхБару}px` : 'нема');
