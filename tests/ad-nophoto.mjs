...
const shape = () => p.evaluate(() => {
  ...
  return {
    ...
    ціна_у_рядку_фактів: !!q('.cm-ad-meta .cm-ad-price--quiet'),
    хвіст_унизу: (() => {
      const saf = q('.cm-ad-safety'), rep = q('.cm-ad-report'), bar = q('.cm-ad-bottom');
      if (!saf || !rep || !bar) return null;
      return Math.round(rc(bar).top - rc(rep).bottom);
    })(),
    порожнеча_після_опису: (() => {
      const t = q('.cm-ad-text'), saf = q('.cm-ad-safety'), au = q('.cm-ad-author');
      const last = au || t;
      return (last && saf) ? Math.round(rc(saf).top - rc(last).bottom) : null;
    })(),
...
// Скарга Вови (IMG_3819): «зливається в одну площину». «Договірна» була окремим рядком —
// тобто подією, — хоч це такий самий факт, як місце й дата. Тепер вона в рядку фактів.
ok('«Договірна» стоїть у рядку фактів, а не окремим рядком',
   sNo.ціна_у_рядку_фактів === true, String(sNo.ціна_у_рядку_фактів));
// Скарга Вови: «коли мало інформації, нижня частина виглядає дуже пустою». Заміряно
// було 302px білого ПІСЛЯ «Поскаржитися». Порожнеча не зникає (вмісту справді мало) —
// вона переїжджає ТУДИ, ДЕ ЇЙ МІСЦЕ: службовий хвіст притиснуто до низу сторінки.
ok('службовий хвіст притиснуто до низу — під ним не лишається порожнечі',
   sNo.хвіст_унизу !== null && sNo.хвіст_унизу < 40, `${sNo.хвіст_унизу}px під «Поскаржитися»`);
