...
const ALLOW = [
  ...
  'hm-ftrack',
  // 🆕 20.09 — капсула автобуса з ДВОМА боками маршруту (замовлення Вови: «якщо
  // через цю локацію проходить рейс у дві сторони, він має відображатися в дві
  // сторони, слайдом»). Гортання вбік тут і є задумом.
  // ⚠️ ЧОМУ ЦЕЙ ВИНЯТОК БЕЗПЕЧНІШИЙ ЗА `hm-ntrack`: капсула заввишки **56px** —
  // того ж класу, що чіпи (32px), а не 300px стрічка, через яку 31.07 прибирали
  // вкладений скролер новин. Вертикальний жест такою смужкою не вкрасти.
  // `overflow-y: hidden` у ній однаково стоїть — правило те саме для всіх
  // горизонтальних доріжок цього застосунку.
  // 🔑 Доріжка зʼявляється ЛИШЕ коли боків справді два (стенд `home-caps`
  // стереже це окремо: один бік → звичайна капсула без доріжки і крапок).
  'hm-capsl-track',
];
