...
const ALLOW = [
  ...
  // 🆕 01.08 — ряд типів угоди на Дошці (Усі / Продам / Куплю / …). Той самий
  // клас випадків, що `cm-cat-row` і `bm-cat-row` вище: це стрічка чіпів-фільтрів,
  // де гортання вбік і є задумом — категорій до восьми, у 390px вони не вміщаються.
  // ⚠️ Вертикальний жест він не краде (висота 32px, напрям браузер розрізняє), тож
  // це НЕ той випадок, через який 31.07 прибрали вкладений скролер новин: там
  // скролер був заввишки 465px і забирав саме вертикальну прокрутку сторінки.
  'bd-types',
  // 🆕 03.08 (потік «ПУЛЬС») — горизонтальні стрічки головного екрана:
  // «Що нового» (новини) і «Поруч» (події). Задумані саме стрічками — це і є
  // спосіб показати кілька новин, не роблячи з них стіни на пів екрана.
  // ⚠️ Умови, за яких вони лишаються здоровими (сторожить `tests/community-home.mjs`):
  // не більше 8 карток і завжди видимий вихід «усі →». Без цього стрічка
  // перетворюється на прихований список — саме той дефект, який 31.07 прибрали
  // з віджета новин (6468px вмісту у вікні 465px).
  'hm-rail',
];
