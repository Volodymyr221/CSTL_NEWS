...
// ── 3. У ШАПЦІ ЛІЧИЛЬНИКА БІЛЬШЕ НЕМАЄ ──────────────────────
// 🔴 18.09 — лічильник версії переїхав у бургер-меню (замовлення Вови: він
// ліз у кожен знімок екрана, а знімки йдуть у презентацію й рекламу).
// Тут стережемо рівно те, що стосується шапки: його в ній немає.
// Де він має бути і чи читається його текст — `tests/version-stamp-place.mjs`.
const штампУШапці = await p.evaluate(() =>
  !!document.querySelector('.app-header .deploy-stamp, header .deploy-stamp'));
ok('🔴 лічильника версії в шапці немає', !штампУШапці,
   штампУШапці ? 'він повернувся в шапку — буде на кожному знімку' : '');
