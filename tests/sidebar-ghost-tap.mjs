...
await page.evaluate(() => document.getElementById('sidebar-toggle').click());
await page.waitForTimeout(700);
// ⚠️ 17.08 — тапаємо по САМОМУ затемненню, а не по числах (30, 500).
// Тут меню перед тапом відкрите, тож координати спрацювали б; але рівно таким
// рядком мовчки зламався `sidebar-overlay`: там меню на той момент було вже
// закрите, тап летів у живий контент новин і відкривав хаб. Числа в координатах
// нічого не знають про те, що під ними лежить, — тому їх тут більше немає.
