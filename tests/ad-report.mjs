...
await p.route('**/vendor/supabase-js*', r => r.fulfill({
  contentType: 'text/javascript', body: '/* підставний клієнт нижче */',
}));

// ── ЧОМУ ТУТ ПІДСТАВНИЙ КЛІЄНТ SUPABASE ─────────────────────────────────────
// Застосунок бере `supabase-js` з нашої теки `vendor/` (`index.html`). У пісочниці він
// тепер завантажується успішно, тому вище ми його ГЛУШИМО — інакше він перезаписав би
// підставного клієнта. Без підстановки будь-яка дія «для залогінених» упирається в
