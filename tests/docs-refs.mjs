...
const KNOWN_MISSING = new Map([
  ...
  // 🗑 04.09 — фронтова «афіша» старого світу. 270 рядків, 0 імпортерів, у
  // збірку не потрапляла ВЗАГАЛІ (`grep CATEGORY_COLORS bundle.js` → 0). Читала
  // `data/events.json`, тобто світ, з якого події спільнот прибрали 04.09.
  // 🔴 Урок не про розмір, а про ПЛАН: коментар в `app.js` казав «лишається для
  // Етапу 6 — Афіша громади», і через нього наступна сесія (і я сам) думала, що
  // афіша вже почата, і латала віджети замість того, щоб добудувати місце.
  // ⚠️ Живе місце подій тепер — `openEventsScreen()` у `src/tabs/feed.js`.
  // 🛑 `data/events.json` і `data/holidays.json` НЕ чіпані: їх пише
  // `scripts/parse_rss.py` і читає ШІ-редактор (`editor/sources/calendar.py`).
  ['src/tabs/events.js',           'ВИДАЛЕНО 04.09 — мертвий фронтовий модуль афіші (0 імпортерів, не був у збірці); живе місце подій — openEventsScreen() у src/tabs/feed.js'],
  ['events.js',                    'те саме, скорочена форма'],
