...
// Дві адреси однієї статті: перша — та, що дає RSS і що редиректить; друга — та,
// на якій стаття живе насправді. Людині треба показати ДРУГУ.
const АДРЕСА_ЗАПИТУ  = 'https://www.pravda.com.ua/news/2026/08/29/8050955/';
const АДРЕСА_КІНЦЕВА = 'https://www.eurointegration.com.ua/news/2026/08/29/7218888/';
const ДОВГА  = 'https://example.com/' + 'aB9xQ7zK2mN5pR8sT1vW4yZ6cE3gJ0hL'.repeat(3);
const АДРЕСА_БЕЗ_РЕДИРЕКТУ = 'https://konkurent.ua/publication/123456/';
let idСтатті = null;
let idБезПоля = null;
...
await page.route('**/data/articles.json', async r => {
  ...
  idСтатті = arts[0].id;
  // 🔴 29.08 — КУДИ ВЕДЕ «ЧИТАТИ ОРИГІНАЛ». `sourceUrl` це адреса ЗАПИТУ, і в
  // мережі «Правди» вона редиректить на інше видання. Парсер тепер кладе кінцеву
  // адресу окремим полем; тут ставимо їх РІЗНИМИ, щоб перевірка могла впасти.
  arts[0].sourceUrl = АДРЕСА_ЗАПИТУ;
  arts[0].finalUrl  = АДРЕСА_КІНЦЕВА;
  // Друга стаття — звичайний випадок: редиректу не було, поля `finalUrl` немає.
  idБезПоля = arts[1].id;
  arts[1].sourceUrl = АДРЕСА_БЕЗ_РЕДИРЕКТУ;
  delete arts[1].finalUrl;
  arts[0].content =
...
// ── КУДИ ВЕДУТЬ ВИХОДИ НА ДЖЕРЕЛО ────────────────────────────────────────
// 🔴 Три місця статті ведуть назовні: підпис джерела в шапці, «Читати повністю»
// (коли RSS дав лише анонс) і «Читати оригінал» унизу. Усі три мусять брати
// КІНЦЕВУ адресу — інакше підпис каже одне видання, а браузер відкриває інше.
// 🛑 Перевіряємо саме всі три, а не одну: 27.08 виправлення підпису зробили в
// одному місці, і рівно тому решта лишилась із старою поведінкою.
const виходи = await page.evaluate(() => {
  const h = с => { const a = document.querySelector(с); return a ? a.getAttribute('href') : null; };
  return {
    підпис:   h('.article-byline-link'),
    оригінал: h('.article-source-link'),
    повністю: h('.article-short-link'),   // може не малюватись — стаття повна
  };
});
ok('🔴 підпис джерела веде на КІНЦЕВУ адресу, не на адресу запиту',
   виходи.підпис === АДРЕСА_КІНЦЕВА, String(виходи.підпис));
ok('🔴 «Читати оригінал» веде на КІНЦЕВУ адресу',
   виходи.оригінал === АДРЕСА_КІНЦЕВА, String(виходи.оригінал));
ok('🛑 і жоден вихід не лишився на адресі запиту',
   !Object.values(виходи).some(v => v === АДРЕСА_ЗАПИТУ), JSON.stringify(виходи));

// ⚠️ ЗАПАСНИЙ ВАРІАНТ — ОКРЕМОЮ СТАТТЕЮ, А НЕ НА СЛОВО.
// `finalUrl` парсер кладе ЛИШЕ коли редирект справді був, тобто у переважної
// більшості статей поля НЕМАЄ. Без цієї сцени правка «беремо тільки finalUrl»
// пройшла б зеленою і лишила ті статті взагалі без виходу на джерело — вада,
// гірша за ту, яку лікуємо. Друга стаття у підміні має лише `sourceUrl`.
await page.evaluate(id => window.location.hash = `#/post/news/${id}`, idБезПоля);
await page.waitForTimeout(1200);
const запасний = await page.evaluate(() => {
  const a = document.querySelector('.article-source-link') || document.querySelector('.article-byline-link');
  return a ? a.getAttribute('href') : null;
});
ok('🔴 стаття БЕЗ finalUrl усе одно веде на джерело (запасний варіант)',
   запасний === АДРЕСА_БЕЗ_РЕДИРЕКТУ, String(запасний));
