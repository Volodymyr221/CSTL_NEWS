...
// ── ФІКСТУРА СТАТЕЙ ───────────────────────────────────────────────────────
// 🔑 По три новини на кожен гео-розділ: блок малює одну велику картку і до двох
// тихих рядків на сторінку, тож трьох рівно вистачає, щоб сторінка набралась
// повністю і жодна не лишилась недомальованою.
const _ЗАГОЛОВКИ = [
  'У громаді відкрили оновлену амбулаторію після капітального ремонту приміщень',
  'Школярі Олицької громади здобули призові місця на обласному конкурсі робіт',
  'Комунальники завершили ремонт дороги між двома селами громади цього тижня',
];
const ФІКСТУРА_СТАТЕЙ = GROUPS.flatMap((гео, gi) =>
  _ЗАГОЛОВКИ.map((заголовок, i) => ({
    id: 900000 + gi * 10 + i,
    title: заголовок,
    excerpt: 'Стислий опис події у кілька речень, щоб картка мала звичайний обсяг тексту.',
    content: 'Повний текст новини для фікстури стенда.',
    category: 'Суспільство',
    geo: гео,
    // Фото лише в ПЕРШОЇ новини розділу: саме таке правило стереже перевірка
    // «картинок не більше однієї на сторінку».
    image: i === 0 ? 'https://example.invalid/fixture.jpg' : null,
    image_type: i === 0 ? 'source' : 'none',
    source: 'Фікстура',
    sourceUrl: 'https://example.invalid/news',
    ts: Date.now() - (gi * 10 + i) * 60000,
    added_ts: Date.now(),
  })));
...
async function openCommunity() {
  ...
  // 🔴 21.09 — СТАТТІ ПІДМІНЮЄМО ФІКСТУРОЮ, І ЦЕ ГОЛОВНА ПРАВКА СТЕНДА.
  //
  // 🔬 ЗАРАДИ ЧОГО. Стенд читав ЖИВИЙ `data/articles.json`, який парсер оновлює
  // щогодини. Через це висота віджета залежала від того, які сьогодні новини:
  // довші заголовки — вищий блок. 📐 Заміряно: у реєстрі він стояв записаний як
  // «74/75, сторож червоний», а 21.09 прогін дав **75/75** — сам позеленів, бо
  // змінився контент. Те саме сталося вдруге з `design-system`.
  // 🛑 Прилад, що міняє колір від новин дня, не стереже НІЧОГО: його червоне не
  // означає регресу, а зелене не означає справності. Ми двічі шукали причину в
  // своїх правках там, де її не було.
  // ✅ Тепер сцена стала: заголовки фіксованої довжини, по три новини на кожен
  // гео-розділ, фото лише в першої. Міряємо КОД, а не стрічку.
  // ⚠️ Довжини заголовків узяті з живих даних (60-110 знаків) — інакше фікстура
  // була б простішою за життя, а це вже окремий спосіб брехні приладу.
  await page.route('**/data/articles.json*', r => r.fulfill({
    contentType: 'application/json',
    body: JSON.stringify({ articles: ФІКСТУРА_СТАТЕЙ }),
  }));
  // Мережа назовні не потрібна: чужі картинки лише сповільнюють вимір і сиплють
  // помилками, які до нашої логіки не стосуються.
...
// 🔴 27.08 — ПЛИТКА-ЗАГЛУШКА ЗАМІСТЬ ФОТО НЕ МАЄ ПОВЕРНУТИСЬ.
// 🗣️ Вова, побачивши її на живому екрані: «придумані картинки з замком біля новини…
// вони негарні, треба придумати без них».
// ⚠️ Правило 26.08 казало протилежне («слот лишається, бо інакше ламається ритм»),
// і саме тому тут потрібна перевірка, а не сподівання на памʼять: наступна сесія,
// читаючи ті коментарі, могла б «полагодити» слот назад.
// 🔑 Міряємо ДВІ речі, бо повернути плитку можна з двох боків: через розмітку
// (рендер малює `.nc-img--mono`) і через обробник битого фото (`.img-fallback`).
const слоти = await page.evaluate(() => {
  const де = [...document.querySelectorAll('#cm-news-board .nc, .nh-list .nc')];
  return {
    карток: де.length,
    заглушок: document.querySelectorAll(
      '#cm-news-board .nc-img--mono, #cm-news-board .nc-img.img-fallback,' +
      '.nh-list .nc-img--mono, .nh-list .nc-img.img-fallback').length,
    // Картка без живого фото мусить нести ознаку стану — інакше CSS не знає,
    // що ширину треба віддати тексту.
    безОзнаки: де.filter(c => !c.querySelector('img') && !c.classList.contains('nc--noimg')).length,
  };
});
ok('🔴 жодної плитки-заглушки замість фото', слоти.заглушок === 0,
   `знайдено ${слоти.заглушок} на ${слоти.карток} картках`);
ok('🔴 картка без фото несе ознаку стану', слоти.безОзнаки === 0,
   `без ознаки: ${слоти.безОзнаки}`);
// 🔴 КОНТРОЛЬ: без нього обидві перевірки були б зеленими й на порожньому екрані.
// Відтворюємо саме ту ваду — додаємо картці слот-заглушку і знімаємо ознаку.
const контрольСлоту = await page.evaluate(() => {
  const c = document.querySelector('#cm-news-board .nc, .nh-list .nc');
  if (!c) return null;
  const було = c.classList.contains('nc--noimg');
  const d = document.createElement('div');
  d.className = 'nc-img nc-img--mono';
  c.prepend(d);
  c.classList.remove('nc--noimg');
  const заглушок = document.querySelectorAll(
    '#cm-news-board .nc-img--mono, .nh-list .nc-img--mono').length;
  const безОзнаки = !c.querySelector('img') && !c.classList.contains('nc--noimg');
  d.remove();
  if (було) c.classList.add('nc--noimg');
  return { заглушок, безОзнаки };
});
ok('🔴 КОНТРОЛЬ: правило ловить повернуту заглушку',
   !!контрольСлоту && контрольСлоту.заглушок > 0 && контрольСлоту.безОзнаки,
   JSON.stringify(контрольСлоту));
