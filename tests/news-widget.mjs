...
// 🔴 21.08 — ПЕРЕПИСАНО З РЕЗУЛЬТАТУ НА ПРАВИЛО. Перевірка «велика картка є»
// почервоніла в ту мить, коли ланцюг кабінету ожив: 10 наших власних статей без
// фото стали найсвіжішими в Громаді, і `markLead` чесно НЕ зробив героя —
// «герой» без знімка це роздутий блок тексту, так і задумано.
// Тобто мірка описувала щасливий збіг у даних, а не поведінку.
// ⚠️ Правило в обидва боки: є фото — має бути герой; немає — героя бути НЕ
// може. Друга половина ловить справжню ваду (роздутий текстовий блок), яку
// стара перевірка пропустила б.
ok('🔴 велика картка є ТОДІ І ЛИШЕ ТОДІ, коли в першої новини є фото',
