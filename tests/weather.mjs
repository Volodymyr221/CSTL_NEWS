...
// 🔄 17.09 — ДОПУСК 12–13.5px (було 12.5–13.5). Причина не «підігнати під код»:
// 03.09 дизайн-аудит округлив опис 13.5 → 13, уточнення лишилось 13 — і два рядки
// зійшлись, тобто ієрархія «головне / пояснення» зникла. Полагодити підняттям
// опису не можна: він стоїть на межі обрізання (133px при 13.5 проти ~135px
// колонки). Лишалось опустити уточнення. 12.5 відхилив сторож `design-system` —
// половинні кеглі в проєкті заборонені, — тож ціле 12px.
ok('рядок «відчувається» — кегль уточнення', s.підPx >= 12 && s.підPx <= 13.5, `${s.підPx}px`);
...
const ШИРИНИ = [
  ...
  // 🔄 29.08 — БУЛО «десктоп». Із цього дня застосунок на компʼютері з мишею НЕ
  // БУДУЄТЬСЯ ВЗАГАЛІ: там показується екран «поки що тільки телефон»
  // (`core/desktop-gate.js`). Тобто ряд погоди на такому екрані більше не існує,
  // і перевіряти в ньому було б нічого — стенд міряв би порожнечу.
  // 🔑 Але сама ШИРИНА нікуди не поділась: 1280 — це планшет у горизонтальній
  // орієнтації, а дотикові пристрої екран пропускає. Тому випадок лишається, лише
  // з правильним пристроєм: дотик замість миші (нижче `hasTouch: true` на всіх).
  { w: 1280, назва: 'планшет горизонтально', іконка: true },
];

const біди = [];
for (const в of ШИРИНИ) {
  const c = await b.newContext({ viewport: { width: в.w, height: 900 },
    // `hasTouch` тепер на всіх ширинах: у Chromium саме він перемикає
    // `(pointer: fine)` на `coarse`, а від цього залежить, чи покаже застосунок
    // екран «тільки телефон». `isMobile` лишається за розміром — він про інше
    // (масштаб сторінки й `viewport`-мета).
    isMobile: в.w < 500, hasTouch: true, serviceWorkers: 'block' });
