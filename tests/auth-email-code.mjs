...
const page = `<!doctype html><html><head><meta charset="utf-8"></head><body><script type="module">
  ...
  // 🔴 30.08: «зачекай N секунд» — це ЗМІСТОВНА відповідь сервера, а не проблема
  // мережі. Вова бачив «Слабкий звʼязок», поки код уже лежав у нього в пошті.
  const лімітТекст = netErrorText({ message: 'For security purposes, you can only request this after 41 seconds', status: 429 });
  out.лімітНеПроМережу = !/звʼязок|зв'язок/i.test(лімітТекст);
  out.лімітКажеСекунди = /41/.test(лімітТекст);
...
ok('🔴 ліміт НЕ називають «слабким звʼязком»',   out.лімітНеПроМережу === true);
ok('ліміт каже точну кількість секунд',        out.лімітКажеСекунди === true);
