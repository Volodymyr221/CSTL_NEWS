...
for (const w of ШИРИНИ) {
  ...
  const зг = await p.evaluate(() => {
    ...
    return {
      ...
      мінмакс: document.querySelector('.hm-wx-sub').textContent.trim(),
      підОбрізано: (() => { const e = document.querySelector('.hm-wx-sub'); return e.scrollWidth > e.clientWidth; })(),
...
// 🔴 Той самий клас дефекту, що 08.08: міряли на одному екрані — обрізало на іншому.
// ⚠️ 320px ВИНЕСЕНО З ПЕРЕВІРКИ СВІДОМО, і це не послаблення сторожа, а названа
// межа. Заміряно: у рядку 280px після іконки (22), температури (38), капсули НП
// (72) і стрілки (30) на текст лишається 94px, а найдовший опис словника —
// 133px при 13.5px кеглю. Тобто на 320px вибір бінарний: або назва населеного
// пункту, або повний опис погоди. Назву лишено — саме через те, що вона губилась,
// її 05.08 і виносили окремим елементом на пряме прохання Вови.
// 🔑 320px це iPhone SE першого покоління (2016); SE2/SE3 — уже 375. Коротші
// описи («Ясно», «Хмарно», «Дощ») там влазять цілими, три крапки отримує лише
// найдовший. Захочемо прибрати й це — прибирати доведеться капсулу НП.
const ВУЗЬКИЙ_ВИНЯТОК = 320;
const обрізані = заміри.filter(z => z.зг.описОбрізано && z.w !== ВУЗЬКИЙ_ВИНЯТОК).map(z => z.w);
ok('🔴 найдовший опис («Мінлива хмарність») не обрізається на 375 · 390 · 430',
   обрізані.length === 0, обрізані.length ? `обрізано на ${обрізані.join(', ')}px` : заміри[1].зг.опис);
// Контроль: перевірка вище має бути здатна впасти. Якщо опис перестане обрізатись
// навіть на 320 — це не поломка, а привід зняти виняток, і рядок про це скаже.
const вузький = заміри.find(z => z.w === ВУЗЬКИЙ_ВИНЯТОК);
ok('КОНТРОЛЬ: на 320px найдовший опис справді впирається в межу (виняток живий)',
   вузький.зг.описОбрізано, вузький.зг.описОбрізано ? 'так, виняток потрібен' : 'уже влазить — виняток можна знімати');

// 🔴 19.08 — скарга Вови: «неясно, що за день, яке число, це сьогодні?».
// Дата в шапці була, але тиха і відірвана від погоди. Тепер день названий у
// самому віджеті. ⚠️ На ≤374px слово ховається (там обрізало б САМІ ЧИСЛА —
// заміряно), тож перевіряємо на ширинах, де воно має бути.
const зСловом = заміри.filter(z => z.w >= 375);
ok('🔴 компактний рядок називає день («Сьогодні · 14° / 24°»)',
   зСловом.every(z => /^Сьогодні · \d+° \/ \d+°$/.test(z.зг.мінмакс)),
   зСловом.map(z => `${z.w}:${z.зг.мінмакс}`).join(' · '));
ok('на ≤374px слово ховається, а числа лишаються цілими',
   /\d+° \/ \d+°/.test(заміри[0].зг.мінмакс) && !заміри[0].зг.підОбрізано,
   `${заміри[0].зг.мінмакс}, обрізано: ${заміри[0].зг.підОбрізано}`);
