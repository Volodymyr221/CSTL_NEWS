...
// 🔄 19.08, уточнення Вови: «стрілочку розташувати самого ліва, ПЕРЕД самою
// іконкою погоди». Тобто вона перша в рядку — у лівому краю в неї немає сусіда,
// з яким її можна сплутати (у правому вона щоразу читалась як частина капсули).
ok('🔴 стрілка стоїть ПЕРШОЮ, перед іконкою погоди',
   заміри.every(z => z.зг.порядок.join(',') === 'hm-wx-toggle,hm-wx-now,hm-wx-t,hm-wx-txt,hm-wx-place'),
...
// б зеленою і на реалізації, що смикає екран на треть висоти.
{
  ...
  // ── 🔴 ПОЛІТ ПАЛЬЦЯ НЕ ЧІПАЄМО (20.08, другий захід) ───────────────────────
  // Скарга Вови на ПЕРШУ версію цієї фічі: «коли віджет ховається, сторінка
  // ЗУПИНЯЄТЬСЯ; тільки тоді як зупинилась можна скролити далі».
  //
  // 🔑 Причина була не в тому, ЩО рахувалось, а КОЛИ: будь-який запис у
  // `scrollTop` під час інерційної прокрутки на iOS обриває інерцію. Тобто
  // сторінку зупиняв не браузер, а власна «плавна» компенсація.
  //
  // 🛑 ЧОМУ ЦЕ ОКРЕМА СЦЕНА, а не доробка попередньої: та ставить прокрутку
  // ОДИН раз і чекає в тиші. У тиші вада не існує — вона є рівно тоді, коли
  // сторінка ще рухається. Сторож без польоту був зелений над зламаною
  // поведінкою, і саме так вона доїхала до телефона.
  const політ = await p.evaluate(async () => {
    const сон = ms => new Promise(res => setTimeout(res, ms));
    const скролер = document.querySelector('.app-main');
    const wx = document.querySelector('.hm-wx');
    скролер.scrollTop = 0;
    ...
    // Імітуємо інерцію: сторінка їде сама, подіями прокрутки, без пауз.
    скролер.scrollTop = 700;
    let подій = 0;
    скролер.addEventListener('scroll', () => { подій++; }, { passive: true });
    let згорнулосьУПольоті = false;
    for (let i = 0; i < 10; i++) {
      скролер.scrollTop += 20;
      await сон(40);
      if (!wx.classList.contains('hm-wx--open')) згорнулосьУПольоті = true;
    }
    const підЧасПольоту = скролер.scrollTop;
    await сон(500);                        // політ скінчився — тиша
    return { межа: Math.round(скролер.scrollHeight - скролер.clientHeight),
             подій: подій,
             згорнулосьУПольоті,
             згорнулосьПісля: !wx.classList.contains('hm-wx--open'),
             підЧасПольоту: Math.round(підЧасПольоту) };
  });

  // 🔑 НАЙПРЯМІША МІРКА ВАДИ: сцена сама відкрутила рівно 700 + 10×20 = 900.
  // Якщо наприкінці польоту там не 900 — значить у `scrollTop` писав хтось ще,
  // і цей «хтось» на iPhone обриває інерцію. До фікса тут було 591, тобто рівно
  // 900 мінус висота панелі: компенсація спрацьовувала прямо в польоті.
  // ⚠️ Перевірка на клас (вище) слабша: вона зловила б лише згортання, а вада —
  // саме в ДОТИКУ до прокрутки, і він міг би лишитись при іншій реалізації.
  ok('🔴 прокрутку в польоті не змінював ніхто, крім самої сторінки',
     політ.підЧасПольоту === 900, `${політ.підЧасПольоту} замість 900`);
  ok('сцена справді летіла (події прокрутки надходили)', політ.подій >= 8, `${політ.подій} подій`);
  ok('🔴 поки сторінка ЛЕТИТЬ — прокрутку не чіпаємо (віджет ще не згорнуто)',
     !політ.згорнулосьУПольоті);
  ok('🔴 щойно політ ущух — віджет згортається', політ.згорнулосьПісля);
