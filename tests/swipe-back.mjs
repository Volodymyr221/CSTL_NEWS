...
await свайп(p, { x: 120, y: 200, dx: 160 });
ok('🔴 свайп управо з середини ЛІВОЇ половини закриває (те, чого просив Вова)',
   await закрито());
...
// ── 1b. МЕЖА ЗОНИ: ліва половина наша, права — ні ────────────────────────────
// Вова: «щоб з лівої частини до половини був екран з закриттям свайпом».
// Екран 390px, тож межа = 195px.
await відновити();
await свайп(p, { x: 170, y: 200, dx: 160 });
ok('🔴 старт у лівій половині (170px із 390) закриває', await закрито());

await відновити();
await свайп(p, { x: 300, y: 200, dx: 160 });
ok('🔴 старт у ПРАВІЙ половині (300px) не закриває — там кнопки й каруселі',
   (await закрито()) === false);
...
await відновити();
await свайп(p, { x: 120, y: 200, dx: 0, dy: 200 });
ok('🔴 вертикальний рух не закриває (це прокрутка)', (await закрито()) === false);

await відновити();
await свайп(p, { x: 120, y: 200, dx: 120, dy: 140 });
...
await відновити();
await свайп(p, { x: 120, y: 200, dx: 40 });
ok('короткий рух (40px) не закриває — намір має бути виразним',
   (await закрито()) === false);

// ── 3. Чужі жести ────────────────────────────────────────────────────────────
await відновити();
await свайп(p, { x: 120, y: 450, dx: 160, ціль: '#свій' });
...
await свайп(p, { x: 120, y: 545, dx: 160, ціль: '#скролер' });
