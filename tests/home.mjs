...
const FUND_ROWS = [{
  ...
  kind: 'community',
  // 🔴 18.09 — ДАТА РАХУЄТЬСЯ, А НЕ ПИШЕТЬСЯ ЦВЯХОМ.
  // Тут стояло `until: '2026-09-15'` — і 16.09 стенд почервонів НА РОБОЧОМУ КОДІ:
  // збір «завершився», код чесно намалював «Збір завершено» замість кнопки, а
  // сторож шукав кнопку. Бомба сповільненої дії, закладена в день написання.
...
// 5.1 🗓 СЕКЦІЯ ПОДІЙ — ВИМОГА ПЕРЕВЕРНУЛАСЬ 04.09, І ЦЕ НЕ ПОСЛАБЛЕННЯ.
//
// 🔴 До 04.09 тут стояло «секції подій на головній НЕМАЄ» — фіксація рішення
// Вови 04.08 «події прибрати». Його причина була НЕ в даних, і він назвав її
// прямо: вести з блока не було куди — вкладки Подій не існує, а кнопка «Афіша →»
// кидала людину просто у Стрічку. Сторож тримав це доти, доки не зʼявиться
// справжній екран події.
//
// ✅ 04.09 Вова замовив події спільнот, і причина зникла: подія — це ДОПИС, тож
// тап веде в саму подію (`focusFeedPost`), а не в абстрактну афішу. Тому вимога
// стала протилежною — і сторож став СИЛЬНІШИМ, а не слабшим: раніше він питав
// одне («секції немає»), тепер три речі одразу.
//
// 🛑 ГОЛОВНЕ ТУТ — ДРУГА ПЕРЕВІРКА: секція мусить бути ПРИХОВАНА, поки подій
// немає (пряме рішення Вови «ховається»). Без неї «секція є» проходило б і для
// порожнього блока з написом «подій немає» — тобто для того, від чого 04.08
// і відмовлялись.
// ⚠️ Сцена стенда без бази, тож подій тут не буває НІКОЛИ — саме тому стан
// «прихована» перевіряється, а «показана з подіями» лишається за
// `community-events.mjs`, який міряє логіку окремо.
const ev = await p.evaluate(()=>({
  sec: !!document.getElementById('hm-events'),
  cont: !!document.getElementById('cm-event-content'),
  hidden: document.getElementById('hm-events')?.hidden === true,
  afisha: [...document.querySelectorAll('#cm-content .hm-more')].some(b => /афіш/i.test(b.textContent)),
}));
ok('🗓 секція подій громади є в розмітці', ev.sec && ev.cont);
ok('🛑 і вона ПРИХОВАНА, поки подій немає (рішення Вови)', ev.hidden,
   `hidden: ${ev.hidden}`);
// Кнопка «Афіша →» не повертається: вести до неї так само нікуди, а веде секція
// у Стрічку — туди, де подія і живе.
ok('кнопки «Афіша →» немає — веде у Стрічку, де подія і лежить', !ev.afisha);
