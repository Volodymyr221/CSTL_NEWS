...
if (!fundItems.length) {
  ...
  const look = await p.evaluate(() => {
    ...
    const bar = card.querySelector('.hm-fund-bar > i');
    const ring = card.querySelector('.hm-fund-ring-n');
    ...
    return {
      ...
      смугаВідсоток: bar && panel
        ? Math.round(bar.getBoundingClientRect().width / bar.parentElement.getBoundingClientRect().width * 100)
        : null,
      кільце: ring ? ring.textContent.trim() : null,
  ...
  // 5.13.2 🔴 ЧИСЛА ПРО ГРОШІ — ТІЛЬКИ З ДАТОЮ СТАНУ (перегляд 16.08).
  //
  // Правило 05.08 звучало як «жодних сум і відсотків», і сторож міряв саме
  // текст. Насправді заборонялось не число, а число БЕЗ ПОХОДЖЕННЯ: Monobank
  // суми чужої банки не віддає, тож її вписує людина руками, і без дати вона
  // старіє мовчки. Тому перевірка тепер міряє ПРАВИЛО, а не слово:
  //   • є `raised` + `raisedAt` → сума показана і поруч видно її вік;
  //   • немає `raisedAt`        → суми немає ВЗАГАЛІ.
  // Обидві половини звіряються з тим самим файлом, який читає застосунок, —
  // інакше сторож міряв би вміст даних, а не поведінку коду (та сама хвороба,
  // через яку цей блок уже переписували 05.08).
  const f0 = fundItems[0] || {};
  const сумаМаєБути = Number.isFinite(Number(f0.raised)) && !!f0.raisedAt;
  const сумаПоказана = /Зібрано/i.test(look?.текст || '');
  ok('🔴 сума показана тоді і лише тоді, коли є дата звірки',
     сумаПоказана === сумаМаєБути,
     `raised: ${f0.raised ?? '—'}, raisedAt: ${f0.raisedAt ?? '—'}, показано: ${сумаПоказана}`);
  if (сумаМаєБути) {
    // Вік числа — не дрібним шрифтом «десь там», а словами в ряду статистики.
    ok('🔴 разом із сумою видно, коли її звіряли', /оновлено/i.test(look?.текст || ''));
    const goalN = Number(f0.goal);
    if (Number.isFinite(goalN) && goalN > 0) {
      const pct = Math.min(100, Math.round(Number(f0.raised) / goalN * 100));
      ok('відсоток у кільці порахований, а не вписаний',
         look?.кільце === `${pct}%`, `кільце: ${look?.кільце}, очікували: ${pct}%`);
      // Смуга міряється ГЕОМЕТРІЄЮ: саме тут ховається клас помилок, коли
      // число підставили в текст, а ширину лишили сталою.
      ok('смуга поступу тієї ж довжини, що й відсоток',
         Math.abs((look?.смугаВідсоток ?? -1) - pct) <= 1,
         `смуга: ${look?.смугаВідсоток}%, відсоток: ${pct}%`);
    }
  }
  // 🛑 Кількість донатів Monobank не віддає — її можна показати ЛИШЕ з даних.
  const донатиМаютьБути = Number(f0.donations) > 0;
  ok('🔴 «донати» лише з даних, не вигадані',
     /донат/i.test(look?.текст || '') === донатиМаютьБути,
     `у файлі: ${f0.donations ?? '—'}`);
