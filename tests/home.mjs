...
// 🔴 19.08 — СПЕРШУ РОЗГОРНУТИ. Віджет погоди отримав два стани (замовлення Вови
// «зробити значно компактнішим»), і дні живуть у панелі, яка згорнута за
// замовчуванням. Сам перехід між станами стереже стенд `weather-fold.mjs`.
await p.locator('.hm-wx-toggle').click();
...
// 🔴 19.08 — ЗГОРТАЄМО НАЗАД, І ЦЕ НЕ ПРИБИРАННЯ ЗА СОБОЮ, А УМОВА ПОДАЛЬШИХ
// ЗАМІРІВ. Розгорнута панель додає сторінці ≈310px, і секція зборів їде за нижню
// межу екрана; її фото лежить у `<img>`, який довантажується лише коли картка
// потрапляє у поле зору. Наслідок був вимірний: чотири перевірки картки збору
// («шар затемнення», «фото окремим шаром», «білий заголовок», «скло») падали не
// через збір, а через те, що картка не встигла намалюватись у своєму фото-варіанті.
// ➡️ Повертаємо віджет у стан за замовчуванням — далі сторінка така сама, як була.
await p.evaluate(() => {
  const wx = document.querySelector('.hm-wx');
  if (wx?.classList.contains('hm-wx--open')) document.querySelector('.hm-wx-toggle')?.click();
});
await p.waitForTimeout(450);
...
// 5.0 🔴 КАПСУЛИ: ТРИ РОЛІ, БЕЗ РУХУ.
//
// ⚠️ 17.08 РОЗДІЛ ПЕРЕПИСАНО РАЗОМ ІЗ КОМПОНЕНТОМ. Було: три капсули з іменами
// РОЗДІЛІВ і статистикою громади, що циклічно мінялась кожні 5.2 с. Стало: три
// слоти за РОЛЛЮ (МОЄ · ЗАРАЗ · НОВЕ), по одному статичному рядку.
// Звідси пішли перевірки «крапки циклу намальовані», «цикл справді міняє
// повідомлення», «поза екраном цикл спиняється» — вони стерегли механізм, якого
// більше немає, і лишати їх означало б тримати сторожа на порожньому місці.
//
// 🔑 ЩО ТУТ ЛИШИЛОСЬ, А ЩО ПЕРЕЇХАЛО. Тут — правила, які стосуються ГОЛОВНОЇ
// СТОРІНКИ загалом (порожня коробка, векторні іконки, відсутність руху). Вміст
// ролей, пріоритет, час до рейсу і тапи міряє окремий стенд `home-caps.mjs`:
// там сцена підроблена цілком (годинник, розклад, база), тож перевірки
// детерміновані. Тут сцена ЖИВА — сьогоднішній розклад і порожня позначка
// «востаннє був на Дошці», — і кількість капсул законно може бути нулем.
// Саме тому нижче ніде не стоїть «капсул > 0»: це була б перевірка погоди, а
// не коду.
...
ok('🔴 іконки капсул векторні', capIc.svg === capIc.n, `${capIc.svg}/${capIc.n}`);
ok('🔴 емодзі в капсулах не лишилось', capIc.emoji === 0);
ok('іконка капсули не дрібна (≥16px)', capIc.n === 0 || capIc.w >= 16, `${capIc.w}px`);
ok('🔴 порожньої коробки капсул не буває', !caps.порожня);
// Роль — це МОЄ / ЗАРАЗ / НОВЕ / ПОВІДОМЛЕННЯ, і нічого іншого. Стара конструкція
// ставила сюди назви розділів; сторож не дає їм повернутись непомітно.
//
// 🔴 20.08 — СТОРОЖ ВІДСТАВ ВІД ПРОДУКТУ НА ТРИ ДНІ, і це його вада, не застосунку.
// Він вимагав, щоб мітка дорівнювала РІВНО «ЗАРАЗ». Але 17.08 мітку свідомо
// зробили двоскладовою («ЗАРАЗ · ЧЕРЕЗ 40 ХВ»): відлік переїхав саме сюди, бо в
// рядку значення він обрізав маршрут на вузькому екрані.
// ⚠️ ЧОМУ ЦЕ НЕ ЛОВИЛОСЬ ЩОДНЯ: капсула автобуса є лише тоді, коли рейс ближчий
// за 120 хв, тож поза цим вікном перевірка проходила ВХОЛОСТУ — на порожньому
// списку `every` завжди істина. Червонів сторож «за годинником», і саме тому
// виглядав як випадковий збій, а не як застаріле правило.
// ✅ Тепер звіряємо ПЕРШИЙ склад мітки (до « · »): роль лишається під наглядом,
// а уточнення після крапки — вільне. Список ролей доповнено ЗАВТРА (20.08,
// нічна капсула першого ранкового рейсу).
// 🔴 21.08 — додано ВИЇХАВ і РЕЙС (скасування). Це не послаблення правила, а
// його дотримання: обидва підписи відповідають на «навіщо цей рядок тут»
// («автобус уже їде», «твого рейсу не буде»), тобто вони РОЛІ, а не назви
// розділів. Правило забороняє саме назву розділу («АВТОБУСИ», «ДОШКА»).
// 🔴 22.08 — МІРКА ЗНОВУ ВІДСТАЛА ВІД ПРОДУКТУ, і знову це її вада. Вона брала
// перший склад ДО «·», але капсула скасування підписана «РЕЙС О 07:40
// СКАСОВАНО» — крапки там немає взагалі, тож звірявся весь рядок і не збігався
// ні з чим. Заміряно на живому розкладі: скасовані рейси є щодня (17 із 350),
// але капсула є лише коли скасований рейс саме твій і саме зараз — тому червоне
// виглядало як випадковий збій, точно як описано в коментарі вище.
// ✅ Звіряємо ПЕРШЕ СЛОВО, а не перший склад: правило забороняє назву розділу
// («АВТОБУСИ», «ДОШКА»), і перше слово ловить її так само надійно, зате не
// залежить від того, чи є в підписі роздільник.
// 🔴 24.08 — ДОДАНО «СЬОГОДНІ», І ЦЕ ЧЕТВЕРТИЙ РАЗ, КОЛИ СПИСОК ВІДСТАВ.
// Підпис зʼявився вранці 24.08 разом із виправленням капсули автобуса (вона
// казала «ЗАВТРА», коли рейс сьогодні ще попереду) — а сюди його не дописали.
// Це РОЛЬ, а не назва розділу: відповідає на «навіщо цей рядок тут» («рейс
// сьогодні»), рівно як уже дозволене «ЗАВТРА». Правило забороняє саме назву
// розділу («АВТОБУСИ», «ДОШКА»), і воно не послаблюється.
//
// 🛑 ЧОМУ ЦЕ НЕ СПІЙМАЛОСЬ НА МЕРДЖІ — важливіше за сам список. Сторож ганяє
// ЖИВИЙ розклад, тож капсула існує лише в певні години: на момент мерджу її
// просто не було на екрані, і сторож був зелений НЕ ТОМУ, що все гаразд.
// Червоніти він почав аж о 16:56, у зовсім іншій роботі, і виглядало це як
// свіжа регресія. ➡️ Міняєш підпис капсули — правь цей список ТИМ САМИМ
// комітом, бо зворотний звʼязок тут приходить із запізненням на години.
const РОЛІ = ['МОЄ', 'ЗАРАЗ', 'НОВЕ', 'ПОВІДОМЛЕННЯ', 'ЗАВТРА', 'СЬОГОДНІ', 'ВИЇХАВ', 'РЕЙС'];
const перше = r => String(r).trim().split(/[\s·]+/)[0];
ok('🔴 підпис капсули — роль, а не назва розділу',
   caps.ролі.every(r => РОЛІ.includes(перше(r))),
   caps.ролі.join(',') || '—');
// 🔴 РУХ НЕ ПОВЕРТАЄТЬСЯ. Сторінку чистили від чотирьох автоматичних рухів
// (аудит 03.08), капсули повернули пʼятий, і 17.08 його прибрали. Міряємо
// НАСЛІДОК — текст через 8с той самий, — а не відсутність `setInterval`:
// таймер можна поставити знову і назвати інакше.
...
ok('🔴 капсули не міняють текст самі собою (ротацію знято 17.08)',
   capBefore.join('|') === capAfter.join('|'), `${capBefore.join('/')} → ${capAfter.join('/')}`);
ok('🔴 крапок циклу в розмітці не лишилось', caps.крапки === 0, `${caps.крапки}`);
...
// 🔄 19.08 — стрілка САМОЇ КАПСУЛИ схована стилем, і це не втрата правила «ікони
// векторні», а наслідок появи другої стрілки поруч: тепер у рядку є справжня
// стрілка розгортання прогнозу, і дві однакові шеврони поряд означали різні речі.
// Вузол лишився в розмітці — стережемо, що він і далі ВЕКТОР, а не емодзі, якщо
// його колись повернуть на видноту.
ok('🔴 стрілка вниз лишилась вектором (хоч і схована стилем)', icons && icons.ch);
ok('🔴 емодзі й символів-стрілок у кнопці не лишилось', icons && !icons.emoji);
// 🔄 19.08 — розмір стрілки КАПСУЛИ більше не міряємо: вона схована стилем
// (поруч зʼявилась справжня стрілка розгортання прогнозу, і дві однакові шеврони
// в одному рядку читались як помилка). Нуль тут тепер означає «сховано», а не
// «дрібно» — тобто старий критерій перестав відрізняти добрий стан від поганого.
// ➡️ Питання «чи не дрібна стрілка» переїхало на ту, яку людина справді бачить.
ok('стрілку капсули справді сховано (а не зменшено)', icons && icons.chW === 0, icons ? `${icons.chW}px` : '—');
const стрілкаРозгортання = await p.evaluate(() => {
  const svg = document.querySelector('.hm-wx-toggle svg');
  return svg ? Math.round(svg.getBoundingClientRect().width) : 0;
});
ok('🔴 стрілка розгортання не дрібна (≥18px)', стрілкаРозгортання >= 18, `${стрілкаРозгортання}px`);
...
const afterPick = await p.evaluate(() => ({
  ...
  сеанс: sessionStorage.getItem('wx_pick_session_v1'),
}));
ok('🔴 після вибору показано обране село', afterPick.name === 'Дерно', `чіп: «${afterPick.name}»`);
// 🔑 22.08 — вибір лягає у ДВА сховища з різним строком життя: сеанс (сильніше
// за геолокацію) і «останнє обране колись» (слабше). Перевіряємо обидва:
// пропаде одне — і або вибір не діятиме зараз, або людина без GPS втратить
// єдиний спосіб сказати застосунку, де вона.
ok('вибір запамʼятався — і на сеанс, і як останній',
   afterPick.saved === 'Дерно' && afterPick.сеанс === 'Дерно',
   `останнє: ${afterPick.saved} · сеанс: ${afterPick.сеанс}`);
...
await p.evaluate(() => {
  // Панель могла лишитись згорнутою після попередніх кроків — розгортаємо, якщо
  // треба (повторний тап по стрілці закрив би вже відкриту).
  const wx = document.querySelector('.hm-wx');
...
await p.evaluate(() => {
  localStorage.removeItem('wx_place_v1');
  // ⚠️ Обидва: сеансове сховище сильніше за геолокацію, і лишити його тут
  // означало б, що всі наступні перевірки цього файлу бачать сторінку «з
  // Дерна», хоч ми нібито повернули стан.
  sessionStorage.removeItem('wx_pick_session_v1');
});
...
const tile = await p.evaluate(()=>{
  ...
  // 🔴 26.08 — ШУКАЄМО СЛОТ ПО ВСІХ ВЕЛИКИХ КАРТКАХ ДОРІЖКИ, а не лише в першій.
  // Причина: з кроку 4 картка без фото слоту НЕ МАЄ ВЗАГАЛІ (`.nc--noimg`), і
  // якщо першою в доріжці стоїть саме така, вимір повертав -1 і перевірка нижче
  // червоніла на цілком правильній поведінці. Радіус фото — властивість
  // КОМПОНЕНТА, тож міряти його можна на будь-якій картці, у якої слот є.
  const img = document.querySelector('#hm-ntrack .nc--hero .nc-img');
...
// ⚠️ `-1` = «на доріжці немає жодної картки зі слотом під фото» (усі перейшли в
// стан `.nc--noimg`). Це не провал і не привід мовчки зеленіти: перевірити нічого,
// і повідомлення каже це прямо, щоб зелений рядок не видавався за доказ.
ok('фото власного радіуса не має — його дає картка',
   tile && (tile.imgR === 0 || tile.imgR === -1),
   tile ? (tile.imgR === -1 ? 'слоту під фото на доріжці немає — міряти нічого' : `${tile.imgR}px`) : '—');
