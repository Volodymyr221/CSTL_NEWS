...
// `data/fundraisers.json` у репозиторії порожній, тож на живій сторінці блока
// бути НЕ МАЄ. Це не дрібниця: блок про допомогу, який показує «зборів немає»,
// займає місце й нічого не дає. Вигляд самої картки перевіряється окремо, на
// підмінених даних — `tests/tools/fund-shot.mjs`.
