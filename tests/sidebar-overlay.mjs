...
//
// 🔴 17.08 — ТУТ СТЕНД ЛАМАВСЯ, І ЛАМАВСЯ МОВЧКИ (25/36 замість 36/36).
// Було: `page.mouse.click(30, 500)` з підписом «тап по лівому краю = по
// затемненню». Але блок 2 вище закриває меню хрестиком, тобто на цей момент
// затемнення ВЖЕ НЕМАЄ — і тап летів у вміст головної сторінки.
// 📐 Заміряно приладом просто в цій точці сцени: під пальцем лежала картка
// новини (`nc-src`), тап відкривав ПОВНОЕКРАННИЙ ХАБ НОВИН (`body.nh-open`), і
// далі бургер був фізично накритий заголовком хаба — звідси «бургер став
// недосяжним для тапу» в блоках 8-9 та решта червоних.
//
// 🔑 ЧОМУ ВОНО «РАПТОМ» ЗЛАМАЛОСЬ, хоч код меню не чіпали: під координатою
// (30, 500) лежить ЖИВИЙ КОНТЕНТ НОВИН. Новини оновлюються парсером щогодини,
// картки їздять — і одного дня в цій точці опинилось те, що відкриває шар.
// Тобто перевірка залежала від даних, про які нічого не знає.
//
// ✅ Тепер: меню ВІДКРИВАЄМО явно, а тапаємо по САМОМУ затемненню (по елементу,
// не по числах). Так сцена не залежить ні від вмісту сторінки, ні від того, що
// лишив попередній блок.
await tapBurger();
await settle();
await page.locator('#sidebar-overlay').click({ position: { x: 30, y: 300 }, force: true });
await settle();
st = await overlayState();
