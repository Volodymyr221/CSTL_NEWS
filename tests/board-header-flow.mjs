...
const rise = await page.evaluate(async () => {
  ...
  await at(700);
  const out = [];
  // ⚠️ ВИМІР ПОЧИНАЄМО НИЖЧЕ ТОЧКИ ВІДЛІПЛЕННЯ, і це не послаблення критерію.
  // Поки шапка прилипла, вона СТОЇТЬ — на кроці, де вона саме відліплюється,
  // частину шляху вона ще стоїть, а частину вже їде, тож рівність зсувів там
  // фізично неможлива (заміряно: 28 проти 30 на 2px переходу). Синхронність має
  // сенс лише там, де шапка ВЖЕ в потоці. Точка відліплення = висота шапки + 6,
  // тобто ~119px; беремо із запасом.
  let prev = await at(100);
  for (const y of [75, 50, 25, 0]) {
