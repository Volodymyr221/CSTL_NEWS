...
// ═══ 14. Я НЕ ВДОМА — АВТОБУС ДОДОМУ (21.08) ════════════════════════════════
// 🔴 Скарга Вови зі знімка: капсула показувала «Олика → Жорнище», коли він
// фізично був у Луцьку — за 35 км від тієї зупинки. Сісти на той автобус не
// можна ніяк, тобто капсула займала слот рядком, який людині марний.
{
  const s = await сцена({
    routes: [РЕЙС_БЛИЗЬКО, РЕЙС_ДАЛЕКО, РЕЙС_МЕТЕЛЬНЕ],
    user: USER, profiles: [{ uid: UID, name: 'Вова', settlement: 'Олика' }],
    гео: { lat: 50.7472, lon: 25.3254 },        // Луцьк, поза громадою
  });
  const к = await капсули(s.p);
  const i = к.ключі.indexOf('now');
  ok('🔴 поза громадою капсула каже про дорогу ДОДОМУ, а не про рейс із села',
     i >= 0 && /ДОДОМУ/.test(к.мітки[i]), i >= 0 ? к.мітки[i] : '(капсули немає)');
  // 🔑 І напрямок саме до мого села: «додому» без дому — просто слово.
  ok('🔴 напрямок веде в моє село', i >= 0 && /Олика/.test(к.тексти[i]),
     i >= 0 ? к.тексти[i] : '');
  await s.ctx.close();
}
{
  // ⚠️ Дзеркальна перевірка: БЕЗ дозволу «додому» не вмикається. Порожній
  // результат геолокації раніше означав і «я далеко», і «невідомо» — якби ці
  // стани лишились одним, людина без дозволу бачила б «ДОДОМУ», сидячи вдома.
  const s = await сцена({
    routes: [РЕЙС_БЛИЗЬКО, РЕЙС_ДАЛЕКО, РЕЙС_МЕТЕЛЬНЕ],
    user: USER, profiles: [{ uid: UID, name: 'Вова', settlement: 'Олика' }],
    гео: null,
  ...
  const к = await капсули(s.p);
  const i = к.ключі.indexOf('now');
  ok('🔴 без дозволу «ДОДОМУ» НЕ вмикається (невідомо ≠ далеко)',
     i < 0 || !/ДОДОМУ/.test(к.мітки[i]), i >= 0 ? к.мітки[i] : '(капсули немає)');
  await s.ctx.close();
}
