...
// Тут стояло «усі заголовки капсом» + «капс має міжлітерний просвіт» — вимога від
// 03.08, коли капс і був способом зробити назву головною.
// 🗣️ Замовлення 03.09: «зробити не "дитячий" вигляд, а преміальний… не керуйся тим
// що є, типу "бо ми вже так зробили"».
// 📐 Дизайн-аудит заміряв: 62 вузли `.bd-ad-title` по 16px/800 КАПСОМ. Пʼять карток
// поспіль — і кожна кричить; саме це найсильніше читалось як «дитяча» Дошка.
// 🔑 Акцент нікуди не подівся — він тепер тримається КЕГЛЕМ і ВАГОЮ проти опису
// (перевірки нижче), а не регістром. Ті перевірки й лишаються справжнім критерієм
// «назву видно при швидкому скролі».
ok('заголовки НЕ капсом (03.09 — знято за замовленням Вови)',
   cards.every(c => c.капс !== 'uppercase'),
...
// 🔴 03.09 — БУЛО `>= 800`. Дизайн-аудит: 41% усього тексту застосунку мав вагу ≥700,
// а 800/900 у системному шрифті на 11-20px майже не відрізняються від 700 — це не
// ієрархія, а шум. Критерій лишається ВІДНОСНИМ (важчий за опис), просто планка
// опустилась до 600: опис має 400, тож відрив у дві сходинки ваги нікуди не зник.
ok('заголовок важчий за опис накресленням', cards.every(c => c.вага_назви >= 600),
