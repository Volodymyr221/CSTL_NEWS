...
const POSTS = [
  ...
  { id: 105, type: 'board', status: 'published', category: 'шукаю', owner_uid: 'u3',
    ...
    location: 'Олика', photos: ['./logo.png'], created_at: дні(28), published_at: дні(28) },
];
...
const зріз = await p.evaluate(() => {
  ...
  return {
    ...
    // 🔴 01.09 — ГОЛОВНА ПЕРЕВІРКА ПІСЛЯ СКАРГИ ВОВИ: «оголошення в віджеті
    // оголошень задуже великі, вони мають поміщатись в екран, весь віджет».
    // 🔬 Заміряно ДО фіксу: секція **1215px** на екрані 844, картки 141/493/509,
    // фото малювалось природним розміром 294×367.
    // 🔑 Корінь був не в розмірі, а в ОБЛАСТІ ДІЇ СТИЛІВ: усі правила картки
    // писались як `#board-content .bd-ad*`, а віджет живе в `#cm-board-content` —
    // тож у ньому не діяло ЖОДНЕ, і картка малювалась майже без стилів.
    // ⚠️ Міряємо ВИСОТУ СЕКЦІЇ, а не окремих карток: скарга була про віджет
    // цілком, і три низькі картки з високою шапкою її б не зняли.
    висотаСекції: Math.round(document.getElementById('hm-board').getBoundingClientRect().height),
    висотаЕкрана: window.innerHeight,
    висотиКарток: cards.map(x => Math.round(x.getBoundingClientRect().height)),
    // 🛑 ДВА РІЗНІ НУЛІ, ЯКІ РАНІШЕ ВИГЛЯДАЛИ ОДНАКОВО (полагоджено 26.09).
    // `зКартинкою` рахує і плейсхолдер-іконку — вона теж має клас `bd-ad-img`,
    // але це НЕ тег `<img>`. Тому «фото не намалювалось» і «у цю трійку фото
    // просто не потрапило» давали той самий `0px`, а підпис звинувачував CSS.
    фотоКарток: cards.filter(x => x.querySelector('img.bd-ad-img')).length,
    ширинаФото: (() => {
      const img = document.querySelector('#cm-board-content img.bd-ad-img');
...
ok('🔴 ВЕСЬ ВІДЖЕТ ВЛАЗИТЬ В ЕКРАН', зріз.висотаСекції <= зріз.висотаЕкрана,
   `секція ${зріз.висотаСекції}px при екрані ${зріз.висотаЕкрана}px`);
ok('жодна картка не вища за 200px', зріз.висотиКарток.every(h => h <= 200),
   зріз.висотиКарток.join(' · ') + 'px');
// 🔴 26.09 — ЧОМУ ЦЕ ДВІ ПЕРЕВІРКИ, А НЕ ОДНА. Віджет бере три оголошення з
// п'яти ВИПАДКОВО. Фото було лише в двох, тож приблизно кожен десятий прогін
// діставав трійку без жодного `<img>` — і стенд червонів підписом «стилі не
// діють», відправляючи наступну сесію копати CSS, де все було справне.
// 🔑 Знято з двох боків: фото тепер у трьох оголошеннях із п'яти (трійка без
// фото фізично неможлива), і сам нуль тепер має власний, чесний підпис.
ok('у трійку потрапило хоч одне справжнє фото (інакше міряти нічого)',
   зріз.фотоКарток >= 1, `фото-карток ${зріз.фотоКарток} із ${зріз.карток}`);
ok('🔴 стилі картки Дошки ДІЮТЬ у віджеті (фото 88px, не природний розмір)',
   зріз.ширинаФото > 0 && зріз.ширинаФото <= 100, `${зріз.ширинаФото}px`);
