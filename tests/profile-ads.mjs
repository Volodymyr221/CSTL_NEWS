...
// 🔴 06.08 — СТОРОЖ НА РЕАЛЬНИЙ БАГ, ЯКИЙ ЗНАЙШОВ ВОВА, А НЕ СТЕНД.
// Перша версія запиту перелічувала колонки поіменно, і серед них був
// `author_name` — поля, якого в `posts` НЕМАЄ (воно є лише в `threads`). Назву я
// взяв із власної фікстури нижче, а не з живої схеми. PostgREST відхиляв увесь
// запит, функція віддавала `[]`, і секція оголошень просто не малювалась —
// МОВЧКИ, без жодної ознаки поломки на екрані.
//
// ⚠️ ЧОМУ ЦЕЙ СТЕНД НЕ МІГ ЦЬОГО ЗЛОВИТИ ВИМІРОМ: підроблена база
// (`_board-fixture.mjs`) ігнорує `.select()` і віддає рядки цілком. Тобто саме
// перелік колонок — та частина запиту, яку в нас НІЧИМ перевірити.
// ➡️ Тому правило просте і перевіряється текстом: поки фікстура не розуміє
// `select`, запити до `posts` беруть `*`. Це не «лінь описати поля» — це відмова
// від конструкції, помилку в якій ніхто не побачить до скарги користувача.
const селект = (тіло.match(/\.select\((['"`])([\s\S]*?)\1\)/) || [])[2];
ok('запит бере * (перелік колонок нічим перевірити — вже ламалось)',
   селект === '*',
   селект === '*' ? "select('*')" : `перелічено поіменно: ${(селект || '').slice(0, 60)}…`);
