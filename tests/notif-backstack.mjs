...
// ⚠️ 16.08 — ЗАСТАВКА БІЛЬШЕ НЕ ТРИМАЄТЬСЯ ФІКСОВАНИЙ ЧАС. Вона зникає, щойно
// готовий перший кадр (межі 700..3500мс, `hideSplashWhenReady()` в `app.js`), тож
// раніше тут стояло «3.5с показу + 0.4с згасання» — опис, який став неправдою.
// 🔑 Число лишається як ЗАПАС ДЛЯ СТАБІЛЬНИХ станів (яка вкладка відкрита, які
// екрани в стопці) — вони не згасають, і зайве очікування їм не шкодить.
// 🛑 Для того, що ЗГАСАЄ (підсвітка коментаря живе 2.4с), число не годиться
// принципово: чекати треба ПОДІЮ зникнення `#splash`. Саме на цьому стенд
// почервонів на працездатному коді — розбір нижче, у блоці 5-7.
const ПІСЛЯ_ЗАСТАВКИ = 4600;
const ПІД_ЗАСТАВКОЮ  = 500;   // заставка живе щонайменше 700мс — тут вона гарантовано ще на екрані
...
// ── 5-7. КОМЕНТАР: лист і підсвітка мусять бути ВИДНІ, а не згаснути ─────────
{
  ...
  // 🔴 16.08 — ЧЕКАЄМО ПОДІЮ, А НЕ ЧИСЛО. Тут стояв `waitForTimeout(4600 …)`,
  // виведений із тодішньої тривалості заставки (3.5с + 0.4с). Коли заставку
  // прискорили до «зникає, щойно екран готовий» (~1.1с), замір поїхав У ХВІСТ
  // підсвітки: вона починається одразу після заставки, живе 2.4с — і на 5-й
  // секунді її вже НЕМА. Стенд червонів на коді, який працює краще за старий.
  // 🔑 Правильний критерій той самий, що й для людини: «щойно заставка зійшла,
  // підсвітка має бути видною». Тому чекаємо саме зникнення `#splash` і даємо
  // невеликий запас на прокрутку до картки — і жодних чисел, виведених із
  // тривалості, яку завтра знову змінять.
  await s.page.waitForFunction(() => !document.getElementById('splash'), null, { timeout: 15000 })
    .catch(() => {});
  await s.page.waitForTimeout(700);
