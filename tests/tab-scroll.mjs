...
// для Дошки це її власні дані про оголошення.
//
// 🔴 25.09 — А ОСЬ І ДРУГИЙ РАЗ, КОЛИ ЦЯ Ж ПЕРЕВІРКА ЗБРЕХАЛА, і того ж класу.
// Вона рахувала звернення до таблиці на імʼя — `posts`. Коли Дошку перевели на
// вітрину `posts_public` (міграція 0001, закриття витоку телефонів), поведінка
// НЕ змінилась ні на крок: тап так само тягне перечитування. А перевірка
// почервоніла, бо стерегла НАЗВУ, а не наслідок. Тому тепер рахуються обидва
// джерела Дошки: питання «чи сходив застосунок по оголошення», а не «чи сходив
// він саме в таблицю з таким іменем».
await p.evaluate(() => window.switchTab('board'));
await p.waitForTimeout(900);
await p.evaluate(() => { window.__cstlQueries = {}; });
await p.evaluate(() => document.querySelector('.tab-item[data-tab="board"]')?.click());
await p.waitForTimeout(1500);
const запитів = await p.evaluate(() => {
  const q = window.__cstlQueries || {};
  return (q.posts || 0) + (q.posts_public || 0);
});
ok('🔴 тап по активній вкладці тягне легке оновлення даних', запитів >= 1,
   `перечитувань оголошень: ${запитів}`);
