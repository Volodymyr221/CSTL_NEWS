...
const кнопка = await pg.evaluate(() => {
  ...
  return {
    ...
    рамка: cs.borderTopWidth,
    ширина: parseFloat(cs.width),
    колоШирина: parseFloat(getComputedStyle(document.querySelector('.tab-home-circle')).width),
  };
});
ok('сцена: кругла кнопка «Громада» на місці', !!кнопка);
if (кнопка) {
  ok('🔴 підкладка під колом ЗАЛИТА (не порожній контур)',
     кнопка.фон !== 'rgba(0, 0, 0, 0)' && кнопка.фон !== 'transparent', кнопка.фон);
  // Біла кайма навколо кола лишається — це задум, а не побічний ефект.
  ok('підкладка ширша за коло, тож біла кайма видима',
     кнопка.ширина > кнопка.колоШирина,
     `${кнопка.ширина}px проти ${кнопка.колоШирина}px`);
}
