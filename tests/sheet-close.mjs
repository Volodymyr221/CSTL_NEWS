...
async function run(freeze) {
  ...
  const frames = await p.evaluate(async (fr) => {
    const out = [window.__geom()];
    window.__dismiss(fr);
    for (let i = 0; i < 14; i++) { await new Promise(r => requestAnimationFrame(r)); out.push(window.__geom()); }
    return out;
  }, freeze);
  await p.close();
  return frames;
...
for (const [label, freeze] of [['СТАРА поведінка (висота росте)', false], ['НОВА поведінка (висота заморожена)', true]]) {
  ...
  if (freeze) {
    ...
    const decel = steps.slice(peak + 1).every((d, i, a) => i === 0 || d <= a[i-1] + 1);
    ok('після піку — рівне гальмування, без ривків', decel, `кроки: ${steps.join(', ')}`);
  } else {
