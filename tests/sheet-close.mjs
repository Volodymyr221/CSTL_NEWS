...
for (const [label, freeze] of [['СТАРА поведінка (висота росте)', false], ['НОВА поведінка (висота заморожена)', true]]) {
  ...
  if (freeze) {
    ...
    const samples = f.slice(1)
      .map((x, i) => ({ v: (x.top - f[i].top) / Math.max(x.t - f[i].t, 1), d: x.top - f[i].top, dt: x.t - f[i].t }))
      .filter(o => o.d > 0);
    // ⚠️ ВІДКИДАЄМО ПРОБИ З АНОМАЛЬНИМ ІНТЕРВАЛОМ — це шум ПРИЛАДУ, не рух аркуша.
    // Під навантаженням (повний прогін стендів) браузер то пропускає кадр (довгий Δt),
    // то видає навздогінні кадри поспіль (Δt близький до нуля). Обидва напрямки псують
    // швидкість: заміряно 0.48 → 2.31 у середині рівного гальмування і 11.29 на старті.
    // Тому лишаємо лише проби, чий інтервал у межах [0.5×, 2×] від медіанного.
    const dts = samples.map(o => o.dt).slice().sort((a, b) => a - b);
    const med = dts.length ? dts[Math.floor(dts.length / 2)] : 16;
    const vel = samples.filter(o => o.dt >= med * 0.5 && o.dt <= med * 2).map(o => o.v);
