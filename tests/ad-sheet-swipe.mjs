...
const st = () => p.evaluate(() => {
  ...
  return {
    ...
    фото_верх_точно: photo ? rc(photo).top : null,
    аркуш_верх_точно: sheet ? rc(sheet).top : null,
    тінь_аркуша: sheet ? getComputedStyle(sheet).boxShadow : null,
    ...
    // Рядок «Учасник CSTL LIFE» і «Ще N оголошень автора» — 06.08 стали ДВОМА рядками.
    підпис_розмір: (() => { const s = q('.cm-ad-author-since'); return s ? parseFloat(getComputedStyle(s).fontSize) : null; })(),
    підпис_текст:  (() => { const s = q('.cm-ad-author-since'); return s ? s.textContent.trim() : null; })(),
    ще_текст:      (() => { const m = q('.cm-ad-author-more'); return m ? m.textContent.trim() : null; })(),
    імя_розмір:    (() => { const n = q('.cm-ad-author-name'); return n ? parseFloat(getComputedStyle(n).fontSize) : null; })(),
    // Два рядки = різні `top`. Міряємо позицію, а не наявність `<br>` чи `display`.
    підпис_і_ще_різні_рядки: (() => {
      const s = q('.cm-ad-author-since'), m = q('.cm-ad-author-more');
      return (s && m) ? Math.round(rc(m).top - rc(s).top) : null;
    })(),
...
// 🔴 ПРАВИЛО ПЕРЕВЕРНУТО 06.08 — на пряме замовлення Вови: «зроби фото з цим блоком одне
// ціле, щоб при скролі вони разом рухались, я хочу подивитись як буде виглядати».
// Було (02.08): «фотографія просто стоїть на місці, а блок опису наскролюється доверху»
// — `position: sticky; top: 0`. Тепер фото їде разом з аркушем, як одна сторінка.
// ⚠️ Це ЕКСПЕРИМЕНТ: Вова дивиться наживо. Повернути назад = один рядок у `.cm-ad-photo`
// (`position: sticky; top: 0`) і перевернути цей сторож назад.
// Міряємо НАСЛІДОК (обидва зсунулись на ту саму відстань), а не запис `position` —
// `sticky` без `top` або зі скролером-предком не прилипає взагалі, і сторож за іменем
// властивості цього не побачив би.
await p.evaluate(() => { document.querySelector('.cm-ad-scroll').scrollTop = 120; });
await p.waitForTimeout(60);
const sMove = await st();
const дФото = Math.round(s0.фото_верх_точно - sMove.фото_верх_точно);
const дАркуш = Math.round(s0.аркуш_верх_точно - sMove.аркуш_верх_точно);
ok('ФОТО і ОПИС їдуть разом — одне ціле',
   дФото > 20 && Math.abs(дФото - дАркуш) <= 1, `фото Δ${дФото}px · аркуш Δ${дАркуш}px`);
// Тінь угорі аркуша малювала межу між фото і описом — рівно те, чого більше нема.
ok('над аркушем немає тіні-межі', s0.тінь_аркуша === 'none', s0.тінь_аркуша);
await p.evaluate(() => { document.querySelector('.cm-ad-scroll').scrollTop = 0; });
await p.waitForTimeout(60);
...
// 🔴 03.09 — ВИМОГА ПЕРЕВЕРНУТА ЗАМОВЛЕННЯМ ВОВИ, І ЦЕ НЕ ПОСЛАБЛЕННЯ СТОРОЖА.
// Дослівно: «зробити не "дитячий" вигляд, а преміальний… не керуйся тим що є».
// Дизайн-аудит заміряв 97 вузлів КАПСОМ на кеглі ≥13px — виділення, яке стоїть
// скрізь, не виділяє нічого. Заголовок сторінки оголошення був найгучнішим із них
// (23px/800 капсом). Стеріг це правило сторож — тож правило й перевернуто ЯВНО,
// а не обійдено видаленням перевірки: капсу тут більше не мусить бути.
ok('назва НЕ капсом (03.09 — знято за замовленням Вови)',
   s0.капс_назви !== 'uppercase', s0.капс_назви);
...
// лише поки рідкісне.
// 🔴 03.09 — ПРАВИЛО ЗБЕРЕЖЕНО ЗА СУТТЮ, ЗМІНЕНО ЗА ФОРМОЮ. Воно казало «економія
// виділення», а міряло «капсом рівно одне». Тепер капсу на цьому екрані немає
// зовсім, тож той самий намір міряється прямо: КАПСУ НЕМАЄ НІДЕ. Це СИЛЬНІША
// вимога за попередню (нуль замість одного), а не послаблена.
ok('капсу на екрані оголошення немає зовсім',
   s0.капс_локації !== 'uppercase' && s0.капс_назви !== 'uppercase',
...
// 🔴 06.08 — ЦЕ СТОРОЖ НА РЕАЛЬНУ ПОЛОМКУ, ЯКУ ЗНАЙШОВ ВОВА, А НЕ СТЕНД.
// Розділення `community.css` → `board.css` (05.08) розрізало коментар навпіл, і
// браузер викинув `.cm-ad-author-since` цілком — рядок лишився без свого розміру
// й успадкував батьківський. Скарга: «чому більшими буквами почало писатись?»
// Тому міряємо ОБЧИСЛЕНИЙ розмір, а не наявність правила у файлі: саме різниця
// між «написано» і «діє» і була суттю бага. Загальний сторож на цей клас поломок
// у всіх файлах — `tests/css-alive.mjs`.
ok('підпис автора ДРІБНІШИЙ за імʼя (правило справді діє)',
   s0.підпис_розмір !== null && s0.підпис_розмір < s0.імя_розмір,
   `підпис ${s0.підпис_розмір}px проти імені ${s0.імя_розмір}px`);
// Замовлення Вови по знімку: «розділи так — „Учасник CSTL LIFE з червня 2026“ /
// „Ще 6 оголошень автора“». Було одним рядком через « · ».
ok('«Учасник…» і «Ще N оголошень» — ДВА окремі рядки',
   s0.підпис_і_ще_різні_рядки !== null && s0.підпис_і_ще_різні_рядки > 8,
   `зсув ${s0.підпис_і_ще_різні_рядки}px · «${s0.підпис_текст}» / «${s0.ще_текст}»`);
// Кількість оголошень рахується з уже завантаженого списку, а дата приїжджає
// мережею. Розділивши їх на два вузли, ми зробили неможливим стирання одного
// одним — раніше хвіст доводилось вирізати з тексту за видимим « · » і клеїти назад.
ok('у підписі більше немає склеєного хвоста «· ще N»',
   s0.підпис_текст && !s0.підпис_текст.includes('·'), s0.підпис_текст);
