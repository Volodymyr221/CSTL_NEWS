...
// ── Оголошення БЕЗ ФОТО: ТАК САМО, а не дзеркально ──────────────────────────
// 🔴 ПЕРЕПИСАНО 02.08 (пʼятий захід). Тут стояло протилежне правило: без фото — свайп
// УНИЗ, рисочка є, бічного немає. Вова скасував його словами «побудуй логічно модалку
// з фото чи без, немає різниці», і разом із ним пішов цілий клас багів (суперечка за
// вертикаль із нативним скролером).
//
// ⚠️ ЗАРАЗОМ ТУТ ЖИВ ХИБНИЙ ЗЕЛЕНИЙ. Перевірка «аркуш без фото закривається свайпом
// УНИЗ» проходила НЕ тому, що жест працював: попередня перевірка вже закрила модалку
// (свайп зліва спрацював, хоч за тодішнім правилом не мав), і наступна знайшла порожньо
// та зарахувала це як успіх. Тому кожна перевірка жесту тепер починається з ЯВНОГО
// підтвердження, що модалка відкрита, — інакше «закрилось» не означає нічого.
await p.waitForTimeout(300);
...
// Вертикаль тепер належить браузеру на ОБОХ екранах — тягнути вниз нічого не закриває.
await p.evaluate(() => { const s = document.querySelector('.cm-ad-scroll'); if (s) s.scrollTop = 0; });
await swipeFrom(195, 400, 0, 260);
ok('без фото: свайп УНИЗ більше не закриває (вертикаль у браузера)', await isOpen() === true);
const afterDownNo = await st();
ok('без фото: невдалий свайп униз не лишає слідів',
   afterDownNo.зсув_модалки === 'none' || afterDownNo.зсув_модалки === 'matrix(1, 0, 0, 1, 0, 0)',
   `${afterDownNo.зсув_модалки}`);
// А той самий жест, що й на екрані з фото, — закриває.
await swipeFrom(8, 420, 260, 0);
ok('без фото: свайп ЗЛІВА закриває — так само, як із фото', await isOpen() === false);
