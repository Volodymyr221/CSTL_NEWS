...
const halves = [...allCss.matchAll(/font-size:\s*(\d+\.5)px/g)].map(m => m[1]);
halves.length
...
const heavy = [...allCss.matchAll(/font-weight:\s*(800|900)\b/g)].map(m => m[1]);
heavy.length
...
// Підпис таб-бару: не капс і не дрібніший за 11px — це головна навігація.
const tab = await p.evaluate(() => {
  const el = document.querySelector('.tab-label');
  if (!el) return null;
  const s = getComputedStyle(el);
  return { fs: parseFloat(s.fontSize), tt: s.textTransform, t: el.textContent.trim() };
});
if (!tab) fail('підпис таб-бару знайдено', 'немає .tab-label');
else {
  tab.tt !== 'uppercase' ? ok(`підпис таб-бару не капсом («${tab.t}»)`, true)
                         : fail('підпис таб-бару не капсом', `«${tab.t}» капсом`);
  tab.fs >= 11 ? ok(`підпис таб-бару не дрібніший за 11px (${tab.fs}px)`, true)
               : fail('підпис таб-бару не дрібніший за 11px', `${tab.fs}px`);
}
