...
<<<<<<< HEAD
// 🔴 ВІДКОЧЕНО 10.08 ТОГО САМОГО ДНЯ. Гумку вмикали — Вова прислав знімок:
// над бордовим блоком привітання зʼявився сірий провал.
// 🔑 Мій вимір був правильний («фон вкладки темний»), а висновок хибний:
// зливатись зона мала б не з «екраном узагалі», а з тим, що лежить РІВНО НАД
// нею — бордовим блоком. Темно-сірий проти бордового читається як діра.
// ➡️ Правильна ціль (слова Вови): гумка починається НЕ від верху сторінки, а
// від скляних капсул — блок привітання не рухається взагалі. Це двошарова
// прокрутка, окремий блок роботи.
// 🛑 Тому тут стережеться ЗВОРОТНЕ: поки блок привітання не пришпилено, відскок
// на Громаді мусить лишатись вимкненим. Увімкнеш назад без пришпилення —
// повернеться той самий провал.
const гром = await наВкладці('community');
ok('🛑 Громада: відскок вимкнено, доки блок привітання не пришпилений',
   гром.overscroll === 'none', `overscroll-behavior-y: ${гром.overscroll}`);

ok('доказ, чому саме так: фон вкладки НЕ бордовий, тож провал був би видно',
