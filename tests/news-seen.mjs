...
const ариаБейджа = await p.evaluate(() =>
  document.querySelector('.cm-news-new')?.getAttribute('aria-label') || '');
// 🔁 ПЕРЕПИСАНО 01.09 «c». Тут стояло `/^\d+\s+(нова|нові|нових)$/` по ВИДИМОМУ
// тексту — і воно **червоніло назавжди** з 01.09, бо #1132 змінив бейдж на
// «20 у Громаді», а сторожа лишив. Класична пара: код правили, перевірку ні.
// ⚠️ Просто послабити регулярку було б утратою: вимога Вови («хай воно
// закінчення підбирає, одна нова, дві нових») нікуди не зникла — вона
// ПЕРЕЇХАЛА в `aria-label`. Повна форма на екрані дає 119px і ламає шапку 390px
// на два рядки (323px при доступних 322), коротка — 83px.
// 🔑 Саме це й наказує HOT_RULES №12: «узгодження форми слова перевіряй в
// `aria`, а не в мітці» — на вузькому екрані мітка коротшає, а читач екрана
// дістає рядок повністю, і помилка узгодження чутна там.
ok('бейдж на екрані називає РОЗДІЛ (коротка форма влазить у шапку)',
   /^\d+\s+у Громаді$/.test(текстБейджа), `«${текстБейджа}»`);
ok('🔴 відмінок не втрачено — він у `aria-label` для читача екрана',
   /^\d+\s+(нова|нові|нових)\s+у розділі Громада$/.test(ариаБейджа), `«${ариаБейджа}»`);
// 🛑 КОНТРОЛЬ: число в `aria` те саме, що на екрані. Без цього рядка обидві
// перевірки зеленіли б і тоді, коли підпис для читача екрана відстав від видимого.
ok('КОНТРОЛЬ: число в `aria` збігається з видимим',
   parseInt(ариаБейджа, 10) === parseInt(текстБейджа, 10),
   `${parseInt(ариаБейджа, 10)} проти ${parseInt(текстБейджа, 10)}`);
