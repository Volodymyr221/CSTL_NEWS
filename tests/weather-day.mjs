...
{
  const { p, ctx, падіння, описКапсули, підКапсули } = await сцена({ погода: зробитиПогоду() });
  ...
  if (s) {
    ...
    // 🔴 Т5 (08.08) — КАПСУЛА І МОДАЛКА НАЗИВАЮТЬ ОДНУ Й ТУ САМУ ГОДИНУ ДОЩУ.
    // Було два незалежні пороги: капсула мовчала до 60%, порада модалки спрацьовувала
    // з 40%. На реальному дні це давало «опади о 21:00» у капсулі і «Можливий дощ
    // з 19:00» у модалці — за один тап одне від одного. Та сама хвороба, що Т2
    // (два описи одного дня), лише про годину. Тепер обидві кличуть спільний
    // `першаМокраГодина`, тож розійтись не можуть у принципі.
    const годинаКапсули = (підКапсули.match(/(\d{2}):00/) || [])[1] || null;
    const годинаМодалки = (String(s.порада).match(/(\d{2}):00/) || [])[1] || null;
    ok('🔴 Т5 — капсула і модалка називають ту саму годину дощу',
       годинаКапсули !== null && годинаКапсули === годинаМодалки,
       `капсула «${підКапсули}» · модалка «${s.порада}»`);
