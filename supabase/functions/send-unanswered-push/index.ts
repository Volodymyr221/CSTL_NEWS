...
async function run(admin: Admin, сухий: boolean) {
  ...
  //
  // Умова одна: людина САМА щось робила в Питаннях за останні 90 днів. Саме в
  // Питаннях, а не будь-де: коментар під оголошенням Дошки («ще продаєте?»)
  // нічого не каже про готовність відповідати громаді.
  //
  // 🔴 25.09 — КОЛО РОЗШИРЕНО З ОДНОГО ФАКТУ ДО ТРЬОХ, І ЦЕ ВИПРАВЛЕННЯ ВАДИ,
  // ЯКА НЕ ДАВАЛА ЖОДНОЇ ПОМИЛКИ.
  // Було: «написала хоча б одну ВІДПОВІДЬ». Це справді факт про людину — але
  // недосяжний у ту мить, коли тип 4 найпотрібніший. 📐 Заміряно на живій базі
  // 25.09: відповідачів за 90 днів **двоє**, із них із пристроєм **один**. А на
  // першому дні запуску відповідачів НУЛЬ за визначенням — тобто «питання без
  // відповіді» мовчало б рівно тоді, коли перше питання громади й повисло без
  // відповіді. Курка і яйце з `KURS.md`, тільки в коді.
  // 🔑 ДОДАНО ДВА ФАКТИ ТОГО САМОГО КЛАСУ (людина зробила це сама, отже правило
  // №12 «сам зробив» витримано):
  //   • ставила питання             — `posts.owner_uid` (є з першого ж дня);
  //   • тиснула «Мене теж цікавить» — серце в `reactions` на питанні.
  // 📐 На живій базі 25.09 це дає 5 людей замість 2, а з пристроєм — той самий
  // ОДИН. Тобто сьогодні зміна не шле нікому зайвого: вона працює на запуск.
  // 🛑 Чого тут НЕМА навмисно: «усі, хто заходив у застосунок». Це вже не факт
  // про Питання, а розсилка всім — рівно те, від чого §14 застерігає, і ціна
  // помилки та сама: одне вимкнення сповіщень забирає й автобусні.
  const { data: усі_питання } = await admin
    .from('posts').select('id, owner_uid, created_at').eq('type', 'chat');
  const питання_всі = (усі_питання || []) as
    Array<{ id: number; owner_uid: string | null; created_at: string }>;
  const питання_ids: number[] = питання_всі.map((p) => p.id);
  ...
  // 2б. ХТО СТАВИВ ПИТАННЯ за той самий строк. Свого питання людина однаково не
  // отримає (§15 і крок 4 нижче), тож це не «нагадати тобі про тебе», а «ти вже
  // питав громаду — може, тепер ти знаєш відповідь».
  const автори = [...new Set(
    питання_всі.filter((p) => p.owner_uid && p.created_at > відколи).map((p) => p.owner_uid),
  )] as string[];

  // 2в. ХТО ТИСНУВ «МЕНЕ ТЕЖ ЦІКАВИТЬ» (серце на питанні).
  // 🛑 ФІЛЬТР ФОРМИ UUID ОБОВʼЯЗКОВИЙ, І ЦЕ НЕ ПЕРЕСТРАХОВКА. `reactions.user_id`
  // має тип TEXT і історично тримає ДВА види id: uid акаунта і анонімний id з
  // `localStorage` (у `getAnonId()` є запасна гілка `'anon-' + …`). А `notif_prefs.uid`
  // і `user_push_devices.uid` — `uuid`. Один сторонній рядок у `.in(...)` відкинув
  // би ВЕСЬ запит («invalid input syntax for type uuid»), тобто через чужий id
  // мовчки не отримав би сповіщення ЖОДЕН із кола. Той самий урок, що в
  // `send-answer-push` (правило «TEXT проти uuid» у `HOT_RULES.md`).
  const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  const { data: серця } = await admin
    .from('reactions').select('user_id').in('post_id', питання_ids).eq('emoji', '❤️')
    .limit(RESPONDER_ROWS);
  const зацікавлені = [...new Set(
    (серця || []).map((r: { user_id: string | null }) => r.user_id)
                 .filter((u: string | null): u is string => !!u && UUID.test(u)),
  )];

  // Коло = обʼєднання трьох фактів. `Set` прибирає людину, яка і питала, і
  // відповідала, і тиснула серце — інакше вона отримала б один push тричі.
  const коло: string[] = [...new Set([...відповідачі, ...автори, ...зацікавлені as string[]])];
  if (!коло.length) return { sent: 0, reason: 'no responders', dry_run: сухий };
  ...
  const { data: свіжі } = await admin
    ...
    .select('uid').in('uid', коло).gte('created_at', межа);
  ...
  const кому = await allowed(admin, коло.filter((u) => !тихі.has(u)), 'questions');
  ...
  if (сухий) {
    // 🔑 Сухий прогін показує ще й СКЛАД КОЛА і в кого немає пристрою — без цього
    // «коло звузилось до одного» видно тільки з бази руками, а це те саме, що
    // не видно (див. звіт нижче). Пристрої питаємо тим самим одним запитом, яким
    // шле справжній прогін, а не окремою логікою: інакше суха перевірка міряла б
    // не те, що робить робоча.
    const { data: свої_пристрої } = await admin
      .from('user_push_devices').select('uid').in('uid', [...план.keys()]);
    const є = new Set((свої_пристрої || []).map((d: { uid: string }) => d.uid));
    return {
      sent: 0, dry_run: true,
      circle: коло.length,
      circle_parts: { responders: відповідачі.length, askers: автори.length, interested: зацікавлені.length },
      no_device: [...план.keys()].filter((uid) => !є.has(uid)).length,
  ...
  // ── 5. НАДСИЛАННЯ ──────────────────────────────────────────────────────────
  const { надіслано, безПристрою } = await pushEach(
    admin, [...план].map(([uid, qs]) => ({ uid, payload: payloadFor(qs) })),
  );
  ...
  //
  // 🔴 25.09 — АЛЕ «ЖОДНОГО ПРИСТРОЮ В БАЗІ» — ЦЕ НЕ СПРОБА ЗВЕРНУТИСЬ.
  // Правило вище написане про МЕРТВУ підписку (рядок є, push не прийняли). У
  // людини без жодного рядка `user_push_devices` слати нікуди фізично, і запис у
  // журнал робив дві шкоди: 📐 у журналі живих рядків 12.09 і 16.09 з `sent: 0`
  // на ту саму людину — тобто вона двічі зʼїла свою триденну стелю, нічого не
  // отримавши. А в день, коли вона нарешті ввімкне сповіщення, стеля змусила б
  // її чекати ще три доби.
  const немає = new Set(безПристрою);
  const рядки = [...план].filter(([uid]) => !немає.has(uid)).map(([uid, qs]) => ({
  ...
  // 🔑 `no_device` У ЗВІТІ — ЩОБ ЧИСЛО НЕ БРЕХАЛО НАЗВОЮ (урок із `NOW.md`).
  // Досі звіт казав `people: 2` при `sent: 1`, і з нього виходило, що звернулись
  // до двох. Насправді в одного з них узагалі немає куди слати — а саме це число
  // і є ознакою «коло є, а дійти нікуди»: без нього звужене коло виглядає
  // здоровим доти, доки хтось не полізе в базу руками.
  return {
    sent: sum, people: рядки.length, no_device: безПристрою.length,
    circle: коло.length, questions: без_відповіді.length,
  };
}
...
async function pushEach(admin: Admin, завдання: { uid: string; payload: Record<string, unknown> }[]) {
  ...
  // Хто з плану взагалі не має пристрою. Це не збій доставки, а відсутність
  // адресата — і викликач мусить відрізняти одне від одного (див. крок 5).
  const безПристрою: string[] = [];
  if (!завдання.length) return { надіслано: результат, безПристрою };

  const uids = завдання.map((z) => z.uid);
  const { data: пристрої } = await admin.from('user_push_devices').select('*').in('uid', uids);
  if (!пристрої?.length) return { надіслано: результат, безПристрою: uids };
  ...
  for (const { uid, payload } of завдання) {
    ...
    if (!свої.length) { безПристрою.push(uid); continue; }
  ...
  return { надіслано: результат, безПристрою };
}
