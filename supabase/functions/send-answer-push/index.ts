...
async function handleAnswer(admin: Admin, commentId: number) {
  ...
  // ТИП 2 за §10 `docs/QA_CONCEPT.md` — `answer-reply`, «X відповів на ваш
  // коментар». Персональне: тобі відповіли. Без затримок і без вікна тиші.
  //
  // 🛑 ПАСТКА НУМЕРАЦІЇ, ЯКА ВЖЕ ОДИН РАЗ СПРАЦЮВАЛА (25.08). Блоки в цьому
  // файлі йдуть у порядку ПРІОРИТЕТУ (2 → 1 → 3, §10: людина отримує рівно
  // один push, найсильніший), а не за номерами типів. Тому перший блок — це
  // тип ДРУГИЙ. Коли ці блоки підписували «1, 2, 3» за порядком у файлі,
  // документація вийшла перевернутою: у пульсі, літописі й журналі стояло
  // «тип 1 «вам відповіли»», хоча «вам відповіли» — це тип 2. Код був
  // правильний, підписи ні. Тому далі кожен блок називає СВІЙ номер із §10.
  if (c.reply_to_id) {
  ...
  // ТИП 1 за §10 — `answer-new`, «X відповів на ваше питання». Автору запису,
  //    з вікном тиші, щоб живе обговорення не перетворилось на чергу
  //    сповіщень. Рахуємо по журналу: чи вже щось надсилалось під цим записом
  //    за останні QUIET_MINUTES хвилин.
  ...
  if (owner && !notified.has(owner)) {
    ...
    // 🛑 ТИП 2 (блок вище) ЦЬОГО НЕ ПИТАЄ навмисно: «вам відповіли на коментар»
    // — персональне звернення до конкретної людини, і воно не притишується
    // нічим. Те саме правило, за яким у типу 2 немає й вікна тиші.
    const дозволено = (await allowed(admin, [owner], isQuestion ? 'questions' : 'board')).length > 0;
    // 🔴 Нічна тиша — для ЦЬОГО типу (1) і для типу 3 нижче; §12 називає 1, 3, 4.
    // Тип 2 (блок вище) її не має навмисно: там особисте звернення, і §12 прямо
    // виводить його з-під правила.
  ...
  // ТИП 3 за §10 — `answer-interest`, 🆕 «МЕНЕ ТЕЖ ЦІКАВИТЬ»: тим, хто попросив
  // стежити за цим питанням.
  //
...
async function allowed(admin: Admin, uids: string[], topic: string) {
  ...
  for (const шматок of частинами(uids, 200)) {
    const { data, error } = await admin
      .from('notif_prefs').select('uid, buses, board, questions, feed').in('uid', шматок);
...
async function push(admin: Admin, uids: string[], payload: Record<string, unknown>) {
  ...
  // Шматками по 200 і сторінками: інакше довгий перелік uid або понад тисяча
  // пристроїв обрізались би мовчки (див. «ТРИ МЕЖІ» вгорі файлу).
  const devices: Array<{ id: number; endpoint: string; p256dh: string; auth_key: string }> = [];
  for (const шматок of частинами(uids, 200)) {
    devices.push(...await усіРядки<typeof devices[number]>((від, до) => admin
